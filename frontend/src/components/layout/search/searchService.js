...
export const fetchPatientData = async (query, callback) => {
  ...
  const endpoints = [
    ...
    `/search?lastName=${lastName || query}`,
    `/search?dateOfBirth=${query}`,
    `/search?nationalID=${query}`,
    `/search?patientID=${query}`,
    `/search?guid=${query}`,
    `/search?subjectNumber=${query}`,
    `/search?gender=${query}`,
    lastName ? `/search?firstName=${firstName}&lastName=${lastName}` : null,
  ].filter(Boolean);
