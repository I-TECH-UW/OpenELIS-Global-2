...
const METHOD_ENDPOINT = "/rest/MethodRenameEntry";
...
function MethodRenameEntry() {
  ...
  const entityName = "method";
  const [selectedItem, setSelectedItem] = useState({});

  const invalidateServerData = useInvalidateServerData();
  const { data: methodRename } = useServerData(METHOD_ENDPOINT);
  const methodListShow = methodRename?.methodList ?? [];
