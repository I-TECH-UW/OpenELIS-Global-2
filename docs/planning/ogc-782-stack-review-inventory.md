...
| T30 | 4196 | `089-order-entry-domain-actions.xml:21` | `MARK_RAN` silently skips on partial drift            | `090-…-guard.xml` — idempotent re-parent + deactivate, safe on any drift state (089 is already applied on AMR, so it cannot be edited) |
...
| `net::ERR_ABORTED /validation?type=order`       | spec + validation code identical to develop. `03 - E2E` runs on push to develop (last: 33886344900 @ d6bab7a5a5, success) — check that run's `E2E / Tests` for this spec before attributing                                                                                                                                                                                                                                                                                                                  | measure first                                                                                                                         |
...
| Cypress `#only-active` not checked              | stack changed `UserManagement.jsx`: role `<Select>` went from `defaultValue` to controlled `value={roleFilter}`; spec changed `adminUser()`→`uncheckAdminUser()` in a _later_ test. `activeUser()` asserts unchecked → click label → checked                                                                                                                                                                                                                                                                 | verify locally whether the controlled Select re-render resets the checkbox                                                            |
