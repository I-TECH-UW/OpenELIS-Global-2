...
4. At a representative small-to-medium laboratory volume, initial display,
   filtering, and movement between bench actions do not interrupt routine work.
   The newest activity is available first and complete history remains available
   on demand. Engineering qualification defines and records the environment,
   data shape, and diagnostic timing thresholds separately.
