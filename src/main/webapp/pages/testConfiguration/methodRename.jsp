...
<c:set var="MethodList" value="${form.methodList}" />
...
<script type="text/javascript">
    ...
    function setForEditing(methodId, name) {
        jQuery("#editDiv").show();
        jQuery("#MethodName").text(name);
...
<form:hidden path="methodId" id="methodId"/>
...
<div id="editDiv" style="display:none;">
    ...
    <h2><%=MessageUtil.getContextualMessage( "method.browse.title" )%>:<span id="methodName"></span></h2>
    <br>
    <table>
        <tr>
            <td></td>
            <th colspan="2" style="text-align: center"><spring:message code="method.methodName"/></th>
        </tr>
        <tr>
            <td></td>
   ...
   <table>
    <% while(itemCount < MethodList.size()){%>
    <tr>
        <td><input type="button" value='<%= ((IdValuePair)MethodList.get(itemCount)).getValue() %>'
                   onclick="setForEditing( '<%= ((IdValuePair)MethodList.get(itemCount)).getId() + "', '" + ((IdValuePair)MethodList.get(itemCount)).getValue() %>');"
                   class="textButton TestSection"/>
            <%
            	itemCount++;
                columnCount = 1;
            %></td>
        <% while(itemCount < MethodList.size() && ( columnCount < columns )){%>
        <td><input type="button" value='<%= ((IdValuePair)MethodList.get(itemCount)).getValue() %>'
                   onclick="setForEditing( '<%= ((IdValuePair)MethodList.get(itemCount)).getId() + "', '" + ((IdValuePair)MethodList.get(itemCount)).getValue() %>' );"
                   class="textButton TestSection"/>
            <%
