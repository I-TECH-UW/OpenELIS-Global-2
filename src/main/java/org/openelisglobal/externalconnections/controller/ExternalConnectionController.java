...
public class ExternalConnectionController extends BaseController {
  ...
  private static final String[] ALLOWED_FIELDS =
      new String[] {
        ...
        "externalConnection.lastupdated",
        "externalConnection.programmedConnection",
        "externalConnection.activeAuthenticationType",
        "externalConnection.uri",
        //
        "externalConnection.descriptionLocalization.lastupdated",
        "externalConnection.descriptionLocalization.id",
        "externalConnection.descriptionLocalization.localizedValue",
        //
        "externalConnection.nameLocalization.lastupdated",
        "externalConnection.nameLocalization.id",
        "externalConnection.nameLocalization.localizedValue",
        //
        "externalConnectionContacts*.id",
        "externalConnectionContacts*.lastupdated",
        //
        "externalConnectionContacts*.person.id",
        "externalConnectionContacts*.person.lastupdated",
        "externalConnectionContacts*.person.lastName",
        "externalConnectionContacts*.person.firstName",
        "externalConnectionContacts*.person.primaryPhone",
        "externalConnectionContacts*.person.email",
        //
        "certificateAuthenticationData.certificate",
        "certificateAuthenticationData.id",
        "certificateAuthenticationData.lastupdated",
        //
        "basicAuthenticationData.username",
        "basicAuthenticationData.password",
        "basicAuthenticationData.id",
        "basicAuthenticationData.lastupdated",
      };

  @Autowired private ExternalConnectionService externalConnectionService;
  ...
  @Autowired private TruststoreService truststoreService;
  ...
  public ModelAndView addEditExternalConnection(
      @Valid @ModelAttribute("form") ExternalConnectionForm form,
      @RequestParam(value = ID, required = false) Integer externalConnectionId)
      throws CertificateException, KeyStoreException, NoSuchAlgorithmException, IOException {

    List<ExternalConnectionContact> externalConnectionContacts =
        form.getExternalConnectionContacts() == null
            ? new ArrayList<>()
    ...
    if (form.getCertificateAuthenticationData() != null) {
      ...
      loadCertificateIntoTruststore(
          form.getExternalConnection().getNameLocalization().getLocalizedValue(),
          form.getCertificateAuthenticationData().getCertificate());
    }

    if (null == externalConnectionId || 0 == externalConnectionId) {
      externalConnectionService.createNewExternalConnection(
          externalConnectionAuthData, externalConnectionContacts, externalConnection);
    } else {
      externalConnectionService.updateExternalConnection(
          externalConnectionAuthData, externalConnectionContacts, externalConnection);
    }
  ...
  private void fillForm(
      Integer externalConnectionId, @ModelAttribute("form") ExternalConnectionForm form) {
    request.setAttribute(
        IActionConstants.PAGE_SUBTITLE_KEY,
    ...
    form.setExternalConnectionContacts(
        externalConnectionContactService.getAllMatching(
            "externalConnection.id", externalConnectionId));
    Map<AuthType, ExternalConnectionAuthenticationData> externalConnectionAuthData =
        externalConnectionAuthenticationDataService.getForExternalConnection(externalConnectionId);
    form.setBasicAuthenticationData(
        (BasicAuthenticationData) externalConnectionAuthData.get(AuthType.BASIC));
