...
public class AccessionValidationRestController extends BaseResultValidationController {
  ...
  private static final String[] ALLOWED_FIELDS =
      new String[] {
        ...
        "resultList*.noteId",
        "resultList*.resultId",
        "resultList*.hasQualifiedResult",
        "resultList*.sampleIsAccepted",
        "resultList*.sampleIsRejected",
        "resultList*.result",
        "resultList*.qualifiedResultValue",
        "resultList*.multiSelectResultValues",
  ...
  public AccessionValidationRestController(
  ...
  @ResponseBody
  public ResultValidationForm showAccessionValidationRange(
  ...
  private ResultValidationForm getResultValidation(
      HttpServletRequest request, ResultValidationForm form, Boolean doRange)
      throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
    ...
    if (GenericValidator.isBlankOrNull(newPage)) {
      ...
      ResultsValidationUtility resultsValidationUtility =
          SpringContext.getBean(ResultsValidationUtility.class);
      if (request.getRequestURI().contains("AccessionValidationRange")) {
        setRequestType(
            ts == null ? MessageUtil.getMessage("validation.range.title") : ts.getLocalizedName());
      } else if (request.getRequestURI().contains("ResultValidationByTestDate")) {
        setRequestType(
            ts == null ? MessageUtil.getMessage("validation.date.title") : ts.getLocalizedName());
      }
      if (!(GenericValidator.isBlankOrNull(form.getTestSectionId())
        ...
        filteredresultList =
            userService.filterAnalysisResultsByLabUnitRoles(
                getSysUserId(request), resultList, Constants.ROLE_VALIDATION);
  ...
  @PostMapping(
      value = "accessionValidationByRangeUpdate",
      produces = MediaType.APPLICATION_JSON_VALUE,
      consumes = MediaType.APPLICATION_JSON_VALUE)
  @ResponseBody
  public ResultValidationForm showAccessionValidationRangeSave(
      HttpServletRequest request,
      @Validated(ResultValidationForm.ResultValidation.class) @RequestBody
          ResultValidationForm form,
    ...
    if (checkResults.size() == 0) {
      ...
      LogEvent.logDebug(
          this.getClass().getSimpleName(), "ResultValidation()", "Attempted save of stale page.");
      return form;
    }
  ...
  private void createUpdateList(
      ...
      List<Result> resultUpdateList,
      List<Note> noteUpdateList,
      List<Result> deletableList,
      IResultSaveService resultValidationSave,
      boolean areListeners) {
        ...
        if (areResults(analysisItem)) {
          List<Result> results =
              createResultFromAnalysisItem(
                  analysisItem, analysis, analysis, noteUpdateList, deletableList);
  ...
  private void createNeededNotes(
      AnalysisItem analysisItem, Analysis analysis, List<Note> noteUpdateList) {
    if (analysisItem.getIsRejected()) {
      Note note =
          noteService.createSavableNote(
              analysis,
              NoteType.INTERNAL,
              MessageUtil.getMessage("validation.note.retest"),
              RESULT_SUBJECT,
              getSysUserId(request));
    ...
    if (!GenericValidator.isBlankOrNull(analysisItem.getNote())) {
      ...
      Note note =
          noteService.createSavableNote(
              analysis, noteType, analysisItem.getNote(), RESULT_SUBJECT, getSysUserId(request));
      noteUpdateList.add(note);
    }
  }

  private void addResultSets(
      Analysis analysis, Result result, IResultSaveService resultValidationSave) {
  ...
  private boolean finalResultAlreadySent(Result result) {
    List<DocumentTrack> documents =
        documentTrackService.getByTypeRecordAndTable(
            RESULT_REPORT_ID, RESULT_TABLE_ID, result.getId());
  ...
  private void createUpdateElisaList(
      List<AnalysisItem> resultItems, List<Analysis> analysisUpdateList) {
  ...
  protected TestResult getTestResult(AnalysisItem analysisItem) {
    ...
    if (TypeOfTestResultServiceImpl.ResultType.DICTIONARY.matches(analysisItem.getResultType())) {
      ...
      List<TestResult> testResultList =
          testResultService.getActiveTestResultsByTest(analysisItem.getTestId());
