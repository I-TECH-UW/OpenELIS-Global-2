...
public class RegisterFhirHooksTask {
  ...
  public void startTask() {
    ...
    try {
      Bundle returnedBundle =
          fhirClient.transaction().withBundle(subscriptionBundle).encodedJson().execute();
      LogEvent.logDebug(
          this.getClass().getSimpleName(),
          "startTask",
          "subscription bundle returned:\n"
              + fhirContext.newJsonParser().encodeResourceToString(returnedBundle));
    } catch (UnprocessableEntityException | DataFormatException e) {
      LogEvent.logError(
          "error while communicating subscription bundle to "
              + localFhirStorePath
              + " for "
              + fhirSubscriber.get(),
          e);
    ...
    DataExportTask dataExportTask =
        dataExportTaskService
            .getDAO()
            .findByEndpoint(fhirSubscriber.get())
            .orElse(new DataExportTask());
    Map<String, String> headers = new HashMap<>();
    headers.put(
        "Server-Name", ConfigurationProperties.getInstance().getPropertyValue(Property.SiteName));
    headers.put(
        "Server-Code", ConfigurationProperties.getInstance().getPropertyValue(Property.SiteCode));

    dataExportTask.setFhirResources(
        Arrays.asList(fhirSubscriptionResources).stream()
  ...
  private void removeOldSubscription() {
          ...
          (Bundle)
              fhirClient
                  ...
                  .where(
                      Subscription.RES_ID
                          .exactly()
                          .code(createSubscriptionIdForResourceType(resourceType)))
                  .execute();
    ...
    Bundle returnedBundle =
        fhirClient.transaction().withBundle(deleteTransactionBundle).encodedJson().execute();
    LogEvent.logTrace(
        this.getClass().getSimpleName(),
        "removeOldSubscription",
  ...
  private Subscription createSubscriptionForResource(ResourceType resourceType) {
    ...
    subscription.setText(
        new Narrative().setStatus(NarrativeStatus.GENERATED).setDiv(new XhtmlNode(NodeType.Text)));
    subscription.setStatus(SubscriptionStatus.REQUESTED);
    subscription.setReason(
        "bulk subscription to detect any Creates or Updates to resources of type " + resourceType);
