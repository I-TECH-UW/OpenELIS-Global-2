...
public class LoginUserServiceImpl extends AuditableBaseObjectServiceImpl<LoginUser, Integer>
  ...
  @Autowired protected LoginUserDAO baseObjectDAO;
  @Autowired private PasswordEncoder passwordEncoder;

  private Pattern BCRYPT_PATTERN =
      Pattern.compile("\\A\\$2[ya]?\\$\\d\\d\\$[./0-9A-Za-z]{53}"); // make sure this
  // variable is
  // current
  ...
  public void hashPassword(LoginUser login, String newPassword) {
    ...
    passwordExpiredDate.add(
        Calendar.MONTH,
        Integer.parseInt(
            SystemConfiguration.getInstance().getLoginUserChangePasswordExpiredMonth()));
  ...
  public boolean isHashedPassword(String password) {
    return BCryptPasswordEncoder.class.isInstance(passwordEncoder)
        && BCRYPT_PATTERN.matcher(password).matches();
  }
