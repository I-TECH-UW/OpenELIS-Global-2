...
public class UserModuleServiceImpl implements UserModuleService, IActionConstants {
  ...
  @Autowired PermissionModuleService<PermissionModule> permissionModuleService;
  @Autowired SystemUserModuleService systemUserModuleService;
  ...
  /**
   * TODO: Setup the user accessible buttons in the user object Preparing and setting the user
   * module select/add/update/delete disable/enable buttons
   *
   * @param request is HttpServletRequest
  ...
  public boolean isVerifyUserModule(HttpServletRequest request) throws LIMSRuntimeException {
    ...
    if (!isFound) {
      try {
        UserSessionData usd =
            (UserSessionData) request.getSession().getAttribute(USER_SESSION_DATA);
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          if (actionName.equals(userAssignedModule)
              || actionName.startsWith(userAssignedModule + "Menu")) {
            isFound = true;
            setupUserButtons(request, systemUserModule, actionName);
            // bugzilla 2154
            LogEvent.logInfo(
                "UserModuleServiceImpl",
                "isVerifyUserModule()",
    ...
    try {
      ...
      if (authentication != null) {
        ...
        if (principal instanceof UserDetails) {
          ...
          DefaultSaml2AuthenticatedPrincipal samlUser =
  ...
  /**
   * TODO: Setup the user accessible buttons in the user object Enabled the buttons for user type
   * admin
   *
   * @param request
   *     <p>N.B. It is not clear why business rules are in a DAO object.
   */
  @Override
  public void enabledAdminButtons(HttpServletRequest request) throws LIMSRuntimeException {
    String active = "true";
    if (request.getAttribute(org.openelisglobal.common.action.IActionConstants.DEACTIVATE_DISABLED)
        != null) {
      active =
          (String)
              request.getAttribute(
                  org.openelisglobal.common.action.IActionConstants.DEACTIVATE_DISABLED);
    }
  ...
  private void setupUserButtons(
      HttpServletRequest request, SystemUserModule systemUserModule, String actionName)
      throws LIMSRuntimeException {
