...
public class DictionaryDAOImpl extends BaseDAOImpl<Dictionary, String> implements DictionaryDAO {
  ...
  public List<Dictionary> getDictionaryEntrysByCategoryAbbreviation(
      String filter, String categoryFilter) throws LIMSRuntimeException {
    try {
      String sql = "";
      ...
      if (filter != null) {
        ...
        if (!StringUtil.isNullorNill(categoryFilter)) {
          sql =
              "from Dictionary d "
                  ...
                  + " null and upper(d.dictEntry) like upper(:param1) and d.isActive= "
                  + enquote(YES)
        ...
        if (!StringUtil.isNullorNill(categoryFilter)) {
          sql =
              "from Dictionary d where d.dictEntry like :param1 and d.isActive= "
                  + enquote(YES)
                  + " and d.dictionaryCategory.categoryName = :param2";
        } else {
      ...
      Query<Dictionary> query =
          entityManager.unwrap(Session.class).createQuery(sql, Dictionary.class);
  ...
  public List<Dictionary> getDictionaryEntrysByCategoryNameLocalizedSort(String categoryName)
    ...
    List<Dictionary> entries =
        getDictionaryEntrysByCategoryAbbreviation("categoryName", categoryName, false);
  ...
  @Transactional(readOnly = true)
  public List<Dictionary> getDictionaryEntrysByCategoryAbbreviation(
      String fieldName, String fieldValue, boolean orderByDictEntry) throws LIMSRuntimeException {
    try {
      ...
      LogEvent.logError(e);
      throw new LIMSRuntimeException(
          "Error in Dictionary getDictionaryEntrysByCategoryAbbreviation(String categoryFilter)",
          e);
    }
  }
  ...
  public boolean duplicateDictionaryExists(Dictionary dictionary) throws LIMSRuntimeException {
    try {
      ...
      if (dictionary.getDictionaryCategory() != null) {
        sql =
            "from Dictionary t where ((trim(lower(t.dictEntry)) = :param and"
        ...
        sql =
            "from Dictionary t where ((trim(lower(t.dictEntry)) = :param and t.dictionaryCategory"
      ...
      if (dictionary.getDictionaryCategory() != null) {
        query.setParameter(
            "param2", dictionary.getDictionaryCategory().getCategoryName().toLowerCase().trim());
      }
