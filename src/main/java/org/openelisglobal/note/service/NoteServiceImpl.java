...
public class NoteServiceImpl extends AuditableBaseObjectServiceImpl<Note, String>
  ...
  private static boolean SUPPORT_INTERNAL_EXTERNAL =
  ...
  @Autowired private NoteDAO baseObjectDAO;

  @Autowired private SampleQaEventService sampleQAService;
  @Autowired private ReferenceTablesService refTableService;
  ...
  public String getNotesAsString(
    ...
    List<Note> noteList =
        getNotesChronologicallyByRefIdAndRefTableAndType(
            noteObject.getObjectId(), noteObject.getTableId(), dbFilter);
  ...
  private List<Note> getNonConformityReasons(NoteObject noteObject) {
    ...
    if (noteObject.getBoundTo() == BoundTo.ANALYSIS) {
      ...
      notes.addAll(
          getNotesChronologicallyByRefIdAndRefTableAndType(
              sampleItem.getId(), SampleItemServiceImpl.getSampleItemTableReferenceId(), filter));

      sample = sampleItem.getSample();
      notes.addAll(
          baseObjectDAO.getNotesChronologicallyByRefIdAndRefTableAndType(
              sample.getId(), SampleServiceImpl.getTableReferenceId(), filter));
    ...
    if (sample != null) {
      ...
      for (SampleQaEvent event : sampleQAList) {
        if (sampleItem == null
            || event.getSampleItem() == null
            || sampleItem.getId().equals(event.getSampleItem().getId())) {
          notes.addAll(
              baseObjectDAO.getNotesChronologicallyByRefIdAndRefTableAndType(
                  event.getId(), QAService.TABLE_REFERENCE_ID, filter));
  ...
  public String getNotesAsString(
      ...
      EncodeContext context) {
    List<Note> noteList =
        getNotesChronologicallyByRefIdAndRefTable(
            noteObject.getObjectId(), noteObject.getTableId());
  ...
  private String notesToString(
    ...
    for (Note note : noteList) {
      if (prefixType) {
        builder.append(
            StringUtil.encodeForContext(getNotePrefix(note, excludeExternPrefix), context));
        builder.append(" ");
      }
  ...
  public List<Note> getNotes(NoteObject noteObject) {
    return getNotesChronologicallyByRefIdAndRefTable(
        noteObject.getObjectId(), noteObject.getTableId());
  }
  ...
  public Note getMostRecentNoteFilteredBySubject(NoteObject noteObject, String filter) {
    ...
    if (GenericValidator.isBlankOrNull(filter)) {
      ...
      noteList =
          getNoteByRefIAndRefTableAndSubject(
              noteObject.getObjectId(), noteObject.getTableId(), filter);
      if (!noteList.isEmpty()) {
  ...
  public List<Note> getTestNotesInDateRangeByType(Date lowDate, Date highDate, NoteType noteType) {
    return baseObjectDAO.getNotesInDateRangeAndType(
        lowDate,
  ...
  public List<Note> getNotesInDateRangeAndType(
    ...
    return getBaseObjectDAO()
        .getNotesInDateRangeAndType(lowDate, highDate, noteType, referenceTableId);
  }
  ...
  public List<Note> getNotesChronologicallyByRefIdAndRefTableAndType(
    ...
    return getBaseObjectDAO()
        .getNotesChronologicallyByRefIdAndRefTableAndType(objectId, tableId, filter);
  }

  @Override
  @Transactional(readOnly = true)
  public List<Note> getNoteByRefIAndRefTableAndSubject(
      String refId, String table_id, String subject) {
    Map<String, Object> propertyValues = new HashMap<>();
    propertyValues.put("referenceId", refId);
    propertyValues.put("referenceTableId", table_id);
