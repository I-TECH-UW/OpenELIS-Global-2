...
public class TestReflexUtil {
  ...
  private static ResultService resultService =
      SpringContext.getBean(org.openelisglobal.result.service.ResultService.class);
  ...
  private static TestReflexService testReflexService =
      SpringContext.getBean(TestReflexService.class);
   ...
   */
  public List<TestReflex> getTestReflexsForDictioanryResultTestId(
      String dictionaryId, String testId, boolean userChoiceOnly) {
    if (GenericValidator.isBlankOrNull(dictionaryId) || GenericValidator.isBlankOrNull(testId)) {
      return new ArrayList<>();
    }

    TestResult testResult =
        testResultService.getTestResultsByTestAndDictonaryResult(testId, dictionaryId);

    if (testResult == null) {
      return new ArrayList<>();
    }

    return userChoiceOnly
        ? testReflexService.getFlaggedTestReflexesByTestResult(testResult, USER_CHOOSE_FLAG)
        : testReflexService.getTestReflexesByTestResult(testResult);
  }

  public List<Analysis> addNewTestsToDBForReflexTests(
      List<TestReflexBean> newResults, String sysUserId) throws IllegalStateException {
    if (sysUserId == null) {
    ...
    for (TestReflexBean reflexBean : newResults) {
      ...
      if (!reflexesForResult.isEmpty()) {
        ...
        reflexesForResult.forEach(
            reflex -> testAnalyteIds.add(Integer.valueOf(reflex.getTestAnalyte().getId())));
    ...
    for (TestReflexBean reflexBean : newResults) {
      ...
      List<String> handledReflexIdList =
          getHandledReflexesForSample(sampleIdToHandledTestReflexIds, reflexBean);
            ...
            if (rule.getOverall().equals(ReflexRuleOptions.OverallOptions.ALL)) {
              ...
              if (analyteTestMap.get(analyteId) != null) {
                if (testAnalyteIds.size() == analyteTestMap.get(analyteId).size()) {
                  newReflexAnalyses =
                      handleAutomaticReflexes(
                          parentAnalysisList, reflexBean, handledReflexIdList, sysUserId);
                }
              }
            } else {
              newReflexAnalyses =
  ...
  private List<TestReflex> getReflexTests(TestReflexBean reflexBean) {
    ...
    List<TestReflex> reflexesForResult =
        reflexResolver.getTestReflexesForResult(reflexBean.getResult());
    if (resultType.equals("D")) {
      ...
      if (resultType.equals("N")) {
        reflexesForResult =
            reflexesForResult.stream()
                .filter(test -> applyNumericRelationRulesForReflex(test, reflexBean))
                .collect(Collectors.toList());
      } else {
        reflexesForResult =
            reflexesForResult.stream()
                .filter(test -> applyTextRelationRulesForReflex(test, reflexBean))
                .collect(Collectors.toList());
      }
    }
  ...
  private List<Analysis> handleUserSelectedReflexes(
      List<Analysis> parentAnalysisList, TestReflexBean reflexBean, String sysUserId) {
    ...
    for (String triggeringTests : reflexBean.getTriggersToSelectedReflexesMap().keySet()) {
      List<String> addedActionIds =
          reflexBean.getTriggersToSelectedReflexesMap().get(triggeringTests);
      ...
      for (String addedActionId : addedActionIds) {
        ...
        Optional<Analysis> newAnalysis =
            addReflexTest(
                reflex,
                reflexBean.getResult(),
                reflexBean.getPatient().getId(),
                reflexBean.getSample(),
                true,
                true,
                addedActionId,
                false,
  ...
  private List<Analysis> handleAutomaticReflexes(
      List<Analysis> parentAnalysisList,
      TestReflexBean reflexBean,
      List<String> handledReflexIdList,
      String sysUserId) {
    ...
    for (TestReflex reflexForResult : reflexesForResult) {
      ...
      // handledRefleIdList
      boolean siblingsSatisfied =
          checkIfSiblingsSatisfiedAndPopulateParentList(
              parentAnalysisList, reflexBean, handledReflexIdList, siblingsOfResultReflex);

      // All the conditions are satisfied so we can handle the
      // reflexes
      if (siblingsSatisfied) {
        boolean allSibAnalysisCausedReflex =
            doAllAnalysisHaveReflex(parentAnalysisList, reflexBean);
        ...
        for (TestReflex siblingReflex : siblingsOfResultReflex) {
          ...
          boolean addTest =
              siblingReflex.getAddedTestId() != null
                  && !siblingReflex.getAddedTestId().equals(reflexForResult.getAddedTestId());
          boolean handleAction =
              siblingReflex.getActionScriptletId() != null
                  && !siblingReflex
                      .getActionScriptletId()
                      .equals(reflexForResult.getActionScriptletId());
  ...
  private List<TestReflex> applyDictionaryRelationRulesForReflex(Result result) {
    ...
    reflexResolver
        .getTestReflexsByAnalyteAndTest(result)
        .forEach(
            reflexTest -> {
              if (reflexTest.getRelation() != null) {
                switch (reflexTest.getRelation()) {
                  ...
                  case OUTSIDE_NORMAL_RANGE:
                    List<ResultLimit> limits =
                        SpringContext.getBean(ResultLimitService.class)
                            .getResultLimits(result.getTestResult().getTest());
                    if (!limits.isEmpty()
                        && StringUtils.isNotBlank(limits.get(0).getDictionaryNormalId())) {
  ...
  private Boolean applyNumericRelationRulesForReflex(
      TestReflex reflexTest, TestReflexBean reflexBean) {
    if (reflexTest.getRelation() == null) {
      return false;
    }
    switch (reflexTest.getRelation()) {
      ...
      case GREATER_THAN:
        return Double.valueOf(reflexTest.getNonDictionaryValue())
            < Double.valueOf(reflexBean.getResult().getValue());
      case LESS_THAN:
        return Double.valueOf(reflexTest.getNonDictionaryValue())
            > Double.valueOf(reflexBean.getResult().getValue());
      ...
      case LESS_THAN_OR_EQUAL:
        return Double.valueOf(reflexTest.getNonDictionaryValue())
            >= Double.valueOf(reflexBean.getResult().getValue());
      case INSIDE_NORMAL_RANGE:
        return Double.valueOf(reflexBean.getResult().getValue())
                >= reflexBean.getResult().getMinNormal()
            && Double.valueOf(reflexBean.getResult().getValue())
                <= reflexBean.getResult().getMaxNormal();
      case OUTSIDE_NORMAL_RANGE:
        return !(Double.valueOf(reflexBean.getResult().getValue())
                >= reflexBean.getResult().getMinNormal()
            && Double.valueOf(reflexBean.getResult().getValue())
  ...
  private Boolean applyTextRelationRulesForReflex(
      TestReflex reflexTest, TestReflexBean reflexBean) {
    if (reflexTest.getRelation() == null) {
      return false;
    }
  ...
  private boolean doAllAnalysisHaveReflex(
      List<Analysis> parentAnalysisList, TestReflexBean reflexBean) {
    if (reflexBean.getResult().getAnalysis() == null
  ...
  protected boolean checkIfSiblingsSatisfiedAndPopulateParentList(
      List<Analysis> parentAnalysisList,
      TestReflexBean resultSet,
      List<String> handledReflexIdList,
      List<TestReflex> siblingsOfResultReflex) {
  ...
  protected List<String> getHandledReflexesForSample(
      Map<String, List<String>> handledReflexsBySample, TestReflexBean resultSet) {
  ...
  private Optional<Analysis> addReflexTest(
      TestReflex reflex,
      Result result,
      String patientId,
      Sample sample,
      boolean addTest,
      boolean handleAction,
      String actionSelectionId,
      boolean failOnDuplicateTest,
      String sysUserId) {
      ...
      if (scriptletReflex != null) {
        if (noConclusionInModifiedResult(reflexList)) {
          List<Result> resultList =
              resultService.getResultsForSample(reflexList.get(0).getSample());
          ...
          if (CD4_SCRIPTLET_ID.equals(scriptletReflex.getActionScriptlet().getId())) {
            ...
            Result calculatedResults =
                reflexAction.getCD4CalculationResult(reflexList.get(0).getSample());
  ...
  private boolean isConclusion(Result result) {
    ...
    return analyte != null
        && (CONCLUSION_ANAYLETE_ID.equals(analyte.getId())
            || CD4_ANAYLETE.getId().equals(analyte.getId()));
  }
}
