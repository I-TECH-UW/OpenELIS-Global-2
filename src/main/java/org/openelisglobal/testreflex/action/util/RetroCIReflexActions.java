...
public class RetroCIReflexActions extends ReflexAction {
  ...
  private static ObservationHistoryTypeService typeService =
  ...
  static {
    ...
    OBSERVATION_HIV_STATUS_ID =
        observationHistoryType == null ? null : observationHistoryType.getId();
  ...
  protected void handleScriptletAction(Scriptlet scriptlet) {
    if (scriptlet != null && INTERPERET_TYPE.equals(scriptlet.getCodeType())) {
      ...
      if (action.equals(HIV_INDETERMINATE)
          || action.equals(HIV_N)
          || action.equals(HIV_1)
          || action.equals(HIV_2)
          || action.equals(HIV_D)) {
  ...
  public Result getCD4CalculationResult(Sample sample) {
    ...
    List<Analysis> analysisList =
        analysisService.getAnalysisBySampleAndTestIds(sample.getId(), CD4_RESULT_TEST_DEPENDANCIES);
    ...
    Result CD4Result =
        resultService.getResultForAnalyteInAnalysisSet(
            ANALYTE_CD4_PER_RESULT.getId(), analysisIDList);
    Result GBResult =
        resultService.getResultForAnalyteInAnalysisSet(ANALYTE_GB_RESULT.getId(), analysisIDList);
    Result LymphResult =
        resultService.getResultForAnalyteInAnalysisSet(
    ...
    if (CD4Result != null && GBResult != null && LymphResult != null) {
      try {
        double result =
            Double.parseDouble(CD4Result.getValue(true))
                * Double.parseDouble(GBResult.getValue(true))
                * Double.parseDouble(LymphResult.getValue(true))
                * 0.1;
        ...
        LogEvent.logInfo(
            this.getClass().getSimpleName(), "getCD4CalculationResult", e.getMessage());
