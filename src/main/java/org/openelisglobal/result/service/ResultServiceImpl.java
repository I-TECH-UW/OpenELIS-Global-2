...
public class ResultServiceImpl extends AuditableBaseObjectServiceImpl<Result, String>
  ...
  private static ResultSignatureService signatureService =
  ...
  public String getSampleType(Result result) {
    return result.getAnalysis() != null
        ? typeOfSampleService.getNameForTypeOfSampleId(
  ...
  /**
   * This gets the simple value of the result, it treats multiresults as single dictionary values
   * and does not try to get the complete set
   *
  ...
  /**
   * This returns a textual representation of the result value. Multiselect results are returned as
   * a comma delimited string. If there is a qualified value it is not included
   *
   * @param printable If true the results will be suitable for printing, otherwise they will be
   *     suitable for a web form
  ...
  public String getResultValue(
      Result result, String separator, boolean printable, boolean includeUOM) {
    if (GenericValidator.isBlankOrNull(result.getValue())) {
      return "";
    }
      ...
      if (!resultList.isEmpty()) {
        if (resultList.size() == 1) {
          ...
          for (Result sibResult : dictionaryResults) {
            ...
            reportResult =
                (dictionary != null && dictionary.getId() != null)
  ...
  @Override
  public String getResultValueForDisplay(
      Result result, String separator, boolean printable, boolean includeUOM) {
    if (GenericValidator.isBlankOrNull(result.getValue())) {
      return "";
    }
  ...
  private String getDictValueForDisplay(Result result) {
    Dictionary dictionary = dictionaryService.getDataForId(result.getValue());
    if (dictionary != null) {
      return dictionary.getLocalizedName() == null
  ...
  public String getMultiSelectSelectedIdValues(Result result) {
    ...
    if (TypeOfTestResultServiceImpl.ResultType.MULTISELECT
        .getCharacterValue()
        .equals(getTestType(result))) {
      StringBuilder buffer = new StringBuilder();
      boolean firstPass = true;
  ...
  /**
   * @return true if any of the result limits for this test have a gender specification
   */
  ...
  public String getDisplayReferenceRange(Result result, boolean includeSelectList) {
    ...
    if (TypeOfTestResultServiceImpl.ResultType.NUMERIC.matches(result.getResultType())) {
      ...
      if (!limits.isEmpty()
          && !GenericValidator.isBlankOrNull(limits.get(0).getDictionaryNormalId())) {
        range =
            dictionaryService
                .getDataForId(limits.get(0).getDictionaryNormalId())
                .getLocalizedName();
      }
    }
    return range;
  ...
  private List<ResultLimit> getResultLimits(Result result) {
    Test test = result.getAnalysis() != null ? result.getAnalysis().getTest() : null;
    List<ResultLimit> resultLimit =
        test != null
            ? resultLimitService.getAllResultLimitsForTest(test.getId())
            : new ArrayList<>();
  ...
  public static List<Result> getResultsInTimePeriodWithTest(
      Date startDate, Date endDate, String testId) {
    return baseObjectDAO.getResultsForTestInDateRange(
        testId, startDate, DateUtil.addDaysToSQLDate(endDate, 1));
  }

  public static List<Result> getResultsInTimePeriodInPanel(
      Date lowDate, Date highDate, String panelId) {
    return baseObjectDAO.getResultsForPanelInDateRange(
        panelId, lowDate, DateUtil.addDaysToSQLDate(highDate, 1));
  }

  public static List<Result> getResultsInTimePeriodInTestSection(
      Date lowDate, Date highDate, String testSectionId) {
    return baseObjectDAO.getResultsForTestSectionInDateRange(
        testSectionId, lowDate, DateUtil.addDaysToSQLDate(highDate, 1));
  }
  ...
  @Override
  @Transactional(readOnly = true)
  public List<Result> getResultsForTestSectionInDateRange(
