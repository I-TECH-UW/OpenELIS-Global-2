...
public class PatientResultsController extends BaseController {
  ...
  public ModelAndView showPatientResults(HttpServletRequest request)
    ...
    System.out.println(
        "Get:PatientResultsController:showPatientResults:patientID:"
    ...
    patientSearch.setSelectedPatientActionButtonText(
        MessageUtil.getMessage("resultsentry.patient.search"));
    ...
    if (GenericValidator.isBlankOrNull(newPage)) {
      ...
      if (!GenericValidator.isBlankOrNull(patientID)) {
        ...
        } else if (statusRules.equals(STATUS_RULES_HAITI)
            || statusRules.equals(STATUS_RULES_HAITI_LNSP)) {
          resultsUtility.addExcludedAnalysisStatus(AnalysisStatus.Canceled);
          resultsUtility.addExcludedAnalysisStatus(AnalysisStatus.SampleRejected);
        }
        ...
        List<TestResultItem> filteredResults =
            userService.filterResultsByLabUnitRoles(
                getSysUserId(request), results, Constants.ROLE_RESULTS);
