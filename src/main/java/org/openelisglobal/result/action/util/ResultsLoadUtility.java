...
public class ResultsLoadUtility {
  ...
  @Autowired private ResultSignatureService resultSignatureService;
  ...
  private static boolean depersonalize =
  ...
  private static boolean supportReferrals =
      FormFields.getInstance().useField(Field.ResultsReferral);
  private static boolean useInitialSampleCondition =
      FormFields.getInstance().useField(Field.InitialSampleCondition);
  private boolean useCurrentUserAsTechDefault =
      ConfigurationProperties.getInstance()
  ...
  public void addIdentifingPatientInfo(Patient patient, PatientInfoForm form) {
    ...
    form.setNationalId(
        GenericValidator.isBlankOrNull(patient.getNationalId())
  ...
  public List<TestResultItem> getUnfinishedTestResultItemsInTestSection(String testSectionId) {
    ...
    List<Analysis> fullAnalysisList =
  ...
  public int getTotalCountAnalysisByTestSectionAndStatus(String testSectionId) {
    return analysisService.getCountAnalysisByTestSectionAndStatus(
        testSectionId, analysisStatusList, sampleStatusList);
  }

  public List<TestResultItem> getGroupedTestsForAnalysisList(
      List<Analysis> filteredAnalysisList, boolean forwardSort) throws LIMSRuntimeException {
    ...
    for (Analysis analysis : filteredAnalysisList) {
      ...
      if (depersonalize) {
        ...
        patientInfo =
            nationalId
                + ", "
                + patientService.getGender(currentPatient)
                + ", "
                + patientService.getBirthdayForDisplay(currentPatient);
      }

      currSample = analysis.getSampleItem().getSample();
      List<TestResultItem> testResultItemList =
          getTestResultItemFromAnalysis(analysis, patientName, patientInfo, nationalId);
  ...
  private void reverseSortByAccessionAndSequence(List<? extends ResultItem> selectedTest) {
    Collections.sort(
        selectedTest,
        new Comparator<ResultItem>() {
          @Override
          public int compare(ResultItem a, ResultItem b) {
            int accessionSort =
                b.getSequenceAccessionNumber().compareTo(a.getSequenceAccessionNumber());
            ...
            if (accessionSort == 0) { // only the accession number sorting is reversed
              if (!GenericValidator.isBlankOrNull(a.getTestSortOrder())
                ...
                try {
                  return Integer.parseInt(a.getTestSortOrder())
                      - Integer.parseInt(b.getTestSortOrder());
  ...
  public void sortByAccessionAndSequence(List<? extends ResultItem> selectedTest) {
    Collections.sort(
        ...
        new Comparator<ResultItem>() {
          ...
          public int compare(ResultItem a, ResultItem b) {
            int accessionSort =
                a.getSequenceAccessionNumber().compareTo(b.getSequenceAccessionNumber());
  ...
  public void setSampleGroupingNumbers(List<? extends ResultItem> selectedTests) {
    ...
    for (ResultItem item : selectedTests) {
      if (!currentSequenceAccession.equals(item.getSequenceAccessionNumber())
          || item.getIsGroupSeparator()) {
        groupingNumber++;
  ...
  private List<TestResultItem> getTestResultItemFromAnalysis(
      Analysis analysis, String patientName, String patientInfo, String nationalId)
    ...
    for (Result result : resultList) {
      ...
      if (result != null) {
        if (useTechSignature) {
          List<ResultSignature> signatures =
              resultSignatureService.getResultSignaturesByResults(resultList);
  ...
  private String getInitialSampleConditionString(SampleItem sampleItem) {
    if (useInitialSampleCondition) {
      List<ObservationHistory> observationList =
          observationHistoryService.getObservationHistoriesBySampleItemId(sampleItem.getId());
  ...
  private ResultInventory getInventoryForResult(Result result) throws LIMSRuntimeException {
    List<ResultInventory> inventoryList =
        resultInventoryService.getResultInventorysByResult(result);
    ...
    for (Sample sample : samples) {
      ...
      for (SampleItem item : sampleItems) {
        ...
        for (Analysis analysis : analysisList) {
          ...
          List<TestResultItem> selectedItemList =
              getTestResultItemFromAnalysis(analysis, NO_PATIENT_NAME, NO_PATIENT_INFO, "");
  ...
  private void addUserSelectionReflexes(List<TestResultItem> testList) {
    ...
    for (TestResultItem resultItem : testList) {
      ...
      List<TestReflex> reflexList =
          reflexUtil.getPossibleUserChoiceTestReflexsForTest(resultItem.getTestId());
      ...
      for (TestReflex testReflex : reflexList) {
        if (!GenericValidator.isBlankOrNull(testReflex.getSiblingReflexId())) {
          if (possibleSibs) {
            TestResultItem sibTestResultItem =
                groupedSibReflexMapping.get(testReflex.getSiblingReflexId());
  ...
  private List<Analysis> getAnalysisForSampleItem(SampleItem item) {
    return analysisService.getAnalysesBySampleItemsExcludingByStatusIds(
        item, excludedAnalysisStatus);
  }

  private TestResultItem createTestResultItem(
      Analysis analysis,
      ...
      String techSignature,
      String techSignatureId,
      String initialSampleConditions,
      String sampleType) {
    ...
    ResultLimit resultLimit =
        SpringContext.getBean(ResultLimitService.class)
            .getResultLimitForTestAndPatient(test, currentPatient);

    String receivedDate =
        currSample == null ? getCurrentDate() : currSample.getReceivedDateForDisplay();
    ...
    String testDate =
        GenericValidator.isBlankOrNull(analysisService.getCompletedDateForDisplay(analysis))
            ? getCurrentDate()
    ...
    testItem.setReadOnly(
        isReadOnly(isConclusion, isCD4Conclusion) && result != null && result.getId() != null);
    ...
    testItem.setFailedValidation(
        statusRules.hasFailedValidation(analysisService.getStatusId(analysis)));
    ...
    testItem.setChildReflex(
        analysisService.getTriggeredReflex(analysis)
            && analysisService.resultIsConclusion(result, analysis));
    testItem.setPastNotes(notes);
    testItem.setDisplayResultAsLog(hasLogValue(test));
    testItem.setNonconforming(
        analysisService.isParentNonConforming(analysis)
  ...
  private void setResultLimitDependencies(
      ResultLimit resultLimit, TestResultItem testItem, List<TestResult> testResults) {
    if (resultLimit != null) {
      ...
      testItem.setUpperNormalRange(
          resultLimit.getHighNormal() == Double.POSITIVE_INFINITY
              ? 0
      ...
      testItem.setLowerCritical(
          resultLimit.getLowCritical() == Double.NEGATIVE_INFINITY
              ? 0
              : resultLimit.getLowCritical());
      testItem.setHigherCritical(
          resultLimit.getHighCritical() == Double.POSITIVE_INFINITY
              ? 0
              : resultLimit.getHighCritical());
      ...
      testItem.setNormalRange(
          SpringContext.getBean(ResultLimitService.class)
              .getDisplayReferenceRange(
                  resultLimit,
  ...
  private void setDictionaryResults(
      TestResultItem testItem, boolean isConclusion, Result result, List<TestResult> testResults) {
    if (isConclusion) {
  ...
  private void setDictionaryResults(
      TestResultItem testItem, List<TestResult> testResults, Result result) {
  ...
  private List<IdValuePair> getAnyDictionaryValues(Result result) {
    ...
    if (result != null
      ...
      List<Dictionary> dictionaryList =
          dictionaryService.getDictionaryEntriesByCategoryId(
              dictionaryValue.getDictionaryCategory().getId());
  ...
  public void addExcludedAnalysisStatus(AnalysisStatus status) {
    excludedAnalysisStatus.add(
        Integer.parseInt(SpringContext.getBean(IStatusService.class).getStatusID(status)));
  }
  ...
  public List<TestResultItem> getUnfinishedTestResultItemsByAccession(String accessionNumber) {
    List<Analysis> analysisList =
        analysisService.getPageAnalysisByStatusFromAccession(
            analysisStatusList, sampleStatusList, accessionNumber);

    return getGroupedTestsForAnalysisList(analysisList, SORT_FORWARD);
  }

  public List<TestResultItem> getUnfinishedTestResultItemsByAccession(
  ...
  public int getTotalCountAnalysisByAccessionAndStatus(String accessionNumber) {
    return analysisService.getCountAnalysisByStatusFromAccession(
        analysisStatusList, sampleStatusList, accessionNumber);
  }
}
