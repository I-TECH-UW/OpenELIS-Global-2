...
public class SchedulerConfig implements SchedulingConfigurer {
  ...
  public long getResultsResendTimeMillis() {
    ...
    String reportInterval =
        ConfigurationProperties.getInstance().getPropertyValue(Property.resultsResendTime);
  ...
  private synchronized void addOrRunSchedule(
      Scheduler reloadableScheduler, CronScheduler cronScheduler)
    ...
    LogEvent.logInfo(
        this.getClass().getSimpleName(), "addOrRunSchedule", "Adding cron job: " + jobName);
    ...
    Trigger trigger =
        newTrigger()
            .withIdentity(jobName + "Trigger", jobName)
            .withSchedule(cronSchedule(cronScheduler.getCronStatement()))
            .forJob(jobName + "Job", jobName)
            .build();
