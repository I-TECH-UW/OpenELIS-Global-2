...
public class TestUsageBacklog {
  ...
  public void run() {
    if (ConfigurationProperties.getInstance()
        .isPropertyValueEqual(Property.testUsageReporting, "true")) {
      handleBacklog();
    }
  }
  ...
  private Timestamp getLatestCollectionDate() {
    ReportExternalExport report =
        reportExternalExportService.getLatestEventReportExport(TEST_UTALIZATION_ID);
    if (report != null) {
  ...
  private void writeReportForDayPeriod(int daysAgo) {
    ...
    for (String id : testBucket.keySet()) {
      if (testBucket.get(id).intValue() > 0) {
        json.put(
            TestServiceImpl.getMap(TestServiceImpl.Entity.TEST_AUGMENTED_NAME).get(id),
            testBucket.get(id));
      }
    }
