...
public class ResultExporter {
  ...
  @Autowired private DocumentTrackService trackService;
  @Autowired private HL7MessageOutService hl7MessageService;
    ...
    if (shouldReportResults()) {
      ...
      for (ReportExternalExport report : reportList) {
        IRowTransmissionResponseHandler responseHandler =
            (IRowTransmissionResponseHandler) SpringContext.getBean("successReportHandler");
  ...
  private boolean shouldReportResults() {
    String reportResults =
        ConfigurationProperties.getInstance().getPropertyValueLowerCase(Property.reportResults);
