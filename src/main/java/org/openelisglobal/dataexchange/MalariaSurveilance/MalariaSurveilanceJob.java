...
public class MalariaSurveilanceJob implements Job {
  ...
  private static final String FINISHED_STATUS_ID =
  ...
  public void execute(JobExecutionContext arg0) throws JobExecutionException {
    ...
    if (sendReport) {
      ...
      new ReportTransmission()
          .sendRawReport(
              buffer.toString(), url, sendAsychronously, responseHandler, HTTP_TYPE.POST);
    }
  }
  ...
  private void writeReportForDayPeriod(int daysAgo) {
    ...
    fillMalariaBuckets(
        dayOne, dayTwo, MALARIA_TEST_NAMES, malairaBucket, MALARIA_DICTIONARY_ID_VALUE_MAP);
    fillMalariaBuckets(
        dayOne, dayTwo, RAPID_TEST_NAMES, rapidBucket, RAPID_DICTIONARY_ID_VALUE_MAP);

    writeMalariaResults(
        malairaBucket, "Recherche de plasmodiun - Especes", MALARIA_DICTIONARY_ID_VALUE_MAP);
    writeMalariaResults(rapidBucket, "Malaria Test Rapide", RAPID_DICTIONARY_ID_VALUE_MAP);
    buffer.append("</report>\n");
  }

  private void writeMalariaResults(
      Map<String, Integer> bucket, String name, Map<String, String> idValueMap) {
  ...
  private void fillMalariaBuckets(
      Timestamp dayOne,
      Timestamp dayTwo,
    ...
    List<Analysis> analysisList =
        analysisService.getAnalysisByTestDescriptionAndCompletedDateRange(
            testNames, sqlDayOne, sqlDayTwo);
  ...
  class ResponseHandler implements ITransmissionResponseHandler {
    ...
    private void setJobTimestamp(Timestamp nowTimestamp) {
      CronScheduler gatherScheduler =
          cronSchedulerService.getCronScheduleByJobName(CRON_MALARIA_SCHEDULER);
