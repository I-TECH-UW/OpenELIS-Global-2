...
public class DBOrderPersister implements IOrderPersister {
  ...
  private void persist(MessagePatient orderPatient) {
    ...
    patient =
        patient == null
            ? patientService.getPatientByExternalId(orderPatient.getExternalId())
            : patient;
    if (patient == null) {
  ...
  private void createNewPatient(MessagePatient orderPatient) {
    ...
    addIdentityIfAppropriate(
        IDENTITY_SUBJECTNUMBER_ID, orderPatient.getSubjectNumber(), identities);
  ...
  private void insertPatientAddress(MessagePatient orderPatient, Patient patient) {
    insertNewPatientInfo(
        ADDRESS_PART_COMMUNE_ID,
        orderPatient.getAddressCommune(),
        "T",
        patient.getPerson().getId());
  }

  private void addIdentityIfAppropriate(
      String typeId, String value, List<PatientIdentity> identities) {
  ...
  private void updatePatient(MessagePatient orderPatient, Patient patient) {
    ...
    updateIdentityIfNeeded(
        IDENTITY_OBNUMBER_ID,
        orderPatient.getObNumber(),
        patient.getId(),
        identityList,
        identityService);
    updateIdentityIfNeeded(
        IDENTITY_STNUMBER_ID,
        orderPatient.getStNumber(),
        patient.getId(),
        identityList,
        identityService);
    updateIdentityIfNeeded(
        IDENTITY_PCNUMBER_ID,
        orderPatient.getPcNumber(),
        patient.getId(),
        identityList,
        identityService);
    updateIdentityIfNeeded(
        IDENTITY_GUID_ID, orderPatient.getGuid(), patient.getId(), identityList, identityService);
    updateIdentityIfNeeded(
        IDENTITY_SUBJECTNUMBER_ID,
        orderPatient.getSubjectNumber(),
        patient.getId(),
        identityList,
        identityService);
  ...
  private void updateAddressPartsIfNeeded(MessagePatient orderPatient, String personId) {
    List<PersonAddress> personAddressList =
        personAddressService.getAddressPartsByPersonId(personId);
  ...
  private void updateIdentityIfNeeded(
      String identityTypeId,
      String newIdentityValue,
      String patientId,
      List<PatientIdentity> identityList,
  ...
  private void updatePatientIfNeeded(MessagePatient orderPatient, Patient patient) {
    ...
    if (needsUpdating(
        orderPatient.getDisplayDOB(), patientService.getBirthdayForDisplay(patient))) {
      patient.setBirthDateForDisplay(orderPatient.getDisplayDOB());
      updatePatient = true;
    }
  ...
  private void updatePersonIfNeeded(MessagePatient orderPatient, Patient patient, Person person) {
    ...
    if (needsUpdating(
        orderPatient.getAddressStreet(), patientService.getPerson(patient).getStreetAddress())) {
      person.setStreetAddress(orderPatient.getAddressStreet());
      updatePerson = true;
    }
    if (needsUpdating(
        orderPatient.getAddressVillage(), patientService.getPerson(patient).getCity())) {
      person.setCity(orderPatient.getAddressVillage());
      updatePerson = true;
    }
    if (needsUpdating(
        orderPatient.getAddressDepartment(), patientService.getPerson(patient).getState())) {
      person.setState(orderPatient.getAddressDepartment());
      updatePerson = true;
    }
    if (needsUpdating(
        orderPatient.getAddressCountry(), patientService.getPerson(patient).getCountry())) {
      person.setCountry(orderPatient.getAddressCountry());
      updatePerson = true;
    }
    ...
    if (needsUpdating(
        orderPatient.getWorkPhone(), patientService.getPerson(patient).getPrimaryPhone())) {
      person.setPrimaryPhone(orderPatient.getWorkPhone());
      updatePerson = true;
    }
    if (needsUpdating(
        orderPatient.getMobilePhone(), patientService.getPerson(patient).getPrimaryPhone())) {
      person.setPrimaryPhone(orderPatient.getMobilePhone());
      updatePerson = true;
    }
  ...
  public void cancelOrder(String referringOrderNumber) {
    if (!GenericValidator.isBlankOrNull(referringOrderNumber)) {
      List<ElectronicOrder> eOrders =
          eOrderService.getElectronicOrdersByExternalId(referringOrderNumber);
