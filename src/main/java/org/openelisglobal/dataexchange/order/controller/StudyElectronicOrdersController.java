...
public class StudyElectronicOrdersController extends BaseController {
  ...
  public ModelAndView rejectElectronicOrders(
  ...
  public ModelAndView showElectronicOrders(
      HttpServletRequest request,
      @ModelAttribute("form") @Valid ElectronicOrderViewForm form,
      BindingResult result) {
    ...
    try {
      ...
      for (ParameterComponent parameter : task.getInput()) {
        if (parameter
            ...
            .getCodingFirstRep()
            .getCode()
            .equals("CI0050005AAAAAAAAAAAAAAAAAAAAAAAAAAA")) { // VL
          // demand date
      ...
      Encounter encounter =
          fhirUtil
              .getFhirClient(defaultRemoteServer)
              .read()
  ...
  protected String findLocalForward(String forward) {
    if (FWD_SUCCESS.equals(forward)) {
      ...
      String searchParameters =
          "?searchType="
              + Encode.forUriComponent(searchType)
              + "&startDate="
              + Encode.forUriComponent(startDate)
              + "&endDate="
              + Encode.forUriComponent(endDate)
              + "&statusId="
              + Encode.forUriComponent(statusId);
