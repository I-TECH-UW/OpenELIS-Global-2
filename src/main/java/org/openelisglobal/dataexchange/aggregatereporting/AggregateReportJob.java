...
public class AggregateReportJob implements Job {
  ...
  private CronSchedulerService cronSchedulerService =
      SpringContext.getBean(CronSchedulerService.class);
  private SiteInformationService siteInfoService =
  ...
  public void execute(JobExecutionContext arg0) throws JobExecutionException {
    ...
    List<ReportExternalExport> sendableReports =
        reportExternalExportService.getUnsentReportExports(LAB_INDICATOR_REPORT_ID);
    ...
    if (!sendableReports.isEmpty()) {
      ...
      SiteInformation authInfo =
          siteInfoService.getSiteInformationByName("testUsageAggregationUserName");
      ...
      String url =
          ConfigurationProperties.getInstance().getPropertyValue(Property.testUsageReportingURL)
              + "/IndicatorAggregation"; //

      boolean sendAsychronously = false;
      ResponseHandler responseHandler =
          (ResponseHandler) SpringContext.getBean("aggregateResponseHandler");
      ...
      new ReportTransmission()
          .sendReport(wrapper, castorPropertyName, url, sendAsychronously, responseHandler);
  ...
  private void updateRunTimestamp() {
    CronScheduler reportScheduler =
        cronSchedulerService.getCronScheduleByJobName("sendSiteIndicators");
  ...
  class ResponseHandler implements ITransmissionResponseHandler {
    ...
    public void setReAttemptTry(
        SendingAggregateReportWrapper wrapper, String castorPropertyName, String url) {
