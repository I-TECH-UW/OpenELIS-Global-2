...
public class FhirTransformServiceImpl implements FhirTransformService {
  ...
  @Async
  @Override
  public AsyncResult<Bundle> transformPersistPatients(List<String> patientIds)
  ...
  public void transformPersistPatient(PatientManagementInfo patientInfo)
      throws FhirLocalPersistingException {
    CountingTempIdGenerator tempIdGenerator = new CountingTempIdGenerator();
    FhirOperations fhirOperations = new FhirOperations();
  ...
  @Async
  @Transactional(readOnly = true)
  public void transformPersistOrderEntryFhirObjects(
    ...
    Optional<ServiceRequest> referingServiceRequest =
        getReferringServiceRequestForSample(updateData.getSample());
    if (referingServiceRequest.isPresent()) {
      updateReferringServiceRequestWithSampleInfo(
          updateData.getSample(), referingServiceRequest.get());
    ...
    if (updateData.getProgramQuestionnaireResponse() != null) {
      updateData
          .getProgramQuestionnaireResponse()
          .setId(updateData.getProgramSample().getQuestionnaireResponseUuid().toString());
      this.addToOperations(
          fhirOperations, tempIdGenerator, updateData.getProgramQuestionnaireResponse());
    }
  ...
  private void updateReferringServiceRequestWithSampleInfo(
      Sample sample, ServiceRequest serviceRequest) {
    if (!serviceRequest.hasRequisition()) {
      serviceRequest.setRequisition(
          this.createIdentifier(
              fhirConfig.getOeFhirSystem() + "/samp_labNo", sample.getAccessionNumber()));
    }
  }

  private Optional<Task> getReferringTaskForSample(Sample sample) {
    List<ElectronicOrder> eOrders =
        electronicOrderService.getElectronicOrdersByExternalId(sample.getReferringId());
  ...
  public Practitioner transformProviderToPractitioner(Provider provider) {
    ...
    practitioner.addIdentifier(
        this.createIdentifier(
            fhirConfig.getOeFhirSystem() + "/provider_uuid", provider.getFhirUuidAsString()));
    practitioner.addName(
        new HumanName()
            .setFamily(provider.getPerson().getLastName())
  ...
  private List<ContactPoint> transformToTelecom(Person person) {
    ...
    contactPoints.add(
        new ContactPoint().setSystem(ContactPointSystem.PHONE).setValue(person.getPrimaryPhone()));
    contactPoints.add(
        new ContactPoint().setSystem(ContactPointSystem.EMAIL).setValue(person.getEmail()));
    contactPoints.add(
        new ContactPoint().setSystem(ContactPointSystem.FAX).setValue(person.getFax()));
  ...
  private Task transformToTask(Sample sample) {
    ...
    if (sample.getStatusId().equals(statusService.getStatusID(OrderStatus.Entered))) {
      task.setStatus(TaskStatus.READY);
        ...
        || sample
            .getStatusId()
            .equals(statusService.getStatusID(AnalysisStatus.TechnicalAcceptance))) {
      task.setStatus(TaskStatus.INPROGRESS);
    } else if (sample
        .getStatusId()
        .equals(statusService.getStatusID(AnalysisStatus.TechnicalRejected))) {
      task.setStatus(TaskStatus.FAILED);
    } else if (sample
            .getStatusId()
            .equals(statusService.getStatusID(OrderStatus.NonConforming_depricated))
        || sample
            .getStatusId()
            .equals(statusService.getStatusID(AnalysisStatus.BiologistRejected))) {
    ...
    task.addIdentifier(
        this.createIdentifier(
            fhirConfig.getOeFhirSystem() + "/order_accessionNumber", sample.getAccessionNumber()));

    for (Analysis analysis : analysises) {
      task.addBasedOn(
          this.createReferenceFor(ResourceType.ServiceRequest, analysis.getFhirUuidAsString()));
      if (sample.getStatusId().equals(statusService.getStatusID(OrderStatus.Finished))) {
        task.addOutput() //
            ...
            .setValue(
                this.createReferenceFor(
                    ResourceType.DiagnosticReport, analysis.getFhirUuidAsString()));
      }
    }
  ...
  private org.hl7.fhir.r4.model.Patient transformToFhirPatient(Patient patient) {
    ...
    fhirPatient.setIdentifier(
        createPatientIdentifiers(subjectNumber, nationalId, stNumber, guid, uuid));
  ...
  private Address transformToAddress(Person person) {
    ...
    List<PersonAddress> personAddressList =
        personAddressService.getAddressPartsByPersonId(person.getId());
  ...
  private List<Identifier> createPatientIdentifiers(
      String subjectNumber, String nationalId, String stNumber, String guid, String fhirUuid) {
    List<Identifier> identifierList = new ArrayList<>();
    if (!GenericValidator.isBlankOrNull(subjectNumber)) {
      identifierList.add(
          createIdentifier(fhirConfig.getOeFhirSystem() + "/pat_subjectNumber", subjectNumber));
    }
    if (!GenericValidator.isBlankOrNull(nationalId)) {
      identifierList.add(
          createIdentifier(fhirConfig.getOeFhirSystem() + "/pat_nationalId", nationalId));
    }
    if (!GenericValidator.isBlankOrNull(stNumber)) {
      identifierList.add(
          createIdentifier(fhirConfig.getOeFhirSystem() + "/pat_stNumber", stNumber));
    }
  ...
  private List<ServiceRequest> transformToServiceRequests(
      SamplePatientUpdateData updateData, SampleTestCollection sampleTestCollection) {
  ...
  private ServiceRequest transformToServiceRequest(Analysis analysis) {
    ...
    Organization organization =
        sampleService.getOrganizationRequester(
            sample, TableIdService.getInstance().REFERRING_ORG_TYPE_ID);
    Organization organizationDepartment =
        sampleService.getOrganizationRequester(
            sample, TableIdService.getInstance().REFERRING_ORG_DEPARTMENT_TYPE_ID);
    ...
    serviceRequest.addIdentifier(
        this.createIdentifier(
            fhirConfig.getOeFhirSystem() + "/analysis_uuid", analysis.getFhirUuidAsString()));
    serviceRequest.setRequisition(
        this.createIdentifier(
    ...
    if (organizationDepartment != null) {
      serviceRequest.addLocationReference(
          this.createReferenceFor(
              ResourceType.Location, organizationDepartment.getFhirUuidAsString()));
    }
    ...
    if (eOrders.size() <= 0) {
      ...
      serviceRequest.addBasedOn(
          this.createReferenceFor(ResourceType.ServiceRequest, sample.getReferringId()));
    ...
    if (analysis.getStatusId().equals(statusService.getStatusID(AnalysisStatus.NotStarted))) {
      ...
      serviceRequest.setStatus(ServiceRequestStatus.REVOKED);
        ...
        .getStatusId()
        .equals(statusService.getStatusID(AnalysisStatus.SampleRejected))) {
      serviceRequest.setStatus(ServiceRequestStatus.ENTEREDINERROR);
    } else {
      serviceRequest.setStatus(ServiceRequestStatus.UNKNOWN);
    }
    ObservationHistory program =
        observationHistoryService.getObservationHistoriesBySampleIdAndType(
            sample.getId(),
    ...
    serviceRequest.addSpecimen(
        this.createReferenceFor(
            ResourceType.Specimen, analysis.getSampleItem().getFhirUuidAsString()));
    serviceRequest.setSubject(
        this.createReferenceFor(ResourceType.Patient, patient.getFhirUuidAsString()));
    if (provider != null && provider.getFhirUuid() != null) {
  ...
  private CodeableConcept transformTestToCodeableConcept(Test test) {
    CodeableConcept codeableConcept = new CodeableConcept();
    codeableConcept.addCoding(
        new Coding("http://loinc.org", test.getLoinc(), test.getLocalizedTestName().getEnglish()));
    return codeableConcept;
  }
  ...
  private Specimen transformToSpecimen(SampleItem sampleItem) {
    ...
    specimen.addIdentifier(
        this.createIdentifier(
            fhirConfig.getOeFhirSystem() + "/sampleItem_uuid", sampleItem.getFhirUuidAsString()));
    ...
    specimen.setCollection(
        transformToCollection(sampleItem.getCollectionDate(), sampleItem.getCollector()));
  ...
  private CodeableConcept transformSampleConditionToCodeableConcept(String sampleConditionId) {
    return transformSampleConditionToCodeableConcept(
        observationHistoryService.get(sampleConditionId));
  }

  private CodeableConcept transformSampleConditionToCodeableConcept(
      ObservationHistory initialSampleCondition) {
    ...
    if (ValueType.DICTIONARY.getCode().equals(initialSampleCondition.getValueType())) {
      ...
      observationDisplay =
          dictionaryService.get(initialSampleCondition.getValue()).getDictEntryDisplayValue();
    ...
    condition.addCoding(
        new Coding(
            fhirConfig.getOeFhirSystem() + "/sample_condition",
            observationValue,
            observationDisplay));
    return condition;
  }

  private SpecimenCollectionComponent transformToCollection(
      Timestamp collectionDate, String collector) {
  ...
  private CodeableConcept transformTypeOfSampleToCodeableConcept(TypeOfSample typeOfSample) {
    ...
    codeableConcept.addCoding(
        new Coding(
            fhirConfig.getOeFhirSystem() + "/sampleType",
            typeOfSample.getLocalAbbreviation(),
    ...
    for (Analysis analysis : actionDataSet.getModifiedAnalysis()) {
      ...
      if (statusService.matches(analysis.getStatusId(), AnalysisStatus.Finalized)) {
        DiagnosticReport diagnosticReport =
            this.transformResultToDiagnosticReport(analysis.getId());
        this.addToOperations(fhirOperations, tempIdGenerator, diagnosticReport);
      }
    }
  ...
  @Override
  @Transactional(readOnly = true)
  public void transformPersistResultValidationFhirObjects(
    ...
    for (Sample sample : sampleUpdateList) {
      ...
      if (referringTask.isPresent()) {
        if (referingTaskMap.containsKey(referringTask.get().getIdElement().getIdPart())) {
          Task existingReferingTask =
              referingTaskMap.get(referringTask.get().getIdElement().getIdPart());
          updateReferringTaskWithTaskInfo(existingReferingTask, task);
          referingTaskMap.put(
              existingReferingTask.getIdElement().getIdPart(), existingReferingTask);
      ...
      if (referingServiceRequest.isPresent()) {
        if (referingServiceRequestMap.containsKey(
            referingServiceRequest.get().getIdElement().getIdPart())) {
          ServiceRequest existingServiceRequest =
              referingServiceRequestMap.get(
                  referingServiceRequest.get().getIdElement().getIdPart());
          updateReferringServiceRequestWithSampleInfo(sample, existingServiceRequest);
          referingServiceRequestMap.put(
              existingServiceRequest.getIdElement().getIdPart(), existingServiceRequest);
  ...
  private void addToOperations(
      FhirOperations fhirOperations, TempIdGenerator tempIdGenerator, Resource resource) {
  ...
  private DiagnosticReport transformResultToDiagnosticReport(Analysis analysis) {
    ...
    if (analysis.getStatusId().equals(statusService.getStatusID(AnalysisStatus.Finalized))) {
      ...
      diagnosticReport.setStatus(DiagnosticReportStatus.PARTIAL);
        ...
        .getStatusId()
        .equals(statusService.getStatusID(AnalysisStatus.NotStarted))) {
    ...
    diagnosticReport.addSpecimen(
        this.createReferenceFor(ResourceType.Specimen, sampleItem.getFhirUuidAsString()));
    diagnosticReport.setSubject(
        this.createReferenceFor(ResourceType.Patient, patient.getFhirUuidAsString()));
    for (Result curResult : allResults) {
      diagnosticReport.addResult(
          this.createReferenceFor(ResourceType.Observation, curResult.getFhirUuidAsString()));
    }
  ...
  private DiagnosticReport genNewDiagnosticReport(Analysis analysis) {
    ...
    diagnosticReport.addIdentifier(
        this.createIdentifier(
            fhirConfig.getOeFhirSystem() + "/analysisResult_uuid", analysis.getFhirUuidAsString()));
    return diagnosticReport;
  }
  ...
  private Observation transformResultToObservation(Result result) {
    ...
    observation.addIdentifier(
        this.createIdentifier(
            fhirConfig.getOeFhirSystem() + "/result_uuid", result.getFhirUuidAsString()));
    ...
    if (result
        .getAnalysis()
        .getStatusId()
        .equals(statusService.getStatusID(AnalysisStatus.Finalized))) {
    ...
    if (!GenericValidator.isBlankOrNull(result.getValue())) {
      ...
      if (result.getAnalysis().getTest().getName().equalsIgnoreCase("Viral Load")) {
        ...
        observation.setValue(
            new CodeableConcept(
                new Coding(
                    fhirConfig.getOeFhirSystem() + "/dictionary_entry",
                    dictionary.getDictEntry(),
                    dictionary.getLocalizedDictionaryName() == null
                        ? dictionary.getDictEntry()
    ...
    observation.setIssued(
        analysis.getReleasedDate()); // update to get Released Date instead of commpleted date
  ...
  public org.hl7.fhir.r4.model.Organization transformToFhirOrganization(Organization organization) {
    ...
    fhirOrganization.setId(
        organization.getFhirUuid() == null
            ? organization.getId()
            : organization.getFhirUuidAsString());
  ...
  public Organization transformToOrganization(org.hl7.fhir.r4.model.Organization fhirOrganization) {
    ...
    organization.setIsActive(
        Boolean.FALSE == fhirOrganization.getActiveElement().getValue()
            ? IActionConstants.NO
  ...
  private void setOeOrganizationIdentifiers(
      Organization organization, org.hl7.fhir.r4.model.Organization fhirOrganization) {
  ...
  private void setFhirOrganizationIdentifiers(
    ...
    if (!GenericValidator.isBlankOrNull(organization.getCliaNum())) {
      fhirOrganization.addIdentifier(
          new Identifier()
              .setSystem(fhirConfig.getOeFhirSystem() + "/org_cliaNum")
              .setValue(organization.getCliaNum()));
    }
    if (!GenericValidator.isBlankOrNull(organization.getShortName())) {
      fhirOrganization.addIdentifier(
          new Identifier()
              .setSystem(fhirConfig.getOeFhirSystem() + "/org_shortName")
              .setValue(organization.getShortName()));
    }
    if (!GenericValidator.isBlankOrNull(organization.getCode())) {
      fhirOrganization.addIdentifier(
          new Identifier()
              .setSystem(fhirConfig.getOeFhirSystem() + "/org_code")
              .setValue(organization.getCode()));
    }
    if (!GenericValidator.isBlankOrNull(organization.getCode())) {
      fhirOrganization.addIdentifier(
          new Identifier()
              .setSystem(fhirConfig.getOeFhirSystem() + "/org_uuid")
              .setValue(organization.getFhirUuidAsString()));
    }
  }

  private void setOeOrganizationTypes(
      Organization organization, org.hl7.fhir.r4.model.Organization fhirOrganization) {
  ...
  private void setOeOrganizationAddressInfo(
      Organization organization, org.hl7.fhir.r4.model.Organization fhirOrganization) {
    organization.setStreetAddress(
        fhirOrganization.getAddressFirstRep().getLine().stream()
  ...
  private void setFhirAddressInfo(
      org.hl7.fhir.r4.model.Organization fhirOrganization, Organization organization) {
  ...
  public void addHumanNameToPerson(HumanName humanName, Person person) {
    person.setFirstName(
        humanName.getGivenAsSingleString() == null
            ? ""
            : humanName.getGivenAsSingleString().strip());
