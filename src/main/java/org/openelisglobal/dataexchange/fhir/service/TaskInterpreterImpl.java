...
public class TaskInterpreterImpl implements TaskInterpreter {
  ...
  @Override
  public List<InterpreterResults> interpret(
  ...
  private MessagePatient createPatientFromFHIR() throws HL7Exception {
    ...
    for (Identifier identifier : patient.getIdentifier()) {
      if (identifier
          .getType()
          .hasCoding(fhirConfig.getOeFhirSystem() + "/genIdType", "externalId")) {
    ...
    if (birthDate != null) {
      String day =
          birthDate.getDay() == null
              ? DateUtil.AMBIGUOUS_DATE_SEGMENT
  ...
  private List<InterpreterResults> buildResultList(boolean exceptionThrown) {
    LogEvent.logDebug(
        this.getClass().getSimpleName(), "buildResultList", "buildResultList: " + exceptionThrown);
