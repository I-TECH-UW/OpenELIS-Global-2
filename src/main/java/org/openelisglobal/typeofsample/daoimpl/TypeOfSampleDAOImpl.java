...
@Component
@Transactional
public class TypeOfSampleDAOImpl extends BaseDAOImpl<TypeOfSample, String>
    implements TypeOfSampleDAO {
  ...
  public void getData(TypeOfSample typeOfSample) throws LIMSRuntimeException {
    try {
      TypeOfSample tos =
          entityManager.unwrap(Session.class).get(TypeOfSample.class, typeOfSample.getId());
      if (tos != null) {
  ...
  public List<TypeOfSample> getAllTypeOfSamples() throws LIMSRuntimeException {
    ...
    try {
      ...
      Query<TypeOfSample> query =
          entityManager.unwrap(Session.class).createQuery(sql, TypeOfSample.class);
  ...
  public List<TypeOfSample> getTypes(String filter, String domain) throws LIMSRuntimeException {
    ...
    try {
      ...
      if (!StringUtil.isNullorNill(domain)) {
        sql =
            "from TypeOfSample t where upper(t.description) like upper(:param) and t.domain ="
                + " :param2 order by upper(t.description)";
      } else {
        sql =
            "from TypeOfSample t where upper(t.description) like upper(:param) order by"
                + " upper(t.description)";
      }
      Query<TypeOfSample> query =
          entityManager.unwrap(Session.class).createQuery(sql, TypeOfSample.class);
  ...
  @Override
  @Transactional(readOnly = true)
  public List<TypeOfSample> getTypesForDomainBySortOrder(SampleDomain domain)
  ...
  public TypeOfSample getTypeOfSampleByLocalAbbrevAndDomain(String localAbbrev, String domain)
      throws LIMSRuntimeException {
    String sql =
        "From TypeOfSample tos where tos.localAbbreviation = :localAbbrev and tos.domain = :domain";
    try {
      Query<TypeOfSample> query =
          entityManager.unwrap(Session.class).createQuery(sql, TypeOfSample.class);
  ...
  public TypeOfSample getTypeOfSampleByDescriptionAndDomain(TypeOfSample tos, boolean ignoreCase)
    ...
    try {
      ...
      if (!StringUtil.isNullorNill(tos.getDomain())) {
        if (ignoreCase) {
          sql =
              "from TypeOfSample tos where trim(lower(tos.description)) = :param and tos.domain ="
                  + " :param2";
        } else {
          sql =
              "from TypeOfSample tos where trim(tos.description) = :param and tos.domain = :param2";
        }
      } else {
        if (ignoreCase) {
  ...
  // bugzilla 1482
  @Override
  public boolean duplicateTypeOfSampleExists(TypeOfSample typeOfSample)
      throws LIMSRuntimeException {
    try {
      ...
      String sql =
          "from TypeOfSample t where (trim(lower(t.description)) = :description and"
  ...
  public TypeOfSample getTypeOfSampleById(String typeOfSampleId) throws LIMSRuntimeException {
    try {
      TypeOfSample tos =
          entityManager.unwrap(Session.class).get(TypeOfSample.class, typeOfSampleId);
      return tos;
    } catch (RuntimeException e) {
  ...
  public TypeOfSample getSampleTypeFromTest(Test test) {
    String sql =
        "select tos from TypeOfSample tos, Test test, TypeOfSampleTest tost "
