...
public class SampleEditController extends BaseController {
  ...
  private static final String[] ALLOWED_FIELDS =
      new String[] {
        ...
        "isEditable",
        "maxAccessionNumber",
        "existingTests*.sampleItemChanged",
        "existingTests*.sampleItemId",
        "existingTests*.analysisId",
        "existingTests*.collectionDate",
        "existingTests*.collectionTime",
        "existingTests*.removeSample",
        "existingTests*.canceled",
        "possibleTests*.testId",
        "possibleTests*.sampleItemId",
        "possibleTests*.add"
      };

  @Autowired SampleEditFormValidator formValidator;
  ...
  @Autowired private SampleEditService sampleEditService;
  ...
  public ModelAndView showSampleEdit(
      HttpServletRequest request,
      @ModelAttribute("form") @Validated(SampleEdit.class) SampleEditForm oldForm,
    ...
    boolean isEditable =
        "readwrite".equals(request.getSession().getAttribute(SAMPLE_EDIT_WRITABLE))
    ...
    if (!GenericValidator.isBlankOrNull(accessionNumber)) {
      ...
      if (sample != null && !GenericValidator.isBlankOrNull(sample.getId())) {
        ...
        List<SampleEditItem> currentTestList =
            getCurrentTestInfo(sampleItemList, accessionNumber, allowedToCancelResults);
        ...
        if (sampleItemList.size() > 0) {
          maxAccessionNumber =
              accessionNumber + "-" + sampleItemList.get(sampleItemList.size() - 1).getSortOrder();
        } else {
  ...
  private void setAddableTestInfo(
      SampleEditForm form, List<SampleItem> sampleItemList, String accessionNumber)
      throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
  ...
  private void addPossibleTestsToList(
      SampleItem sampleItem, List<SampleEditItem> possibleTestList, String accessionNumber) {

    TypeOfSample typeOfSample = typeOfSampleService.get(sampleItem.getTypeOfSampleId());

    List<TypeOfSampleTest> typeOfSampleTestList =
        typeOfSampleTestService.getTypeOfSampleTestsForSampleType(typeOfSample.getId());
    ...
    if (!typeOfTestSampleItemList.isEmpty()) {
      ...
      typeOfTestSampleItemList
          .get(0)
          .setAccessionNumber(accessionNumber + "-" + sampleItem.getSortOrder());
  ...
  public ModelAndView showSampleEditUpdate(
      HttpServletRequest request,
    ...
    try {
      sampleEditService.editSample(
          form, request, updatedSample, sampleChanged, getSysUserId(request));
  ...
  protected String getPageTitleKey() {
    boolean isEditable =
        "readwrite".equals(request.getSession().getAttribute(SAMPLE_EDIT_WRITABLE))
            || "readwrite".equals(request.getParameter("type"));
    return isEditable
        ? MessageUtil.getContextualKey("sample.edit.title")
  ...
  protected String getPageSubtitleKey() {
    boolean isEditable =
        "readwrite".equals(request.getSession().getAttribute(SAMPLE_EDIT_WRITABLE))
            || "readwrite".equals(request.getParameter("type"));
    return isEditable
        ? MessageUtil.getContextualKey("sample.edit.subtitle")
