...
public abstract class BaseSampleEntryController extends BaseController {
  ...
  /**
   * various maps full of a various lists used by the entry form (typically for drop downs) and
   * other forms who want to do patient entry. @ all from setProperty problems caused by developer
   * mistakes.
   */
  ...
  /**
   * If the URL parameter says we want to do a particular type of request, figure out which type and
   * pass that on to any other controller page in the flow via an attribute of the session TODO
   * PAHill is putting it in the session just clutter? Should this be in the request instead?
   */
   ...
   * reselection, so that all forms know the current projectForm, so that the use is presented with
   ...
   */
  protected String handleSave(
      HttpServletRequest request, IAccessioner accessioner, IAccessionerForm form) {
    String forward;
    try {
  ...
  protected void logAndAddMessage(
    ...
    LogEvent.logError(
        this.getClass().getSimpleName(), methodName, "Unable to enter sample into system");
