...
public class SampleDAOImpl extends BaseDAOImpl<Sample, String> implements SampleDAO {
  ...
  public List<Sample> getPageOfSamples(int startingRecNo) throws LIMSRuntimeException {
    ...
    try {
      ...
      for (int i = 0; i < samples.size(); i++) {
        ...
        samp.setCollectionDateForDisplay(
            DateUtil.convertTimestampToStringDate(samp.getCollectionDate()));
        samp.setTransmissionDateForDisplay(
            DateUtil.convertSqlDateToStringDate(samp.getTransmissionDate()));
   ...
   * @return Sample The Sample for the specified accession number, or null if the accession number
   *     does not exist.
   */
  @Override
  @Transactional(readOnly = true)
  ...
  public List<Sample> getSamplesWithPendingQaEvents(
      Sample sample,
      boolean filterByQaEventCategory,
    ...
    try {
      ...
      if (filterByDomain) {
        if (filterByQaEventCategory) {
          sql =
              "from Sample s where s.id IN (select sqe.sample.id from SampleQaEvent sqe where"
  ...
  /**
   * @see org.openelisglobal.sample.dao.SampleDAO#getSamplesReceivedInDateRange(String, String)
   *     (java.lang.String, java.lang.String)
   */
  @Override
  @Transactional(readOnly = true)
  public List<Sample> getSamplesReceivedInDateRange(
      String receivedDateStart, String receivedDateEnd) throws LIMSRuntimeException {
    List<Sample> list;
    ...
    try {
      String sql =
          "from Sample as s where s.receivedTimestamp >= :start AND s.receivedTimestamp < :end";
      Query<Sample> query = entityManager.unwrap(Session.class).createQuery(sql, Sample.class);
  ...
  public List<Sample> getSamplesByProjectAndStatusIDAndAccessionRange(
    ...
    String sql =
        "from Sample s where s.statusId in (:statusList) and s.accessionNumber >= :minAccess and"
  ...
  public List<Sample> getSamplesByAccessionRange(String minAccession, String maxAccession)
    ...
    String sql =
        "from Sample s where s.accessionNumber >= :minAccess and s.accessionNumber <= :maxAccess";
    try {
      Query<Sample> query = entityManager.unwrap(Session.class).createQuery(sql, Sample.class);
      query.setParameter("minAccess", minAccession);
      query.setParameter("maxAccess", maxAccession);
  ...
  public String getLargestAccessionNumber() throws LIMSRuntimeException {
    ...
    try {
      ...
      LogEvent.logError(e);
      throw new LIMSRuntimeException(
          "Exception occurred in SampleDAOImpl.getLargestAccessionNumber", e);
    }
  ...
  public String getLargestAccessionNumberWithPrefix(String prefix) throws LIMSRuntimeException {
    ...
    try {
      String sql =
          "select max(s.accessionNumber) from Sample s where s.accessionNumber like :prefix";
      Query<String> query = entityManager.unwrap(Session.class).createQuery(sql, String.class);
  ...
  public String getLargestAccessionNumberMatchingPattern(String startingWith, int accessionSize)
    ...
    try {
      String sql =
          "select max(s.accessionNumber) from Sample s where s.accessionNumber LIKE :starts and"
  ...
  public List<Sample> getSamplesWithPendingQaEventsByService(String serviceId)
      throws LIMSRuntimeException {
    String sql =
        "Select sqa.sample From SampleQaEvent sqa where sqa.sample.id IN (select sa.sample.id from"
  ...
  public List<Sample> getConfirmationSamplesReceivedInDateRange(
      Date receivedDateStart, Date receivedDateEnd) throws LIMSRuntimeException {
    String sql =
        "from Sample s where s.isConfirmation = true and s.receivedTimestamp BETWEEN :lowDate AND"
            + " :highDate";
    try {
      Query<Sample> query = entityManager.unwrap(Session.class).createQuery(sql, Sample.class);
  ...
  public List<Sample> getSamplesBySampleItem(Integer sampleitemId) throws LIMSRuntimeException {
    ...
    String sql =
        "from Sample s where s.id in (select si.sample.id from SampleItem si where si.id ="
            + " :sampleitemId)";
    try {
      Query<Sample> query = entityManager.unwrap(Session.class).createQuery(sql, Sample.class);
  ...
  public List<Sample> getSamplesByAnalysisIds(List<String> analysisIds) {
    String hql =
        "FROM Sample s WHERE s.id IN (SELECT si.sample.id FROM SampleItem si WHERE si.id IN (SELECT"
  ...
  public List<Sample> getSamplesForSiteBetweenOrderDates(
    ...
    String hql =
        "FROM Sample s WHERE s.enteredDate BETWEEN :lowerDate AND :upperDate AND s.id IN (SELECT"
