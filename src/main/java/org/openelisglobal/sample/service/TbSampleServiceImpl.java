...
public class TbSampleServiceImpl implements TbSampleService {
  ...
  @Autowired private ObservationHistoryTypeService observationHistoryTypeService;
  ...
  @Autowired private SampleOrganizationService sampleOrganizationService;
  ...
  private List<String> persistObservations(SampleTbEntryForm formData) {
    ...
    tbFollowupReasonPeriodLine1.setObservationHistoryTypeId(
        getObservationHistoryTypeId("TbFollowupReasonPeriodLine1"));
    ...
    tbFollowupReasonPeriodLine2.setObservationHistoryTypeId(
        getObservationHistoryTypeId("TbFollowupReasonPeriodLine2"));
  ...
  private Patient persistPatientData(SampleTbEntryForm formData) {
    ...
    if (ObjectUtils.isEmpty(oldPatient)) {
      ...
      patient.setBirthDate(
          DateUtil.convertStringDateToTruncatedTimestamp(formData.getPatientBirthDate()));
      ...
      oldPatient.setBirthDate(
          DateUtil.convertStringDateToTimestamp(formData.getPatientBirthDate() + " 00:00"));
  ...
  private String createPatientIdentity(SampleTbEntryForm formData, String patientId) {
    ...
    PatientIdentity patientIdentity =
        patientIdentityService.getPatitentIdentityForPatientAndType(patientId, typeID);
  ...
  private void createPersonAddresses(SampleTbEntryForm formData, String personId) {
    // define addresses
    List<PersonAddress> existingAddresses =
    ...
    if (ObjectUtils.isEmpty(existingAddresses)) {
      ...
      patientStreetAddress.setAddressPartId(
          addressPartService.getAddresPartByName("street").getId());
      ...
      existingAddresses.forEach(
          address -> {
            if (address
                .getAddressPartId()
                .equals(addressPartService.getAddresPartByName("phone").getId())) {
              address.setValue(formData.getPatientPhone());
            }
            if (address
                .getAddressPartId()
                .equals(addressPartService.getAddresPartByName("street").getId())) {
  ...
  private String persistSampleData(SampleTbEntryForm formData) {
    ...
    if (ObjectUtils.isEmpty(formData.getSampleId())) {
      ...
      sample.setCollectionDate(
          DateUtil.convertStringDateToTruncatedTimestamp(formData.getRequestDate()));
  ...
  private List<String> persistAnalysisData(SampleTbEntryForm formData) {
    ...
    for (String testId : formData.getNewSelectedTests()) {
      ...
      analysis.setSampleTypeName(
          typeOfSampleService.get(formData.getTbSpecimenNature()).getDescription());
  ...
  private String persistSampleOrganizationData(SampleTbEntryForm formData) {
    ...
    if (ObjectUtils.isNotEmpty(formData.getSampleId())) {
      sampleOrganization =
          sampleOrganizationService.getDataBySample(sampleService.get(formData.getSampleId()));
      if (ObjectUtils.isNotEmpty(sampleOrganization)) {
        ...
        sampleOrganization.setSysUserId(formData.getSysUserId());
        sampleOrganization.setOrganization(
            organizationService.get(formData.getReferringSiteCode()));
