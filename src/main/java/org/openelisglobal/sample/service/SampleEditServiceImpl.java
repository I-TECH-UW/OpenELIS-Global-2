...
public class SampleEditServiceImpl implements SampleEditService {
  ...
  static {
    ...
    CANCELED_SAMPLE_STATUS_ID =
        SpringContext.getBean(IStatusService.class).getStatusID(SampleStatus.Canceled);
  }
  ...
  @Autowired TypeOfSampleTestService typeOfSampleTestService;
  ...
  public void editSample(
    ...
    List<SampleEditItem> existingTests =
        form.getExistingTests() != null ? form.getExistingTests() : new ArrayList<>();
    ...
    List<SampleItem> cancelSampleItemList =
        createCancelSampleList(existingTests, cancelAnalysisList, sysUserId);
    List<Analysis> addAnalysisList =
        createAddAanlysisList(
            form.getPossibleTests() != null ? form.getPossibleTests() : new ArrayList<>(),
            sysUserId);
    ...
    boolean useReceiveDateForCollectionDate =
    ...
    SampleAddService sampleAddService =
        new SampleAddService(
            form.getSampleXML(), sysUserId, updatedSample, collectionDateFromRecieveDate);
    ...
    SampleOrderService.SampleOrderPersistenceArtifacts orderArtifacts =
        sampleOrderService.getPersistenceArtifacts(updatedSample, sysUserId);
    ...
    for (SampleTestCollection sampleTestCollection : addedSamples) {
      ...
      if (sampleTestCollection.sampleNature != null) {
        ...
        sampleTestCollection.sampleNature.setSampleId(
            sampleTestCollection.item.getSample().getId());
    ...
    if (orderArtifacts.getProviderOrganization() != null) {
      ...
      if (link) {
        organizationService.linkOrganizationAndType(
            orderArtifacts.getProviderOrganization(), RequesterService.REFERRAL_ORG_TYPE_ID);
      }
    }
    ...
    if (orderArtifacts.getProviderDepartmentOrganization() != null) {
      ...
      if (link) {
        organizationService.linkOrganizationAndType(
            orderArtifacts.getProviderDepartmentOrganization(), orgTypeId);
      }
    }
    ...
    if (orderArtifacts.getSampleOrganizationDepartRequester() != null) {
      if (orderArtifacts.getProviderDepartmentOrganization() != null) {
        orderArtifacts
            .getSampleOrganizationDepartRequester()
  ...
  private void addExternalResultsToDeleteList(
      Analysis analysis,
      Patient patient,
      Sample updatedSample,
  ...
  private List<SampleItem> createSampleItemUpdateList(
      List<SampleEditItem> existingTests, String sysUserId) {
      ...
      if (editItem.isSampleItemChanged()) {
        ...
        if (sampleItem != null) {
          ...
          if (GenericValidator.isBlankOrNull(collectionTime)) {
            ...
            collectionTime +=
                " "
                    + (GenericValidator.isBlankOrNull(editItem.getCollectionTime())
                        ? "00:00"
  ...
  private Analysis populateAnalysis(
      ...
      Test test,
      String userSelectedTestSection,
      SampleAddService sampleAddService) {
    java.sql.Date collectionDateTime =
        DateUtil.convertStringDateTimeToSqlDate(sampleTestCollection.collectionDate);
    ...
    analysis.setStartedDate(
        collectionDateTime == null ? DateUtil.getNowAsSqlDate() : collectionDateTime);
  ...
  private List<SampleTestCollection> createAddSampleList(
      SampleEditForm form, SampleAddService sampleAddService) {
    ...
    if (!GenericValidator.isBlankOrNull(maxAccessionNumber)) {
      sampleAddService.setInitialSampleItemOrderValue(
          Integer.parseInt(maxAccessionNumber.split("-")[1]));
    }

    return sampleAddService.createSampleTestCollection();
  }

  private List<SampleItem> createCancelSampleList(
      List<SampleEditItem> list, List<Analysis> cancelAnalysisList, String sysUserId) {
  ...
  private Analysis newOrExistingCanceledAnalysis(SampleEditItem sampleEditItem) {
    List<Analysis> canceledAnalysis =
        analysisService.getAnalysesBySampleItemIdAndStatusId(
            sampleEditItem.getSampleItemId(), CANCELED_TEST_STATUS_ID);
  ...
  private boolean cancelAnalysisListContainsId(
      String analysisId, List<Analysis> cancelAnalysisList) {
