...
public class SampleServiceImpl extends AuditableBaseObjectServiceImpl<Sample, String>
  ...
  @Autowired protected SampleDAO sampleDAO;
  ...
  @Autowired private ReferenceTablesService referenceTableService;
  @Autowired private RequesterTypeService requesterTypeService;
  @Autowired private OrganizationService organizationService;
  @Autowired private TestService testService;
  @Autowired private SampleAdditionalFieldDAO sampleAdditionalFieldDAO;
  ...
  public Timestamp getOrderedDate(Sample sample) {
    ...
    ObservationHistory observation =
        SpringContext.getBean(ObservationHistoryService.class)
            .getObservationForSample(
                ObservationHistoryServiceImpl.ObservationType.REQUEST_DATE, sample.getId());
  ...
  public List<SampleQaEvent> getSampleQAEventList(Sample sample) {
    return sample == null
        ? new ArrayList<>()
        : sampleQaEventService.getSampleQaEventsBySample(sample);
  }
  ...
  public Person getPersonRequester(Sample sample) {
    ...
    List<SampleRequester> requesters =
        sampleRequesterService.getRequestersForSampleId(sample.getId());
  ...
  public List<Organization> getOrganizationRequesters(Sample sample) {
    ...
    for (SampleRequester requester : requesters) {
      if (ORGANIZATION_REQUESTER_TYPE_ID == requester.getRequesterTypeId()) {
        ...
        if (org != null
            && org.getOrganizationTypes().stream()
                .anyMatch(
                    e -> e.getId().equals(TableIdService.getInstance().REFERRING_ORG_TYPE_ID))) {
          orgs.add(org);
        }
      }
    }
    ...
    for (SampleRequester requester : requesters) {
      if (ORGANIZATION_REQUESTER_TYPE_ID == requester.getRequesterTypeId()) {
        Organization org =
            organizationService.getOrganizationById(String.valueOf(requester.getRequesterId()));
        if (org != null
            && org.getOrganizationTypes().stream().anyMatch(e -> e.getId().equals(orgTypeId))) {
          return requester;
        }
      }
    }
  ...
  @Transactional(readOnly = true)
  public Sample getPatientPreviousSampleForTestName(
      Sample sample, Patient patient, String testName) {
    ...
    List<Analysis> analysisList =
        analysisService.getAnalysesBySampleIdTestIdAndStatusId(sampIDList, testIDList, statusList);
  ...
  @Override
  @Transactional(readOnly = true)
  public List<Sample> getConfirmationSamplesReceivedInDateRange(
      Date receivedDateStart, Date receivedDateEnd) {
    return getBaseObjectDAO()
        .getConfirmationSamplesReceivedInDateRange(receivedDateStart, receivedDateEnd);
  }

  @Override
  @Transactional(readOnly = true)
  public List<Sample> getSamplesByProjectAndStatusIDAndAccessionRange(
    ...
    return getBaseObjectDAO()
        .getSamplesByProjectAndStatusIDAndAccessionRange(
            inclusiveProjectIdList, inclusiveStatusIdList, minAccession, maxAccession);
  }
  ...
  public List<Sample> getSamplesByProjectAndStatusIDAndAccessionRange(
    ...
    return getBaseObjectDAO()
        .getSamplesByProjectAndStatusIDAndAccessionRange(
            projectId, inclusiveStatusIdList, minAccession, maxAccession);
  }
  ...
  @Override
  @Transactional(readOnly = true)
  public List<Sample> getSamplesWithPendingQaEvents(
      Sample sample, boolean filterByCategory, String qaEventCategoryId, boolean filterByDomain) {
    return getBaseObjectDAO()
        .getSamplesWithPendingQaEvents(sample, filterByCategory, qaEventCategoryId, filterByDomain);
  }
  ...
  @Override
  @Transactional(readOnly = true)
  public List<Sample> getSamplesReceivedInDateRange(
      String receivedDateStart, String receivedDateEnd) {
  ...
  @Override
  public SampleAdditionalField getSampleAdditionalFieldForSample(
      String sampleId, AdditionalFieldName fieldName) {
    return sampleAdditionalFieldDAO
        .getFieldForSample(fieldName, sampleId)
        .orElse(new SampleAdditionalField());
  }

  @Override
  public SampleAdditionalField saveSampleAdditionalField(
      SampleAdditionalField sampleAdditionalField) {
    if (sampleAdditionalField.getLastupdated() == null) {
      return sampleAdditionalFieldDAO
          .get(sampleAdditionalFieldDAO.insert(sampleAdditionalField))
          .get();
    } else {
  ...
  @Override
  public List<Sample> getSamplesForSiteBetweenOrderDates(
  ...
  @Override
  public List<Sample> getStudySamplesForSiteBetweenOrderDates(
      String referringSiteId, LocalDate lowerDate, LocalDate upperDate) {
