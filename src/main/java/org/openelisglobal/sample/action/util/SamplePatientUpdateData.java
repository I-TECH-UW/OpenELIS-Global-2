...
public class SamplePatientUpdateData {
  ...
  public void createObservation(
      String observationData, String observationType, ObservationHistory.ValueType valueType) {
  ...
  public void validateSample(Errors errors) {
    ...
    IAccessionNumberValidator.ValidationResults result =
        AccessionNumberUtil.checkAccessionNumberValidity(accessionNumber, null, null, null);
  ...
  private SampleRequester initSampleRequesterDepartment(SampleOrderItem orderItem) {
    ...
    if (!GenericValidator.isBlankOrNull(orgId)) {
      requester =
          createSiteRequester(orgId, TableIdService.getInstance().ORGANIZATION_REQUESTER_TYPE_ID);
    }
  ...
  private SampleRequester initSampleRequester(SampleOrderItem orderItem) {
    ...
    if (!GenericValidator.isBlankOrNull(orgId)) {
      ...
      if (confirmNewRequesterName(orderItem.getNewRequesterName())) {
        ...
        requester =
            createSiteRequester("0", TableIdService.getInstance().ORGANIZATION_REQUESTER_TYPE_ID);

        setNewOrganization(new Organization());

        if (FormFields.getInstance().useField(Field.SampleEntryHealthFacilityAddress)) {
          addOrgAddressExtra(
              orderItem.getFacilityPhone(), "T", TableIdService.getInstance().ADDRESS_PHONE_ID);
          addOrgAddressExtra(
              orderItem.getFacilityFax(), "T", TableIdService.getInstance().ADDRESS_FAX_ID);
          addOrgAddressExtra(
        ...
        if (!GenericValidator.isBlankOrNull(orgId)) {
          requester =
              createSiteRequester(
                  orgId, TableIdService.getInstance().ORGANIZATION_REQUESTER_TYPE_ID);
        }
      }
    }
  ...
  public void initSampleData(
      String sampleXML, String receivedDate, boolean trackPayments, SampleOrderItem sampleOrder) {
    createPopulatedSample(receivedDate, sampleOrder);

    addObservations(sampleOrder, trackPayments);

    SampleAddService sampleAddService =
        new SampleAddService(sampleXML, currentUserId, getSample(), receivedDate);
  ...
  private void addObservations(SampleOrderItem sampleOrder, boolean trackPayments) {
    ...
    createObservation(
        ...
        observationHistoryService.getObservationTypeIdForType(ObservationType.REQUEST_DATE),
        ValueType.LITERAL);
    createObservation(
    ...
    if (ConfigurationProperties.getInstance()
        .isPropertyValueEqual(Property.USE_BILLING_REFERENCE_NUMBER, "true")) {
      createObservation(
    ...
    if (ConfigurationProperties.getInstance()
        .isPropertyValueEqual(Property.ORDER_PROGRAM, "true")) {
      createObservation(
    ...
    if (ConfigurationProperties.getInstance()
      ...
      if (!GenericValidator.isBlankOrNull(sampleOrder.getProgramId())) {
        createObservation(
            programService.get(sampleOrder.getProgramId()).getProgramName(),
            observationHistoryService.getObservationTypeIdForType(ObservationType.PROGRAM),
            ValueType.LITERAL);
      }
    }
