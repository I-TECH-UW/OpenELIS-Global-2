...
public class OrganizationDAOImpl extends BaseDAOImpl<Organization, String>
  ...
  public void getData(Organization organization) throws LIMSRuntimeException {
    try {
      Organization org =
          entityManager.unwrap(Session.class).get(Organization.class, organization.getId());
      if (org != null) {
  ...
  public List<Organization> getAllOrganizations() throws LIMSRuntimeException {
    ...
    try {
      ...
      Query<Organization> query =
          entityManager.unwrap(Session.class).createQuery(sql, Organization.class);
      list = query.list();
  ...
  public List<Organization> getPagesOfSearchedOrganizations(int startingRecNo, String searchString)
    ...
    try {
      ...
      if (wCdPosition == -1) // no wild card looking for exact match
      {
        ...
        sql =
            "from Organization o where trim(lower (o.organizationName)) = :param  order by"
                + " o.organizationName";
      } else {
        newSearchStr = searchString.replace(wildCard, "%").toLowerCase().trim();
        sql =
            "from Organization o where trim(lower (o.organizationName)) like :param  order by"
                + " o.organizationName";
      }
      Query<Organization> query =
          entityManager.unwrap(Session.class).createQuery(sql, Organization.class);
  ...
  public List<Organization> getOrganizations(String filter) throws LIMSRuntimeException {
    ...
    try {
      String sql =
          "from Organization o where upper(o.organizationName) like upper(:param) and"
  ...
  public Organization getActiveOrganizationByName(Organization organization, boolean ignoreCase)
    ...
    try {
      if (ignoreCase) {
        sql =
            "from Organization o where trim(lower(o.organizationName)) = :param and o.isActive='Y'";
      } else {
  ...
  @Override
  @Transactional(readOnly = true)
  public Organization getOrganizationByLocalAbbreviation(
      Organization organization, boolean ignoreCase) throws LIMSRuntimeException {
    String sql = null;
    try {
      if (ignoreCase) {
        sql =
            "from Organization o where trim(lower(o.organizationLocalAbbreviation)) = :param and"
                + " o.isActive='Y'";
      } else {
        sql =
            "from Organization o where o.organizationLocalAbbreviation = :param and o.isActive='Y'";
      }
      ...
      if (ignoreCase) {
        query.setParameter(
            "param", organization.getOrganizationLocalAbbreviation().trim().toLowerCase());
      } else {
      ...
      LogEvent.logError(e);
      throw new LIMSRuntimeException(
          "Error in Organization getOrganizationByLocalAbbreviation()", e);
    }
  }
  ...
  @Override
  public boolean duplicateOrganizationExists(Organization organization)
      throws LIMSRuntimeException {
    try {
      ...
      if (organization.getIsActive().equalsIgnoreCase(IActionConstants.YES)) {
        ...
        // duplicates
        String sql =
            "from Organization o where ((trim(lower(o.organizationName))) = :organizationName and"
        ...
        if (organization.getOrganization() != null
          ...
          query.setParameter(
              "parentOrgId", Integer.parseInt(organization.getOrganization().getId()));
        } else {
        ...
        LogEvent.logDebug(
            this.getClass().getSimpleName(), "duplicateOrganizationExists", "org id is " + orgId);
  ...
  public Integer getTotalSearchedOrganizationCount(String searchString)
    ...
    try {
      ...
      if (wCdPosition == -1) // no wild card looking for exact match
      {
        newSearchStr = searchString.toLowerCase().trim();
        sql =
            "select count (*) from Organization o where trim(lower (o.organizationName)) = :param ";
      } else {
        newSearchStr = searchString.replace(wildCard, "%").toLowerCase().trim();
        sql =
            "select count (*) from Organization o where trim(lower (o.organizationName)) like"
                + " :param ";
      }
  ...
  /**
   * @see
   *     org.openelisglobal.organization.dao.OrganizationDAO#getOrganizationsByOrgTypeName(java.lang.String,
   *     java.lang.String[])
   */
  @Override
  @Transactional(readOnly = true)
  public List<Organization> getOrganizationsByTypeName(
      String orderByProperty, String... typeNames) {
    try {
      String sql =
          "SELECT o FROM Organization o INNER JOIN o.organizationTypes ot WHERE ot.name IN (:names)"
              + " ";
      ...
      Query<Organization> query =
          session.createQuery(sql, Organization.class).setParameterList("names", typeNames);
  ...
  @Override
  @Transactional(readOnly = true)
  public List<Organization> getOrganizationsByTypeNameAndLeadingChars(
      String partialName, String typeName) {

    try {
      String sql =
          "SELECT o FROM Organization AS o INNER JOIN o.organizationTypes AS ot WHERE ot.name ="
