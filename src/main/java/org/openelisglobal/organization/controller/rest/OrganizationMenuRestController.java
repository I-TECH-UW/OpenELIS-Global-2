...
public class OrganizationMenuRestController extends BaseMenuController<Organization> {
    ...
    public ResponseEntity<byte[]> exportOrganizations(@RequestParam(name = "active", defaultValue = "true") String active)
        ...
        try {
            byte[] data = organizationExportService.exportFhirOrganizationsFromOrganizations(Boolean.valueOf(active)).getBytes();
    ...
    @Override
    protected List<Organization> createMenuList(AdminOptionMenuForm<Organization> form, HttpServletRequest request) throws LIMSRuntimeException {
