...
public class OrganizationMenuController extends BaseMenuController<Organization> {
  ...
  @Autowired private OrganizationExportService organizationExportService;
  ...
  @RequestMapping(
      value = {"/OrganizationMenu", "/SearchOrganizationMenu"},
      method = RequestMethod.GET)
  public ModelAndView showOrganizationMenu(
  ...
  public @ResponseBody byte[] exportOrganizations(
    ...
    return organizationExportService
        .exportFhirOrganizationsFromOrganizations(Boolean.valueOf(active))
        .getBytes();
  }

  @Override
  protected List<Organization> createMenuList(
      AdminOptionMenuForm<Organization> form, HttpServletRequest request) {
    ...
    if (YES.equals(request.getParameter("search"))) {
      organizations =
          organizationService.getPagesOfSearchedOrganizations(
    ...
    if (YES.equals(request.getParameter("search"))) {
      request.setAttribute(
          MENU_TOTAL_RECORDS,
          String.valueOf(
              organizationService.getTotalSearchedOrganizationCount(
  ...
  public ModelAndView showDeleteOrganization(
      HttpServletRequest request,
      @RequestParam(value = ID, required = false) @Pattern(regexp = "[a-zA-Z0-9 -]*") String id,
      @ModelAttribute("form") @Valid OrganizationMenuForm form,
