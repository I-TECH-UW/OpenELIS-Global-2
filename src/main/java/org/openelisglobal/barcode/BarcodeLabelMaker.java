...
public class BarcodeLabelMaker {
  ...
  public void generatePrePrintLabels(
      Integer numSetsOfLabels,
      Integer numOrderLabelsPerSet,
      Integer numSpecimenLabelsPerSet,
      String facilityName,
      List<Test> tests,
      String startingAt)
  ...
  private String genNextPrePrintedAccessionNumber(
      IAccessionNumberGenerator accessionValidator, String startingAt)
      throws LIMSInvalidConfigurationException {
  ...
  private Image scaleCentreTableAsImage(Label label, PdfWriter writer, PdfPTable table)
    ...
    labelAsImage.setAbsolutePosition(
        ((label.pdfWidth) - labelAsImage.getScaledWidth()) / 2,
  ...
  private PdfPCell create128Barcode(Label label, PdfWriter writer, int colspan)
    ...
    PdfPCell cell =
        new PdfPCell(barcode.createImageWithBarcode(writer.getDirectContent(), null, null), true);
  ...
  /**
   * Create code 128 bar code without bar code as text below bar code Recommended for large fonts
   *
   * @param label The label containing the bar code
   * @param writer For the stream
   * @param colspan The number of columns the bar code will span
