...
public abstract class PatientProgramReport extends Report implements IReportCreator {
  ...
  protected PathologySampleService pathologySerivice =
      SpringContext.getBean(PathologySampleService.class);
  ...
  protected void createReportParameters() {
    ...
    Optional<Image> labDirectorSignature =
        imageService.getImageBySiteInfoName("labDirectorSignature");
    reportParameters.put("useLabDirectorSignature", labDirectorSignature.isPresent());
    if (labDirectorSignature.isPresent()) {
      reportParameters.put(
          "labDirectorSignature", new ByteArrayInputStream(labDirectorSignature.get().getImage()));
    }
  ...
  private void findPatientInfo() {
    ...
    if (ADDRESS_DEPT_ID != null) {
      PersonAddress deptAddress =
          addressService.getByPersonIdAndPartId(
              patientService.getPerson(patient).getId(), ADDRESS_DEPT_ID);
    ...
    if (ADDRESS_COMMUNE_ID != null) {
      PersonAddress deptAddress =
          addressService.getByPersonIdAndPartId(
              patientService.getPerson(patient).getId(), ADDRESS_COMMUNE_ID);
  ...
  protected ProgramSampleReportData buildClinicalPatientData() {
    ...
    data.setSampleType(
        currentAnalysis != null
            ? analysisService.getTypeOfSample(currentAnalysis).getLocalizedName()
            : "");
    ...
    data.setStNumber(
        getLazyPatientIdentity(patient, STNumber, PatientServiceImpl.getPatientSTIdentity()));
    data.setSubjectNumber(
        getLazyPatientIdentity(
            patient, subjectNumber, PatientServiceImpl.getPatientSubjectIdentity()));
    data.setHealthRegion(
        getLazyPatientIdentity(
            patient, healthRegion, PatientServiceImpl.getPatientHealthRegionIdentity()));
    data.setHealthDistrict(
        getLazyPatientIdentity(
            patient, healthDistrict, PatientServiceImpl.getPatientHealthDistrictIdentity()));

    data.setLabOrderType(
        observationHistoryService.getValueForSample(
            ObservationType.PROGRAM, sampleService.getId(sample)));
    data.setTestName(testName);
    data.setPatientSiteNumber(
        observationHistoryService.getValueForSample(
            ObservationType.REFERRERS_PATIENT_ID, sampleService.getId(sample)));
    data.setBillingNumber(
        observationHistoryService.getValueForSample(
            ObservationType.BILLING_REFERENCE_NUMBER, sampleService.getId(sample)));
    data.setOrderDate(orderDateForDisplay);
    data.setSampleSortOrder(
        currentAnalysis != null ? currentAnalysis.getSampleItem().getSortOrder() : "");
    if (AccessionFormat.ALPHANUM
      ...
      data.setAccessionNumber(
          sampleService.getAccessionNumber(sample) + "-" + data.getSampleSortOrder());
    }
      ...
      data.setContactTracingIndexName(
          sampleService
              .getSampleAdditionalFieldForSample(
                  sampleService.getId(sample), AdditionalFieldName.CONTACT_TRACING_INDEX_NAME)
              .getFieldValue());
  ...
  protected void setCollectionTime(
      Set<SampleItem> sampleSet, ProgramSampleReportData data, boolean addAccessionNumber) {
