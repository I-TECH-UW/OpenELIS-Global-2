...
public abstract class PatientVLReport extends RetroCIPatientReport {
  ...
  protected void setPatientInfo(VLReportData data) {
    ...
    data.setVlSuckle(
        ohService.getMostRecentValueForPatient(ObservationType.VL_SUCKLE, reportPatient.getId()));
    data.setVlPregnancy(
        ohService.getMostRecentValueForPatient(
            ObservationType.VL_PREGNANCY, reportPatient.getId()));
    data.setvih(
        ohService.getMostRecentValueForPatient(ObservationType.HIV_STATUS, reportPatient.getId()));
    ...
    if (AccessionFormat.ALPHANUM
      ...
      data.setAccessionNumber(
          AlphanumAccessionValidator.convertAlphaNumLabNumForDisplay(
              reportSample.getAccessionNumber()));
    } else {
      data.setAccessionNumber(reportSample.getAccessionNumber());
    }
    data.setReceptiondate(
        DateUtil.convertTimestampToStringDateAndTime(reportSample.getReceivedTimestamp()));
