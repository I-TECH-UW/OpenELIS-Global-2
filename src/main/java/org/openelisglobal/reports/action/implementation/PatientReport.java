...
public abstract class PatientReport extends Report {
  ...
  protected ReferralReasonService referralReasonService =
      SpringContext.getBean(ReferralReasonService.class);
  protected ReferralService referralService = SpringContext.getBean(ReferralService.class);
  protected ReferralResultService referralResultService =
      SpringContext.getBean(ReferralResultService.class);
  protected ObservationHistoryService observationService =
  ...
  protected static final NoteType[] FILTER = {
    NoteType.EXTERNAL, NoteType.REJECTION_REASON, NoteType.NON_CONFORMITY
  };
  ...
  protected String convertToAlphaNumericDisplay(Sample currentSample) {
    ...
    if (AccessionFormat.ALPHANUM
      ...
      displayAccesionNumber =
          AlphanumAccessionValidator.convertAlphaNumLabNumForDisplay(
              sampleService.getAccessionNumber(currentSample).split("-")[0]);
    } else {
  ...
  private List<Sample> findReportSamplesForSite(
    ...
    if (DateType.ORDER_DATE.equals(dateType)) {
      sampleList =
          sampleService.getSamplesForSiteBetweenOrderDates(
      ...
      List<Analysis> analysises =
          analysisService.getAnalysisForSiteBetweenResultDates(
              sampleRequesterOrgId,
              DateUtil.convertStringDateToLocalDate(lowerDateRange),
              DateUtil.convertStringDateToLocalDate(upperDateRange));
      sampleList =
  ...
  private void findPatientInfo() {
    ...
    if (ADDRESS_DEPT_ID != null) {
      PersonAddress deptAddress =
          addressService.getByPersonIdAndPartId(
              patientService.getPerson(currentPatient).getId(), ADDRESS_DEPT_ID);
    ...
    if (ADDRESS_COMMUNE_ID != null) {
      PersonAddress deptAddress =
          addressService.getByPersonIdAndPartId(
              patientService.getPerson(currentPatient).getId(), ADDRESS_COMMUNE_ID);
  ...
  private void findContactInfo() {
    ...
    Organization referringOrg =
        sampleService.getOrganizationRequester(
            currentSample, TableIdService.getInstance().REFERRING_ORG_TYPE_ID);
    Organization referringDepartmentOrg =
        sampleService.getOrganizationRequester(
            currentSample, TableIdService.getInstance().REFERRING_ORG_DEPARTMENT_TYPE_ID);

    currentSiteInfo += referringOrg == null ? "" : referringOrg.getOrganizationName();
    currentSiteInfo +=
        "|" + (referringDepartmentOrg == null ? "" : referringDepartmentOrg.getOrganizationName());

    // Person person = sampleService.getPersonRequester(currentSample);
    Person person =
        (ObjectUtils.isNotEmpty(sampleHumanService.getProviderForSample(currentSample)))
  ...
  private boolean validateAccessionNumbers() {
    ...
    if (lowerNumber.length() != upperNumber.length()
        || AccessionNumberUtil.correctFormat(lowerNumber, false)
  ...
  protected void findPatientFromSample() {
    ...
    if (currentPatient == null
        || !patient.getId().equals(patientService.getPatientId(currentPatient))) {
      STNumber = null;
      patientDOB = null;
  ...
  protected void createReportParameters() {
    ...
    if (ConfigurationProperties.getInstance()
            .isCaseInsensitivePropertyValueEqual(Property.configurationName, "CI LNSP")
        || ConfigurationProperties.getInstance()
            .isCaseInsensitivePropertyValueEqual(Property.configurationName, "CI IPCI")
        || ConfigurationProperties.getInstance()
            .isCaseInsensitivePropertyValueEqual(Property.configurationName, "CI_REGIONAL")
        || ConfigurationProperties.getInstance()
            .isCaseInsensitivePropertyValueEqual(Property.configurationName, "RETROCI")
        || ConfigurationProperties.getInstance()
  ...
  protected void reportResultAndConclusion(ClinicalPatientData data) {
    ...
    if (SpringContext.getBean(IStatusService.class)
        .matches(analysisService.getStatusId(currentAnalysis), AnalysisStatus.Canceled)) {
       ...
       */
            ...
            .matches(analysisService.getStatusId(currentAnalysis), AnalysisStatus.Finalized)
        && !(SpringContext.getBean(IStatusService.class)
                .matches(
                    analysisService.getStatusId(currentAnalysis), AnalysisStatus.TechnicalRejected)
            && ConfigurationProperties.getInstance()
  ...
  private void setCorrectedStatus(Result result, ClinicalPatientData data) {
    if (currentAnalysis.isCorrectedSincePatientReport()
  ...
  private String getAugmentedResult(ClinicalPatientData data, Result result) {
    ...
    if (TestIdentityService.getInstance()
        .isTestNumericViralLoad(analysisService.getTest(currentAnalysis))) {
      try {
        resultValue +=
            " (" + formatTwoDecimals(Math.log10(Double.parseDouble(resultValue))) + ")log ";
  ...
  protected String getResultFlag(Result result, String imbed, ClinicalPatientData data) {
    ...
    try {
      ...
      } else if (TypeOfTestResultServiceImpl.ResultType.isDictionaryVariant(
  ...
  protected String getUnitOfMeasure(Test test) {
    return (test != null && test.getUnitOfMeasure() != null)
  ...
  private void setAppropriateResults(List<Result> resultList, ClinicalPatientData data) {
    ...
    if (!resultList.isEmpty()) {
      ...
      if (resultList.size() == 1) {
        ...
        if (TypeOfTestResultServiceImpl.ResultType.DICTIONARY.matches(
            resultResultService.getTestType(result))) {
          data.setAbnormalResult(resultResultService.isAbnormalDictionaryResult(result));
          List<Result> dictionaryResults = new ArrayList<>();
          Result quantification = null;
          for (Result sibResult : resultList) {
            if (TypeOfTestResultServiceImpl.ResultType.DICTIONARY.matches(
                sibResult.getResultType())) {
              dictionaryResults.add(sibResult);
            } else if (TypeOfTestResultServiceImpl.ResultType.ALPHA.matches(
                    sibResult.getResultType())
        ...
        } else if (TypeOfTestResultServiceImpl.ResultType.isMultiSelectVariant(
            resultResultService.getTestType(result))) {
          Dictionary dictionary = new Dictionary();
  ...
  protected void setCollectionTime(
      Set<SampleItem> sampleSet,
      List<ClinicalPatientData> currentSampleReportItems,
    ...
    for (SampleItem sampleItem : sampleSet) {
      ...
      if (sampleItem.getCollectionDate() == null) {
        ...
        buffer.append(" ");
        buffer.append(
            DateUtil.convertTimestampToStringDateAndConfiguredHourTime(
  ...
  /**
   * Pushes all of the information about a patient, analysis, result and the conclusion into a new
   * reporting object
   *
  ...
  protected ClinicalPatientData buildClinicalPatientData(boolean hasParent) {
    ...
    data.setStNumber(
        getLazyPatientIdentity(
            currentPatient, STNumber, PatientServiceImpl.getPatientSTIdentity()));
    data.setSubjectNumber(
        getLazyPatientIdentity(
            currentPatient, subjectNumber, PatientServiceImpl.getPatientSubjectIdentity()));
    data.setHealthRegion(
        getLazyPatientIdentity(
            currentPatient, healthRegion, PatientServiceImpl.getPatientHealthRegionIdentity()));
    data.setHealthDistrict(
        getLazyPatientIdentity(
            currentPatient, healthDistrict, PatientServiceImpl.getPatientHealthDistrictIdentity()));

    data.setLabOrderType(
        observationHistoryService.getValueForSample(
            ObservationType.PROGRAM, sampleService.getId(currentSample)));
    data.setTestName(testName);
    data.setPatientSiteNumber(
        observationHistoryService.getValueForSample(
            ObservationType.REFERRERS_PATIENT_ID, sampleService.getId(currentSample)));
    data.setBillingNumber(
        observationHistoryService.getValueForSample(
            ObservationType.BILLING_REFERENCE_NUMBER, sampleService.getId(currentSample)));
    ...
    if (AccessionFormat.ALPHANUM
      ...
      if (doAnalysis) {
        data.setSampleId(
            AlphanumAccessionValidator.convertAlphaNumLabNumForDisplay(
                    sampleService.getAccessionNumber(currentSample))
                + "-"
    ...
    if (Boolean.valueOf(
      ...
      data.setContactTracingIndexName(
          sampleService
              .getSampleAdditionalFieldForSample(
    ...
    if (testSection.equals("Tuberculose")) {
      data.setTbOrderReason(
          observationHistoryService.getValueForSample(
              ObservationType.TB_ORDER_REASON, sampleService.getId(currentSample)));
      data.setTbDiagnosticReason(
          observationHistoryService.getValueForSample(
              ObservationType.TB_DIAGNOSTIC_REASON, sampleService.getId(currentSample)));
      data.setTbFollowupReason(
          observationHistoryService.getValueForSample(
              ObservationType.TB_FOLLOWUP_REASON, sampleService.getId(currentSample)));
      data.setTbAnalysisMethod(
          observationHistoryService.getValueForSample(
              ObservationType.TB_ANALYSIS_METHOD, sampleService.getId(currentSample)));
      data.setTbSampleAspect(
          observationHistoryService.getValueForSample(
              ObservationType.TB_SAMPLE_ASPECT, sampleService.getId(currentSample)));
      data.setTbFollowupPeriodLine1(
          observationHistoryService.getValueForSample(
              ObservationType.TB_FOLLOWUP_PERIOD_LINE1, sampleService.getId(currentSample)));
      data.setTbFollowupPeriodLine2(
          observationHistoryService.getValueForSample(
              ObservationType.TB_FOLLOWUP_PERIOD_LINE2, sampleService.getId(currentSample)));
    }
  ...
  private String getTestName(boolean indent) {
    ...
    if (useReportingDescription()) {
      testName =
          TestServiceImpl.getUserLocalizedReportingTestName(
              analysisService.getTest(currentAnalysis));
    } else {
  ...
  /**
   * Given a list of referralResults for a particular analysis, generated the next displayable value
   * made of from one or more of the values from the list starting at the given index. It uses
   * multiresult form the list when the results are for the same test.
   *
   * @param referralResultsForReferral The referral
   * @param i starting index.
   * @return last index actually used. If you start with 2 and this routine uses just item #2, then
   *     return result is 2, but if there are two results for the same test (e.g. a multi-select
   *     result) and those are in item item 2 and item 3 this routine returns #3.
   */
  protected int lastUsedReportReferralResultValue(
      List<ReferralResult> referralResultsForReferral, int i) {
    ...
    reportReferralResultValue =
        reportReferralResultValue.substring(0, reportReferralResultValue.length() - 2);
    i--;
    return i;
  }
