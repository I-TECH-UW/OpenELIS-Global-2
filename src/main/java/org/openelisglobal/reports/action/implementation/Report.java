...
public abstract class Report implements IReportCreator {
  ...
  /**
   * Make sure we have a reportParameters map and make sure there is lab director in that map (for
   * any possible error report). All reports need a director name either in their header including
   * or on their error report page."
   */
  protected void createReportParameters() {
    ...
    reportParameters.put(
        "siteName", ConfigurationProperties.getInstance().getPropertyValue(Property.SiteName));
    reportParameters.put(
        "additionalSiteInfo",
    ...
    reportParameters.put(
        ...
        ConfigurationProperties.getInstance()
            .getPropertyValue(Property.USE_PAGE_NUMBERS_ON_REPORTS));
    ...
    if (rightLogo.isPresent()) {
      reportParameters.put(
          "rightHeaderImage", new ByteArrayInputStream(rightLogo.get().getImage()));
    }
  ...
  public byte[] runReport()
    ...
    return JasperRunManager.runReportToPdf(
        fullReportFilename, getReportParameters(), getReportDataSource());
  }
  ...
  /**
   * Utility routine for a sequence done in many places. Adds a message to the errorMsgs
   *
  ...
  /**
   * Checks a given date to make sure it is ok, filling in with a default if not found, logging a
  ...
  public class DateRange {
    ...
    /**
     * If you need to compare a Date which started as a date string to a bunch of timestamps, you
     * should move it from 00:00 at the beginning of the day to the end of the day at 23:59:59.999.
     *
    ...
    public String toString() {
      ...
      try {
        ...
        LogEvent.logInfo(
            this.getClass().getSimpleName(), "persistPatientType", "ignoring exception");
      }
      return range;
    }
