...
public class CSVStatisticsReportColumnBuilder extends CSVColumnBuilder {
    ...
    public void makeSQL() {
        ...
        query.append("SELECT t.test_name AS test_name, ts.description AS sample_type, p.priority AS test_priority,"
                + " o.name AS lab_unit, s.received_date AS reception_date, s.entered_date AS test_date"
                + " FROM test t"
                + " INNER JOIN sample s ON t.sample_id = s.id"
                + " INNER JOIN priority p ON t.priority_id = p.id"
                + " INNER JOIN lab_unit o ON s.lab_unit_id = o.id"
                + " INNER JOIN type_of_sample ts ON s.sample_type_id = ts.id"
                + " WHERE s.entered_date BETWEEN '" + lowDate + "' AND '" + highDate + "'");
        
        query.append(" AND p.priority IN ('Routine', 'ASAP', 'STAT', 'Timed', 'Future STAT')");
        
        
        query.append(" AND ("
                + "(s.reception_time BETWEEN '09:00' AND '15:30' AND p.priority IN ('Routine', 'ASAP'))"
