...
public abstract class CIColumnBuilder extends CSVColumnBuilder {
  ...
  protected static final String FROM_SAMPLE_PATIENT_ORGANIZATION =
      " FROM sample as s, patient as pat, person as per, sample_human as sh, sample_projects AS sp,"
  ...
  protected void appendRepeatingObservation(
      SQLConstant aOhTypeName, int maxCols, Date lowDate, Date highDate, Date byDate) {
    appendCrosstabPreamble(aOhTypeName);
    ...
    query.append(
        " crosstab( ' SELECT s.id as s_id, type, value FROM Sample AS s  LEFT JOIN ( SELECT"
            ...
            + formatDateForDatabaseSql(byDate)
            + " >= date(''"
            + formatDateForDatabaseSql(lowDate)
            + "'') "
            + " AND "
