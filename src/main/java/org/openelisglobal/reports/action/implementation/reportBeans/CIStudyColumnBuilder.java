...
public abstract class CIStudyColumnBuilder extends CSVColumnBuilder {
  ...
  protected static final String FROM_SAMPLE_PATIENT_ORGANIZATION =
      " FROM patient as pat, person as per, sample_human as sh, sample_projects AS sp,"
   ...
   */
  protected String buildWhereSamplePatienOrgSQL(
      String lowDatePostgres, String highDatePostgres, String byDate) {
    String WHERE_SAMPLE_PATIENT_ORG =
        " WHERE "
            ...
            + "\n AND so.samp_id = s.id "
            + ((GenericValidator.isBlankOrNull(projectStr))
                ? ""
                : " AND sp.proj_id = " + projectStr)
            + "\n AND o.id = so.org_id ";
    return WHERE_SAMPLE_PATIENT_ORG;
  }

  protected void appendRepeatingObservation(
      SQLConstant aOhTypeName, int maxCols, Date lowDatePostgres, Date highDatePostgres) {
  ...
  protected void appendRepeatingObservation(
      SQLConstant aOhTypeName,
      int maxCols,
      Date lowDatePostgres,
    ...
    query.append(
        " crosstab( ' SELECT s.id as s_id, type, value FROM Sample AS s  LEFT JOIN ( SELECT"
            ...
            + " oh.sample_id = s.id AND dt.row_id = s.id AND "
            + byDate
            + " >= date(''"
            + lowDatePostgres
            + "'') "
