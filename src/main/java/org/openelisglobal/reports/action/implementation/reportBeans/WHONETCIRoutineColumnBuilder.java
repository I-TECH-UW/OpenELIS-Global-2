...
public abstract class WHONETCIRoutineColumnBuilder extends WHONETCSVRoutineColumnBuilder {
    ...
    protected static final String WHONET_SELECT =
            // "SELECT
            // t.id,t.method_id,t.uom_id,t.description,t.loinc,t.reporting_description,"
            // + "t.sticker_req_flag , t.is_active , t.active_begin , t.active_end ,
            // t.is_reportable,"
            // +
            ...
            "select s.accession_number,pe.last_name, pe.first_name,t.id,t.description,t.loinc,t.guid"
                    + " from analysis a" + " join sample_item si on a.sampitem_id = si.id"
                    + " join sample s on si.samp_id = s.id" + " join sample_human sh on sh.samp_id = s.id"
                    + " join patient pa on pa.id = sh.patient_id" + " join person pe on pe.id = pa.person_id"
                    + " join test t on t.id = a.test_id" + " left join result r on r.analysis_id = a.id"
