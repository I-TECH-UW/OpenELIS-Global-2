...
public class RTRIColumnBuilder extends CIStudyColumnBuilder {
  ...
  public void makeSQL() {
    ...
    List<Test> tests =
        SpringContext.getBean(TestService.class).getActiveTestByName("Asante HIV-1 Rapid Recency");
    String ids =
        tests.stream()
            .map(
                e -> {
    ...
    query.append(
        "\n"
            + ", a.started_date,a.completed_date,a.released_date, a.status_id as"
            + " analysis_status_id, r.value as \"Asante HIV-1 Rapid"
            + " Recency\",a.type_of_sample_name, demo.* ");
