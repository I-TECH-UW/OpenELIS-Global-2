...
import static org.openelisglobal.reports.action.implementation.reportBeans.WHONETCSVRoutineColumnBuilder.Strategy.DICT;
import static org.openelisglobal.reports.action.implementation.reportBeans.WHONETCSVRoutineColumnBuilder.Strategy.TEST_RESULT;
...
public abstract class WHONETCSVRoutineColumnBuilder {
  ...
  /** */
  public WHONETCSVRoutineColumnBuilder(StatusService.AnalysisStatus validStatusFilter) {
  ...
  /** The list of all columns that can be exported for lookup when report is generated */
  private final Map<String, CSVRoutineColumn> columnsByDbName = new HashMap<>();

  /**
   * The list of columns in order of definition, so that we can generate JasperReports XML in order
   * for display
   */
  private final List<CSVRoutineColumn> columnsInOrder = new ArrayList<>();

  /** In order as postgres would order varchar columns (alphabetical ignoring case?). */
  ...
  /**
   * This table provide a mapping from test name (CSV column heading also) to the TestResult record
   * so we can look at the result type (Dictionary vs. constant etc.)
   */
  ...
  private static final SimpleDateFormat postgresDateTime =
  ...
  /**
   * the test have to be sorted by the name, because they have to match the pivot table order in the
   * results
   */
  ...
  public enum Strategy {
    ...
    SAMPLE_STATUS,
    PROJECT,
    PROGRAM, // program defined in routine
    // order.
    ...
    AGE_WEEKS,
    DEBUG,
    CUSTOM, // special handling which is encapsulated in an instance of
  ...
  protected void buildResultSet() throws SQLException {
    ...
    // LogEvent.logWarn(this.getClass().getSimpleName(), "buildResultSet", sql);
  ...
  public String getValue(CSVRoutineColumn column, String accessionNumber)
    ...
    if (result == null) {
      LogEvent.logInfo(
          this.getClass().getSimpleName(), "getValue", "A null found " + column.dbName);
    }
    return result;
  }
  ...
  /**
   * A utility routine for finding the project short tag (used in exporting etc.) from a projectId.
   */
  /*
  ...
  public class CSVRoutineColumn {
    ...
    public String translate(String value, String accessionNumber)
        throws SQLException, ParseException {
      switch (strategy) {
        case CUSTOM:
        ...
        case SAMPLE_STATUS:
          OrderStatus orderStatus =
              SpringContext.getBean(IStatusService.class).getOrderStatusForID(value);
          if (orderStatus == null) {
            return "?";
          }
          switch (orderStatus) {
    ...
    private String translateGendResult(String gendResultAnalyteId, String sampleItemId) {
      // 'generated CD4 Count'
      Result gendCD4Result =
          resultService.getResultForAnalyteAndSampleItem(gendResultAnalyteId, sampleItemId);
    ...
    public String translateTestResult(String testName, String value) throws SQLException {
      ...
      } else if (TypeOfTestResultServiceImpl.ResultType.MULTISELECT
          .getCharacterValue()
          .equals(type)) {
  ...
  /**
   * For every sample, one row per sample item, one column per test result for that sample item.
   *
   * @param lowDate
   * @param highDate
   */
  protected void appendResultCrosstab(java.sql.Date lowDate, java.sql.Date highDate) {
    ...
    query.append(
        "\n ON si.id = "
            + listName
            + ".si_id " // the inner use a few lines above
  ...
  protected void appendCrosstabPostfix(
      java.sql.Date lowDate, java.sql.Date highDate, SQLConstant listName) {
    query.append(
        "\n ON s.id = "
            + listName
            + ".s_id "
  ...
  protected void addAllResultsColumns() {
    /*
     * for (Test test : allTests) { String testTag =
     * TestServiceImpl.getLocalizedTestNameWithType(test); add(testTag,
     * TestServiceImpl.getLocalizedTestNameWithType(test), TEST_RESULT); }
     */
  }

  /**
   * Useful for the 1st line of a CSV files. This produces a completely escaped for MSExcel comma
