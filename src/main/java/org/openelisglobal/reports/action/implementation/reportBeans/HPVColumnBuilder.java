...
public class HPVColumnBuilder extends CIStudyColumnBuilder {
  ...
  protected void appendResultCrosstab(Date lowDate, Date highDate) {
    ...
    query.append(
        " crosstab( \n"
            ...
            + " \n"
            + " ORDER BY 1, 2 \n"
            + " ', 'SELECT t.description FROM test t JOIN panel_item pi ON t.id = pi.test_id  JOIN"
            + " panel p ON p.id =pi.panel_id AND p.name = ''HPV HR'' where t.is_active = ''Y''"
            + " ORDER BY 1' ) ");
    query.append(
        "\n as "
            + listName
  ...
  protected static final String SELECT_SAMPLE_PATIENT_ORGANIZATION =
      "SELECT DISTINCT s.id as sample_id, s.accession_number, s.entered_date, s.received_date,"
          ...
          + " pat.gender \n"
          + ", o.short_name as organization_code, o.name AS organization_name, sp.proj_id as"
          + " project_id \n"
          + ", o.datim_org_code, o.datim_org_name,a.type_of_sample_name, a.status_id as"
          + " analysis_status_id, a.started_date,a.completed_date,a.released_date \n"
          + " ";
  ...
  public void makeSQL() {
    ...
    query.append(
        buildWhereSamplePatienOrgSQL(lowDatePostgres, highDatePostgres, dateColumn)
