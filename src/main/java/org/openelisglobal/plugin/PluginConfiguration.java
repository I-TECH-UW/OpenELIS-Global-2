...
public class PluginConfiguration implements BeanFactoryAware {
  ...
  private boolean loadFromXML(JarFile jar, JarEntry entry) {
    ...
    try {
      ...
      if (versionElement == null) {
        LogEvent.logError(
            this.getClass().getSimpleName(), "loadFromXml", "Missing version number in plugin");
        LogEvent.logInfo(
            this.getClass().getSimpleName(), "loadFromXml", "Missing version number in plugin");
        return false;
      }
      if (!PluginLoader.SUPPORTED_VERSION.equals(versionElement.getData())) {
        LogEvent.logError(
            ...
            "Unsupported version number.  Expected "
                + PluginLoader.SUPPORTED_VERSION
                + " got "
      ...
      if (servlet != null) {
        description =
            servlet
                .element(PluginLoader.EXTENSION_POINT)
                .element(PluginLoader.DESCRIPTION)
                .attribute(PluginLoader.VALUE);
        Attribute servletPath =
            servlet
                .element(PluginLoader.EXTENSION_POINT)
                .element(PluginLoader.SERVLET_PATH)
                .attribute(PluginLoader.VALUE);
        Attribute servletName =
            servlet
                .element(PluginLoader.EXTENSION_POINT)
                .element(PluginLoader.SERVLET_NAME)
                .attribute(PluginLoader.VALUE);
        Attribute classPath =
            servlet
                .element(PluginLoader.EXTENSION_POINT)
                .element(PluginLoader.EXTENSION)
                .attribute(PluginLoader.PATH);
        loadActualPlugin(url, servletName.getValue(), servletPath.getValue(), classPath.getValue());
        LogEvent.logInfo(
            this.getClass().getSimpleName(), "loadFromXML", "Loaded: " + description.getValue());
      }
  ...
  private void loadActualPlugin(
      URL url, String servletName, String servletPath, String servletClassPath) throws IOException {
