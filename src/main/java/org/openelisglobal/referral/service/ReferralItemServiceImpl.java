...
public class ReferralItemServiceImpl implements ReferralItemService {
  ...
  @Autowired ReferralResultService referralResultService;
  ...
  private List<ReferredTest> getAdditionalReferralTests(List<ReferralResult> referralResults) {
    ...
    while (!referralResults.isEmpty()) {
      ReferralResult referralResult =
          referralResults.get(0); // use the top one to load various bits of
      // information.
      ReferredTest referralTest = new ReferredTest();
      referralTest.setReferralId(referralResult.getReferralId());
      referralResults =
          setReferralItemForNextTest(referralTest, referralResults); // remove one or more
  ...
  private List<ReferralResult> setReferralItemForNextTest(
    ...
    if (!TypeOfTestResultServiceImpl.ResultType.isMultiSelectVariant(resultType)) {
      ...
      referralItem.setMultiSelectResultValues(
          ResultServiceImpl.getJSONStringForMultiSelect(resultList));
    }
