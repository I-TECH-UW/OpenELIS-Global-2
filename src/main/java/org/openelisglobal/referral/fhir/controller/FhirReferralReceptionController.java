...
public class FhirReferralReceptionController extends BaseController {
  ...
  public ModelAndView showFhirReferralReceptionPage(
      @ModelAttribute @Valid FhirReferralSearchForm searchForm, BindingResult result) {
    if (result.hasErrors()) {
      saveErrors(result);
    ...
    if (!GenericValidator.isBlankOrNull(searchForm.getExternalAccessionNumber())
      ...
      List<ElectronicOrder> electronicOrders =
          electronicOrderService.getAllElectronicOrdersContainingValuesOrderedBy(
              searchForm.getExternalAccessionNumber(),
              searchForm.getPatientLastName(),
              searchForm.getPatientFirstName(),
              searchForm.getGender(),
              SortOrder.LAST_UPDATED_ASC);
      // TODO add to sql query instead of filtering here
      electronicOrders =
          electronicOrders.stream()
      ...
      int endIndex =
          startIndex + 50 > electronicOrders.size() ? electronicOrders.size() : startIndex + 50;
