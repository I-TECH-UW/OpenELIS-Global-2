...
public class FhirReferralServiceImpl implements FhirReferralService {
  ...
  @Autowired private ReferralResultService referralResultService;
  ...
  @Autowired private ResultValidationService resultValidationService;
  ...
  @Override
  @Transactional
  public Bundle referAnalysisesToOrganization(Referral referral)
    ...
    org.openelisglobal.organization.valueholder.Organization referralOrganization =
        organizationService.get(referralOrganizationId);
    Organization fhirOrg = getFhirOrganization(referralOrganization);
    if (fhirOrg == null) {
      LogEvent.logError(
          this.getClass().getSimpleName(),
          "referAnalysisesToOrganization",
    ...
    ServiceRequest serviceRequest =
        fhirPersistanceService
            .getServiceRequestByAnalysisUuid(analysis.getFhirUuidAsString())
            .orElseGet(
                () -> {
    ...
    Task task =
        createReferralTask(
            ...
            fhirPersistanceService
                .getPatientByUuid(
                    sampleHumanService.getPatientForSample(sample).getFhirUuidAsString())
                .orElseThrow(),
            serviceRequest,
            requester,
            sample);
  ...
  public Task createReferralTask(
  ...
  public void setReferralResult(ReferralResultsImportObjects resultsImport) {
            ...
            .orElseThrow(
                () -> {
                  return new RuntimeException(
                      "no matching analysis with FhirUUID: "
                          + resultsImport
                              .originalReferralObjects
                              .serviceRequests
                              .get(0)
    ...
    try {
      ...
      referralSetService.updateReferralSets(
          referralSets, new ArrayList<>(), new HashSet<>(), new ArrayList<>(), "1");
      LogEvent.logDebug(
          this.getClass().getSimpleName(),
          "setReferralResult",
      ...
      fhirPersistanceService.updateFhirResourceInFhirStore(
          resultsImport.originalReferralObjects.task);
    } catch (FhirPersistanceException e) {
      LogEvent.logError(e);
    }
  }

  private Result getResultFromObservation(
      Observation observation, List<Result> currentResults, Analysis analysis) {
    Result result;
    LogEvent.logDebug(
        this.getClass().getSimpleName(),
        "getResultFromObservation",
    ...
    if (TypeOfTestResultServiceImpl.ResultType.isMultiSelectVariant(result.getResultType())
      ...
      String inferredValue =
          ((CodeableConcept) observation.getValue()).getCodingFirstRep().getCode();
      List<TestResult> testResults =
          testResultService.getAllActiveTestResultsPerTest(analysisService.getTest(analysis));
      ...
      for (TestResult testResult : testResults) {
        if (StringUtils.equals(
            inferredValue, dictionaryService.get(testResult.getValue()).getDictEntry())) {
          LogEvent.logDebug(
              this.getClass().getSimpleName(),
              "setReferralResult",
      ...
      LogEvent.logDebug(
          this.getClass().getSimpleName(), "getResultFromObservation", "numeric result type");
      ...
      LogEvent.logDebug(
          this.getClass().getSimpleName(), "getResultFromObservation", "text result type");
  ...
  private void addResultSets(
    ...
    LogEvent.logDebug(
        this.getClass().getSimpleName(), "addResultSets", "creating resultSet for referral");
    ...
    if (finalResultAlreadySent(result)) {
      LogEvent.logDebug(
          this.getClass().getSimpleName(), "addResultSets", "final results already sent");
      ...
      LogEvent.logDebug(
          this.getClass().getSimpleName(), "addResultSets", "final results not already sent");
    ...
    LogEvent.logDebug(
        this.getClass().getSimpleName(), "addResultSets", "referral result added to set");
  }
  ...
  private void recordResultForReferral(
      ReferralResultsImportObjects resultsImport,
      Analysis analysis,
      Result result,
      List<ReferralSet> referralSets) {
    LogEvent.logDebug(
        this.getClass().getSimpleName(),
        "recordResultForReferral",
        "recording result for referral");

    ReferralSet referralSet = new ReferralSet();

    Referral referral = referralService.getReferralByAnalysisId(analysis.getId());
    LogEvent.logDebug(
        this.getClass().getSimpleName(), "recordResultForReferral", "got referral for analysis");
    ...
    referralSet.setExistingReferralResults(
        referralResults == null ? new ArrayList<>() : referralResults);
    ...
    referralSet.setNote(
        noteService.createSavableNote(
            referral.getAnalysis(),
            NoteServiceImpl.NoteType.INTERNAL,
            "referral result imported automatically",
            RESULT_SUBJECT,
            "1"));
    LogEvent.logDebug(
        this.getClass().getSimpleName(),
