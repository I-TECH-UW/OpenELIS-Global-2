...
public class CobasTaqmanDBSReader extends AnalyzerLineInserter {
  ...
  public boolean insert(List<String> lines, String currentUserId) {
    ...
    MappedTestName mappedName =
        AnalyzerTestNameCache.getInstance()
            .getMappedTest(AnalyzerTestNameCache.COBAS_DBS, TEST_NAME);

    if (mappedName == null) {
      mappedName =
          AnalyzerTestNameCache.getInstance()
              .getEmptyMappedTestName(AnalyzerTestNameCache.COBAS_DBS, TEST_NAME);
    }
  ...
  private void createAnalyzerResultFromLine(
      String line, List<AnalyzerResults> resultList, MappedTestName mappedName) {
    String[] fields = line.split(DELIMITER);
  ...
  private String getAppropriateResults(String result) {
    ...
    if ("Target Not Detected".equals(result)) {
      ...
      //
      //			if (Double.parseDouble(splitResult[0]) * Math.pow(10, Double.parseDouble(splitResult[1]))
      // < THREASHOLD) {
