...
public class AnalyzerLineReader extends AnalyzerReader {
  ...
  private static final CharSequence EVOLIS_INTEGRAL_INDICATOR =
      "DBehring Enzygnost HIV integral II";
  private static final CharSequence EVOLIS_INTEGRAL_DBS_INDICATOR =
      "DBehring Enzygnost HIV integral IIDBS";
  ...
  private void setInserter() {
    ...
    if (lines.get(0).contains(COBAS_INDICATOR)) { // Cobas is found on the first line
      ...
      inserter = new EvolisReader();
        ...
        && lines.get(1).contains(SYSMEX_XT_INDICATOR)) { // Sysmex model found on data
      // line
      inserter = new SysmexReader();
    } else if (lines.get(1) != null
        && lines.get(1).contains(FACSCALIBUR_INDICATOR)) { // Fascalibur software found
      // on data line
      inserter = new FacscaliburReader();
    } else if (lines.get(1) != null
        && (lines.get(1).contains(COBAS_TAQMAN_INDICATOR)
      ...
      inserter = new FACSCantoReader();
        ...
        && lines.get(1).toUpperCase().contains(COBAS_TAQMAN_DBS_INDICATOR)) {
