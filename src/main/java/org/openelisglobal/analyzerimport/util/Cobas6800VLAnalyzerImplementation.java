...
public class Cobas6800VLAnalyzerImplementation extends AnalyzerLineInserter {
    ...
    private final String UNDER_THREASHOLD = "< LL";
    private final double THREASHOLD = 20.0;

    private final String ABOVE_SUPLOG = "> Log7";
    private final String SUPLOG = ">1.1E7";

    private String RESULT_FLAG = "OBX";
    private String VL_FLAG = "Viral Load";

    private final String DATE_PATTERN = "yyyy/MM/dd HH:mm:ss";
    String ANALYZER_ID;
    private final String projectCode = MessageUtil.getMessage("sample.entry.project.LART");

    HashMap<String, Test> testHeaderNameMap = new HashMap<>();
    HashMap<String, String> indexTestMap = new HashMap<>();
    HashMap<String, String> unitsIndexMap = new HashMap<>();
    ...
    private Cobas6800VLAnalyzerImplementation() {
    ...
    private String getAppropriateResults(String result) {
        ...
        if (result.contains(SUPLOG)) {
            ...
            try {
                Double resultAsDouble = Double.parseDouble(result);// Double.parseDouble(splitResult[0]) * Math.pow(10,
                                                                   // Double.parseDouble(splitResult[1]));

                if (resultAsDouble <= THREASHOLD) {
                    result = UNDER_THREASHOLD;
                } else {
                    result = String.valueOf((int) (Math.round(resultAsDouble)));// String.valueOf((int)(Math.round(resultAsDouble)))
                                                                                // +
    ...
    public void ordersExport(List<AnalyzerResults> results) {
        ...
        try {
            ...
            String sql = "SELECT s.accession_number, a.test_id,pat.national_id,pat.external_id,pat.gender,pat.birth_date "
                    ...
                    + "WHERE  a.status_id=13  AND a.test_id IN (" + test.getId() + ") AND " + "a.sampitem_id=si.id AND "
                    + "si.samp_id=s.id AND " + "sh.samp_id=s.id AND " + "sh.patient_id=pat.id " + "ORDER BY 1";
            ...
            // FileOutputStream("C:/Temp/RetroCIPRESC.AST"), StandardCharsets.UTF_8));
            // Writer writer = new BufferedWriter(new OutputStreamWriter(new
            // FileOutputStream("/media/windowsshare/ARVPRESC.AST"),
            // StandardCharsets.UTF_8));
            // Writer writer = new BufferedWriter(new OutputStreamWriter(new
            // FileOutputStream("/home/oeserver/Desktop/PRESCRIPTIONS.AST"),
            // StandardCharsets.UTF_8));
            ...
            while (rs.next()) {
                ...
                writer.write("P|" + inc + "|" + labno + "|||Patient-XXXXX||"
                        + birth_date.substring(0, 10).replace("-", "") + "|" + sexe + "||||||||||||||||||");
                writer.write("\r\n");
                writer.write("OBR|1|" + labno + "||Viral Load|R|||||||||||||" + patID + "||||||||||");
                writer.write("\r\n");
        ...
        System.out.println("Operation done successfully");// Statement
    ...
    public HashMap<Integer, Integer> getResultsLines(List<String> lines, String RESULT_FLAG, String TEST_FLAG) {
        HashMap<Integer, Integer> IdValuePair = new HashMap<>();
    ...
    public void createVLResultFromEntry(List<String> lines, Entry<Integer, Integer> entry,
        ...
        analyzerResults.setCompleteDate(DateUtil.convertStringDateToTimestampWithPattern(completedDate.substring(0, 4)
                + "/" + completedDate.substring(4, 6) + "/" + completedDate.substring(6, 8) + " 00:00:00",
                DATE_PATTERN));
        ...
        /*
         * if (analyzerResults.getIsControl()) { accessionNumber = accessionNumber + ":"
         * + fields[this.SAMPLE_TYPE].replace("\"", "").trim(); }
         */
