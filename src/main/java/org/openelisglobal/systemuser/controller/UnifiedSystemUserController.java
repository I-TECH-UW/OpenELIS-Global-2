...
public class UnifiedSystemUserController extends BaseController {
  ...
  public List<IdValuePair> getUsersWithRole(@PathVariable String roleName) {
    ...
    return users.stream()
        .filter(e -> userRoleService.userInRole(e.getId(), roleName))
  ...
  public ModelAndView showUnifiedSystemUser(
      HttpServletRequest request, @RequestParam(name = "ID", defaultValue = "") String id)
  ...
  private void setupRoles(
      UnifiedSystemUserForm form, HttpServletRequest request, boolean doFiltering) {
  ...
  private List<DisplayRole> sortAndGroupRoles(List<DisplayRole> roles) {
    ...
    for (DisplayRole role : roles) {
      if (!GenericValidator.isBlankOrNull(role.getParentRole())
          && !groupIds.contains(role.getParentRole())) {
  ...
  private void setPropertiesForExistingUser(
      UnifiedSystemUserForm form, String id, boolean doFiltering)
      throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
  ...
  public ModelAndView showUpdateUnifiedSystemUser(
      HttpServletRequest request,
      @ModelAttribute("form") @Valid UnifiedSystemUserForm form,
  ...
  private String validateAndUpdateSystemUser(
      HttpServletRequest request, UnifiedSystemUserForm form) {
    ...
    LoginUser loginUser =
        createLoginUser(form, loginUserId, loginUserNew, passwordUpdated, loggedOnUserId);
    ...
    try {
      if (form.getAllowCopyUserRoles().equals(NO)) {
        ...
        if (StringUtils.isNotBlank(form.getSystemUserIdToCopy().trim())) {
          String globalParentRoleId =
              roleService.getRoleByName(Constants.GLOBAL_ROLES_GROUP).getId();
          List<String> globaRolesIds =
              getAllRoles().stream()
                  .filter(role -> role.getGroupingParent() != null)
                  .filter(role -> role.getGroupingParent().equals(globalParentRoleId))
                  .map(role -> role.getId())
                  .collect(Collectors.toList());
          List<String> copiedRoleIds =
              userRoleService.getRoleIdsForUser(form.getSystemUserIdToCopy().trim());
          List<String> globalCopiedRoleIds =
              copiedRoleIds.stream()
                  .filter(role -> globaRolesIds.contains(role))
                  .collect(Collectors.toList());
          ...
          UserLabUnitRoles labRoles =
              userService.getUserLabUnitRoles(form.getSystemUserIdToCopy().trim());
          Map<String, Set<String>> copiedLabUnitRolesMap = new HashMap<>();
          labRoles
              .getLabUnitRoleMap()
              .forEach(
                  roleMap ->
                      copiedLabUnitRolesMap.put(
                          new String(roleMap.getLabUnit()), new HashSet<>(roleMap.getRoles())));
  ...
  private void validateUser(
      UnifiedSystemUserForm form,
      Errors errors,
      boolean loginUserIsNew,
      boolean passwordUpdated,
      String loginUserId) {
    boolean checkForDuplicateName =
        loginUserIsNew || userNameChanged(loginUserId, form.getUserLoginName());
    // check login name
  ...
  private SystemUser createSystemUser(
    ...
    String initial =
        systemUser.getFirstName().substring(0, 1) + systemUser.getLastName().substring(0, 1);
  ...
  private void saveUserLabUnitRoles(
      SystemUser user, UnifiedSystemUserForm form, String loggedOnUserId) {
  ...
  private void setLabunitRolesForExistingUser(UnifiedSystemUserForm form) {
    ...
    if (roles != null) {
      ...
      if (userLabUnits.contains(ALL_LAB_UNITS)) {
        roleMaps.stream()
            .filter(map -> map.getLabUnit().equals(ALL_LAB_UNITS))
            .forEach(
                map ->
                    userLabData.put(
                        map.getLabUnit(), map.getRoles().stream().collect(Collectors.toList())));
      } else {
  ...
  private void parseUserLabRolesData(
      UnifiedSystemUserForm form, Map<String, Set<String>> selectedLabUnitRolesMap) {
    /**
     * The ui-form can dynamically render more fields that are mapped to the same field (ie for
     * testSectionId ,selectedLabUnitRoles )of the form-backing object, so we append a common
     * pre-fix to the values in every New lab Unit Roles Set to identify the distinct values for
     * each set . This method parses the data from the form ,and builds a Map ie <String
     * testSectionId ,Set<LabUnitRoles>> for each Lab Unit Role Set based on the suffix appended ie
     * testSectionId = "1=2,2=5,2=6" ,selectedLabUnitRoles = [1=56, 2=36 ,3=44]
     */
    ...
    if (selectedLabUnitRolesMap.containsKey(ALL_LAB_UNITS)) {
      ...
      allTestSectionIds.forEach(
          testScetionId -> selectedLabUnitRolesMap.put(testScetionId, labRolesId));
    }
  }

  private JSONArray getDisplaySystemUsersJsonArray() {
    JSONArray displayUsers = new JSONArray();
    systemUserService.getAll().stream()
        .filter(user -> user.getIsActive().equals(YES))
        .map(
            user ->
                new JSONObject()
                    .put(
