...
public class UserServiceImpl implements UserService {
  ...
  @Override
  @Transactional
  public void saveUserLabUnitRoles(
      SystemUser systemUser,
      Map<String, Set<String>> selectedLabUnitRolesMap,
      String loggedOnUserId) {
    ...
    updateUserRoles(
        labUnitRoles.stream().collect(Collectors.toList()), systemUser, loggedOnUserId, true);
  }
  ...
  private void updateUserRoles(
      List<String> selectedRoles, SystemUser systemUser, String loggedOnUserId, Boolean isLabRole) {
  ...
  public List<IdValuePair> getUserTestSections(String systemUserId, String roleId) {
    ...
    List<IdValuePair> allTestSections =
    ...
    if (userLabUnits.contains(UnifiedSystemUserController.ALL_LAB_UNITS)) {
      ...
      List<IdValuePair> userTestSections =
          allTestSections.stream()
    ...
    if (allTests != null) {
      ...
      allTests.forEach(
          test -> {
            ...
            if (sampleTypes != null) {
              sampleIds.addAll(
                  sampleTypes.stream().map(e -> e.getId()).collect(Collectors.toList()));
            }
          });
    }
  ...
  @Override
  public List<IdValuePair> getUserSampleTypes(
      String systemUserId, String roleName, String testSectionName) {
    String resultsRoleId = roleService.getRoleByName(roleName).getId();
    List<IdValuePair> testSections = getUserTestSections(systemUserId, resultsRoleId);
    ...
    List<Test> allTests = testService.getTestsByTestSectionIds(testUnitIds);
    // List<String> allTBTestIds = typeOfSampleService.getAllActiveTestsByTestUnit(true,
    ...
    if (allTests != null) {
      ...
      allTests.forEach(
          test ->
              sampleIds.addAll(
                  typeOfSampleService.getTypeOfSampleForTest(test.getId()).stream()
                      .map(e -> e.getId())
                      .collect(Collectors.toList())));
    }
  ...
  @Override
  public List<TestResultItem> filterResultsByLabUnitRoles(
      String systemUserId, List<TestResultItem> results, String roleName) {
    String resultsRoleId = roleService.getRoleByName(roleName).getId();
    List<IdValuePair> testSections = getUserTestSections(systemUserId, resultsRoleId);
    List<Integer> testUnitIds = new ArrayList<>();
    if (testSections != null) {
    ...
    return results.stream()
        .filter(result -> allTestsIds.contains(result.getTestId()))
        .collect(Collectors.toList());
  }
  ...
  public List<IdValuePair> getAllDisplayUserTestsByLabUnit(String SystemUserId, String roleName) {
    ...
    List<IdValuePair> allDisplayUserTests =
        DisplayListService.getInstance()
            .getListWithLeadingBlank(DisplayListService.ListType.ALL_TESTS);
    return allDisplayUserTests.stream()
        .filter(test -> allTestsIds.contains(test.getId()))
        .collect(Collectors.toList());
  }

  @Override
  public List<AnalysisItem> filterAnalysisResultsByLabUnitRoles(
      String SystemUserId, List<AnalysisItem> results, String roleName) {
    String resultsRoleId = roleService.getRoleByName(roleName).getId();
    List<IdValuePair> testSections = getUserTestSections(SystemUserId, resultsRoleId);
    List<Integer> testUnitIds = new ArrayList<>();
    if (testSections != null) {
  ...
  @Override
  public List<Analysis> filterAnalysesByLabUnitRoles(
      String SystemUserId, List<Analysis> results, String roleName) {
    String resultsRoleId = roleService.getRoleByName(roleName).getId();
    List<IdValuePair> testSections = getUserTestSections(SystemUserId, resultsRoleId);
    List<Integer> testUnitIds = new ArrayList<>();
    if (testSections != null) {
    ...
    allTests.forEach(test -> allTestsIds.add(test.getId()));
    return results.stream()
        .filter(result -> allTestsIds.contains(result.getTest().getId()))
        .collect(Collectors.toList());
  }
  ...
  public List<IdValuePair> getUserPrograms(String systemUserId, String userRole) {
    ...
    return allPrograms.stream()
        .filter(
            p ->
                programService.get(p.getId()).getTestSection() == null
