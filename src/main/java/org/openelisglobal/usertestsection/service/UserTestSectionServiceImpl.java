...
public class UserTestSectionServiceImpl implements UserTestSectionService {
  ...
  public List<TestSection> getAllUserTestSectionsByName(
      HttpServletRequest request, String testSectionName) throws LIMSRuntimeException {
    List<TestSection> list = new ArrayList<>();

    try {
      if (SystemConfiguration.getInstance()
          .getEnableUserTestSection()
          .equals(IActionConstants.NO)) {
        list = testSectionService.getTestSections(testSectionName);
      } else {
        UserSessionData usd =
            (UserSessionData) request.getSession().getAttribute(IActionConstants.USER_SESSION_DATA);
        // bugzilla 2160

        if (!userModuleService.isUserAdmin(request)) {
          list =
              testSectionService.getTestSectionsBySysUserId(testSectionName, usd.getSystemUserId());
        } else {
          list = testSectionService.getTestSections(testSectionName);
        }
      }
      ...
      throw new LIMSRuntimeException(
          "Error in UserTestSectionServiceImpl getAllUserTestSectionsByName()", e);
    }
    return list;
  }
  ...
  //    @Override
  //    @Transactional(readOnly = true)
  //    public List getPageOfTestsBySysUserId(HttpServletRequest request, int startingRecNo, String
  // doingSearch,
  ...
  // doingSearch.equals(IActionConstants.YES)) {
  ...
  // doingSearch.equals(IActionConstants.YES)) {
  //
  //                        list = testService.getPageOfSearchedTestsBySysUserId(startingRecNo,
  // usd.getSystemUserId(),
  //                                searchStr);
  //                    } else {
  //                        list = testService.getPageOfTestsBySysUserId(startingRecNo,
  // usd.getSystemUserId());
  //                    }
  //                } else {
  //
  ...
  //                if (!userModuleService.isUserAdmin(request)) {
  //                    list =
  // testSectionService.getAllTestSectionsBySysUserId(usd.getSystemUserId());
  //                } else {
  //                    list = testSectionService.getAllTestSections();
  //                }
  //            }
  ...
  //    @Override
  //    @Transactional(readOnly = true)
  //    public List getSamplePdfList(HttpServletRequest request, Locale locale, String sampStatus,
  // String humanDomain)
  //            throws LIMSRuntimeException {
  //
  ...
  //    @Override
  //    @Transactional(readOnly = true)
  //    public List getAnalyses(HttpServletRequest request, List analyses, List testSections) throws
  ...
  // (org.openelisglobal.analysis.valueholder.Analysis) analyses
  //                                .get(i);
  //                        for (int j = 0; j < testSections.size(); j++) {
  //                            TestSection testSection = (TestSection) testSections.get(j);
  ...
  //            // bugzilla 2154
  //            LogEvent.logError(e);
  //            throw new LIMSRuntimeException("Error in UserTestSectionServiceImpl getAnalyses()",
  // e);
  //        }
  //        return newAnalyses;
  //
