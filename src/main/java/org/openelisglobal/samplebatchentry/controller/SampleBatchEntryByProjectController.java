...
public class SampleBatchEntryByProjectController extends BaseSampleEntryController {
  ...
  public ModelAndView showSampleBatchEntryByProject(
      HttpServletRequest request,
      @ModelAttribute("form") @Validated(SampleBatchEntryForm.SampleBatchEntrySetup.class)
          SampleBatchEntryForm form,
  ...
  private void setupEID(SampleBatchEntryForm form, HttpServletRequest request)
    ...
    if (projectData.getDbsTaken()) {
      sampleTypes =
          sampleTypes + " " + MessageUtil.getMessage("sample.entry.project.title.dryBloodSpot");
    }
  ...
  private void setupViralLoad(SampleBatchEntryForm form, HttpServletRequest request)
    ...
    if (projectData.getEdtaTubeTaken()) {
      sampleTypes =
          sampleTypes + " " + MessageUtil.getMessage("sample.entry.project.ARV.edtaTubeTaken");
    }
  ...
  protected void addOrganizationLists(SampleBatchEntryForm form)
    ...
    form.getProjectData()
        .setEidSecondPCRReasonList(ObservationHistoryList.EID_SECOND_PCR_REASON.getList());

    // Get SPE Request Reasons
    form.getProjectData()
        .setRequestReasons(ObservationHistoryList.SPECIAL_REQUEST_REASONS.getList());
