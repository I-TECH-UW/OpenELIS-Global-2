...
public class ObservationHistoryServiceImpl
    extends AuditableBaseObjectServiceImpl<ObservationHistory, String>
  ...
  public enum ObservationType {
    ...
    VL_PREGNANCY("vlPregnancy"),
    VL_SUCKLE("vlSuckle"),
    TB_ORDER_REASON("TbOrderReason"),
    TB_DIAGNOSTIC_REASON("TbDiagnosticReason"),
    TB_FOLLOWUP_REASON("TbFollowupReason"),
    TB_FOLLOWUP_PERIOD_LINE1("TbFollowupReasonPeriodLine1"),
    TB_FOLLOWUP_PERIOD_LINE2("TbFollowupReasonPeriodLine2"),
    TB_ANALYSIS_METHOD("TbAnalysisMethod"),
  ...
  private final Map<ObservationType, String> observationTypeToIdMap =
      new EnumMap<>(ObservationType.class);

  @Autowired private ObservationHistoryDAO baseObjectDAO;
  ...
  @Override
  @Transactional(readOnly = true)
  public List<ObservationHistory> getObservationsByTypeAndValue(
    ...
    if (!GenericValidator.isBlankOrNull(typeId)) {
      return baseObjectDAO.getObservationHistoriesByValueAndType(
          value, typeId, ValueType.LITERAL.getCode());
    } else {
      return null;
    }
  }
  ...
  public ObservationHistory getLastObservationForPatient(ObservationType type, String patientId) {
    ...
    if (!GenericValidator.isBlankOrNull(typeId)) {
      List<ObservationHistory> observationList =
          baseObjectDAO.getObservationHistoriesByPatientIdAndType(patientId, typeId);
  ...
  @Override
  @Transactional(readOnly = true)
  public List<ObservationHistory> getAll(
      Patient patient, Sample sample, String observationHistoryTypeId) {
    return getBaseObjectDAO().getAll(patient, sample, observationHistoryTypeId);
  }

  @Override
  @Transactional(readOnly = true)
  public ObservationHistory getObservationHistoriesBySampleIdAndType(
      String sampleId, String observationHistoryTypeId) {
    return getBaseObjectDAO()
        .getObservationHistoriesBySampleIdAndType(sampleId, observationHistoryTypeId);
  }

  @Override
  @Transactional(readOnly = true)
  public List<ObservationHistory> getObservationHistoriesByPatientIdAndType(
      String patientId, String observationHistoryTypeId) {
    return getBaseObjectDAO()
        .getObservationHistoriesByPatientIdAndType(patientId, observationHistoryTypeId);
  }
  ...
  @Override
  @Transactional(readOnly = true)
  public List<ObservationHistory> getObservationHistoriesByValueAndType(
