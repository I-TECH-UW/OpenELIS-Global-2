...
@Service
public class PatientServiceImpl extends AuditableBaseObjectServiceImpl<Patient, String>
  ...
  @Autowired private PatientDAO baseObjectDAO;

  @Autowired private PatientIdentityService patientIdentityService;
  @Autowired private GenderService genderService;
  @Autowired private PatientIdentityTypeService identityTypeService;
  @Autowired private AddressPartService addressPartService;
  @Autowired private PersonAddressService personAddressService;
  @Autowired private PatientPatientTypeService patientPatientTypeService;
  @Autowired private PersonService personService;
  @Autowired protected FhirPersistanceService fhirPersistanceService;
  ...
  public Patient getPatientForGuid(String guid) {
    List<PatientIdentity> identites =
        patientIdentityService.getPatientIdentitiesByValueAndType(guid, PATIENT_GUID_IDENTITY);
  ...
  private String getIdentityInfo(Patient patient, String identityId) {
    ...
    PatientIdentity identity =
        patientIdentityService.getPatitentIdentityForPatientAndType(patient.getId(), identityId);
  ...
  @Override
  @Transactional(readOnly = true)
  public List<String> getPatientIdentityBySampleStatusIdAndProject(
      List<Integer> inclusiveStatusIdList, String study) {
    return getBaseObjectDAO()
        .getPatientIdentityBySampleStatusIdAndProject(inclusiveStatusIdList, study);
  }

  @Override
  @Transactional
  public void persistPatientData(
      PatientManagementInfo patientInfo, Patient patient, String sysUserId)
  ...
  protected void persistIdentityTypes(
      PatientManagementInfo patientInfo, Patient patient, String sysUserId) {
    ...
    persistIdentityType(
        patientInfo.getInsuranceNumber(), "INSURANCE", patientInfo, patient, sysUserId);
    ...
    persistIdentityType(
        patientInfo.getMothersInitial(), "MOTHERS_INITIAL", patientInfo, patient, sysUserId);
    persistIdentityType(patientInfo.getEducation(), "EDUCATION", patientInfo, patient, sysUserId);
    persistIdentityType(
        patientInfo.getMaritialStatus(), "MARITIAL", patientInfo, patient, sysUserId);
    persistIdentityType(
        patientInfo.getNationality(), "NATIONALITY", patientInfo, patient, sysUserId);
    persistIdentityType(
        patientInfo.getHealthDistrict(), "HEALTH DISTRICT", patientInfo, patient, sysUserId);
    persistIdentityType(
        patientInfo.getHealthRegion(), "HEALTH REGION", patientInfo, patient, sysUserId);
    persistIdentityType(
        patientInfo.getOtherNationality(), "OTHER NATIONALITY", patientInfo, patient, sysUserId);
  ...
  private void persistExtraPatientAddressInfo(
    ...
    List<PersonAddress> personAddressList =
        personAddressService.getAddressPartsByPersonId(patient.getPerson().getId());
    ...
    if (commune == null) {
      insertNewPatientAddressInfo(
          ADDRESS_PART_COMMUNE_ID, patientInfo.getCommune(), "T", patient, sysUserId);
    }

    if (village == null) {
      insertNewPatientAddressInfo(
          ADDRESS_PART_VILLAGE_ID, patientInfo.getCity(), "T", patient, sysUserId);
    }

    if (dept == null
        && patientInfo.getAddressDepartment() != null
        && !patientInfo.getAddressDepartment().equals("0")) {
      insertNewPatientAddressInfo(
          ADDRESS_PART_DEPT_ID, patientInfo.getAddressDepartment(), "D", patient, sysUserId);
    }
  }

  @Override
  public void insertNewPatientAddressInfo(
      String partId, String value, String type, Patient patient, String sysUserId) {
  ...
  public void persistIdentityType(
      String paramValue,
      String type,
    ...
    if (patientInfo.getPatientUpdateStatus() == PatientUpdateStatus.UPDATE) {
      ...
      for (PatientIdentity listIdentity : patientInfo.getPatientIdentities()) {
        if (listIdentity.getIdentityTypeId().equals(typeID)) {
          ...
          if ((listIdentity.getIdentityData() == null
                  && !GenericValidator.isBlankOrNull(paramValue))
  ...
  protected void persistPatientType(
      PatientManagementInfo patientInfo, Patient patient, String sysUserId) {
    ...
    if (!GenericValidator.isBlankOrNull(typeName) && !"0".equals(typeName)) {
      ...
      PatientPatientType patientPatientType =
          patientPatientTypeService.getPatientPatientTypeForPatient(patient.getId());
