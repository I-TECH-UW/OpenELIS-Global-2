...
/**
 * Update/Creates, as needed, a Sample and Patient and all associated parts in order to enter that
 * patient in the system. To use one of these to accession, you should: <nl>
 * <li>Provide a constructor (which probably includes whatever structure comes from your UI form, so
 *     it can be used below).
 * <li>set the expected new RecordStatus for the patient and sample (maybe in the constructor), but
 *     you can change that later.
 * <li>implement canAccession() to decide if this is the right time to try this accession algorithm.
 * <li>implement validate* and match* methods to make sure all the entities created and found hang
 *     together.
 * <li>implement the missing populate* methods (including providing any empty implementation).
 * <li>possibly override the persist* methods, if there is more to do than simply saving what has
 *     been built (delete any old ones? Update instead?). </nl> Use:<br>
 *     if (myAccessioner1.canAccession()) { if (!myAccession1.accession(...)) { errors =
 *     myAccession1.getMessages(); saveErrors(request, errors);
 *     request.setAttribute(Globals.ERROR_KEY, errors); return mapping.findForward(FWD_FAIL); } else
 *     { return mapping.findForward(FWD_SUCCESS); } } else (myAccession2.canAccession() { ... }
 *
 *     <p>PAH 07/2010 This object is still a work in progress. For example, we have a member for
 *     projectFormMapper, but all its use in the subclasses at this time . Maybe the
 *     projectFormMapper should just be injected after creation? It is also the case that there are
 *     form field property names listed in these various acccesioning classes when the
 *     formFieldMapper class is really the class that should know the right place to find values on
 *     the submitted form.
 *
 * @author pahill
 */
public abstract class Accessioner implements IAccessioner {
  ...
  /**
   * Find the projectFormName where ever we normally store it. This is for that could which needs
   * this information before creating a projectFormMapper
   *
   * @param form
   ...
   */
  public static IProjectFormMapper getProjectFormMapper(IAccessionerForm form)
      throws LIMSRuntimeException {
   ...
   * @return
   */
  public static IProjectFormMapper getProjectFormMapper(
      String projectFormName, IAccessionerForm form) {
  ...
  protected PatientIdentityService identityService =
  ...
  protected SampleProjectService sampleProjectService =
  ...
  protected Accessioner(
      String sampleIdentifier, String patientIdentifier, String siteSubjectNo, String sysUserId) {
    this();
  ...
  /**
   * Primary entry point for processing a patient/sample combination Check the messages, on true
  ...
  public String save()
      throws IllegalAccessException, LIMSRuntimeException, InvocationTargetException,
  ...
  /** This is probably the result of Sample(Second)Entry, so nothing to due by default */
  ...
  /**
   * This method is called when this object contains a patient from the database found by ID.
   * Sometimes we need to validate some of the fields with the values from the input. Place an error
   * in the messages list to return an error.
   *
   * @return TRUE => all is well with the patient we have found in the database vs. the data coming
   *     from the input.
   */
  ...
  /**
   * This method is called when this object contains a sample from the database found by ID.
   * Sometimes we need to validate some of the fields with the values from the input. Place an error
   * in the messages list to return an error.
   *
   * @return TRUE => all is well with the SAMPLE we have found in the database vs. the data coming
   *     from the input.
   */
  ...
  /**
   * This method is called when there is something to check to make sure the patient and sample go
   * together correctly.
   *
   ...
   *
   * @return TRUE => patient was found in the database, FALSE => patient not found, simply created.
   */
  protected boolean findPatient() {
    ...
    if (patientIdentifiersDoNotMatch()) {
      ...
      if (otherPatient == null) { // unknown
        int otherSamplesOnCurrentPatient =
            sampleHumanService.getSamplesForPatient(patientId).size() - 1;
  ...
  private boolean patientIdentifiersDoNotMatch() {
    ...
    String existingSubjectNo =
        StringUtil.replaceNullWithEmptyString(patientInDB.getNationalId()).trim();

    if (!GenericValidator.isBlankOrNull(patientIdentifier)
        && patientIdentifier.equals(existingSubjectNo)) {
      return false;
    }

    String existingSiteSubjectNo =
        StringUtil.replaceNullWithEmptyString(patientInDB.getExternalId()).trim();
  ...
  /**
   * @return if the sample we are working with is NOT in the database this object is about to create
   *     it.
   */
  ...
  /**
   * @return TRUE if the patient is not the known patient already associated with the known sample.
   */
  ...
  /**
   * Call this to record that when we started, the patient record associated with the original
   * sample record is NOT the record we are now working with. We record this because, once we
   * updating records to write, we can longer compare IDs to come up with the answer.
   */
  protected void setADifferentPatient(boolean force) {
    ...
    // to true
    aDifferentPatientRecord =
        (force || patientInDB == null || statusSet.getPatientId() != patientInDB.getId());
  }

  /**
   * Test this object to see if we should even begin. This is intended for checking existing
   * patient/sample record status.
   *
   * @return TRUE => all is well; FALSE (Default) => this particular version of the accession
   *     process is not appropriate for the status combination.
   */
  ...
  protected void populateObservationHistory()
    ...
    ObservationData observationData =
        (ObservationData) (PropertyUtils.getProperty(projectFormMapper.getForm(), "observations"));
  ...
  /**
   * Move data from the form to the sample and sample related objects, include SampleOrganization
   * but not to any of the entities which tie a patient to a sample; don't include
   * ObservationHistory and SampleHuman
   *
  ...
  /**
   * Build an example Patient record with the ID only for the current patient of the sample
   *
   * @return
   */
  ...
  protected void populateSample(
      Timestamp receivedDateForDisplay, Timestamp collectionDateForDisplay) {
    sample.setAccessionNumber(accessionNumber);
  ...
  private boolean isSampleInProject() {
    ...
    SampleProject oldSampleProject =
        sampleProjectService.getSampleProjectBySampleId(sample.getId());
    return oldSampleProject != null;
  }

  /**
   * create a sampleOrganization record, finding any old one to delete. While there can be more than
   * one organization per sample, this code only supports one.
   *
   ...
   * @param paramValue a value for a particular patient identity; if null, do nothing.
   */
  ...
  /**
   * Given a possibly ambiguous birthday date string, get the right values into both (1) the patient
   * birthdate and (2) make sure we create a patient_identity for any ambiguous value.
   */
  protected void populatePatientBirthDate(String birthDateForDisplay) {
    patientInDB.setBirthDateForDisplay(birthDateForDisplay);
  }

  protected void populateSampleItems(
      List<TypeOfSampleTests> typeofSampleTestList, Timestamp collectionDate) {
  ...
  protected SampleItem buildSampleItem(
      Sample sample, TypeOfSample typeofsample, Timestamp collectionDate) {
  ...
  /**
   * @param itemsByType
   * @return the maximum value which has already been used in the set of SampleItems
   */
  ...
  /**
   * If during entry we find (1) all the analysis are finished, and (2) the sample is not already
   * been declared bad, then we're ready to mark the sample as done. @
   */
  @Transactional
  public void completeSample() {
    if (isAllAnalysisDone()
        && !SpringContext.getBean(IStatusService.class)
  ...
  /**
   * The question is whether we are ready to update the sample status. TODO Pahill maybe we could
   * move this to SpringContext.getBean(IStatusService.class)?
   */
  ...
  private Analysis buildAnalysis(
      String analysisRevision, SampleItemAnalysisCollection sampleTestCollection, Test test) {
    java.sql.Date collectionDateTime =
        DateUtil.convertStringDateToSqlDate(sampleTestCollection.collectionDate);
  ...
  protected void persistSampleHuman() {
    if (sampleHuman != null) {
      SampleHuman otherSampleHuman =
          sampleHumanService.getMatch("sampleId", sample.getId()).orElse(null);
  ...
  protected void persistObservationHistory() {
    if (isADifferentPatient()) {
      List<ObservationHistory> oldOHes =
          observationHistoryService.getAll(knownPatientTemplate(), knownSampleTemplate());
    ...
    for (ObservationHistory newOh : observationHistories) {
      List<ObservationHistory> existingTypeOHes =
  ...
  private Map<String, ObservationHistory> findExistingObservationHistories(String nameKey) {
    ...
    List<ObservationHistory> existingOHes =
        observationHistoryService.getAll(patientInDB, sample, ohTypeId);
  ...
  protected void persistRecordStatus() {
    ...
    SpringContext.getBean(IStatusService.class)
        .persistRecordStatusForSample(
            sample, newSampleStatus, patientInDB, newPatientStatus, sysUserId);
  }
  ...
  protected void deleteOldPatient() {
    if (patientToDelete != null) {
      try {
        ...
        List<PatientIdentity> oldIdentities =
            identityService.getPatientIdentitiesForPatient(patientToDelete.getId());
  ...
  private static String getObservationHistoryTypeId(
      ObservationHistoryTypeService ohtService, String name) {
