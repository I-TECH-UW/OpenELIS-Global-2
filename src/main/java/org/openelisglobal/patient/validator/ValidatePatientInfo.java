...
public class ValidatePatientInfo {
  ...
  private static final String AMBIGUOUS_DATE_CHAR =
  ...
  public static void validatePatientInfo(Errors errors, PatientManagementInfo patientInfo) {
    boolean disallowDuplicateSubjectNumbers =
        ConfigurationProperties.getInstance()
            .isPropertyValueEqual(
                ConfigurationProperties.Property.ALLOW_DUPLICATE_SUBJECT_NUMBERS, "false");
    boolean disallowDuplicateNationalIds =
        ConfigurationProperties.getInstance()
            .isPropertyValueEqual(
                ConfigurationProperties.Property.ALLOW_DUPLICATE_NATIONAL_IDS, "false");
    if (disallowDuplicateSubjectNumbers || disallowDuplicateNationalIds) {
      String newSTNumber =
          GenericValidator.isBlankOrNull(patientInfo.getSTnumber())
              ? null
              : patientInfo.getSTnumber();
      String newSubjectNumber =
          GenericValidator.isBlankOrNull(patientInfo.getSubjectNumber())
              ? null
      ...
      if (!results.isEmpty()) {
        ...
        for (PatientSearchResults result : results) {
          if (!result.getPatientID().equals(patientInfo.getPatientPK())) {
            if (disallowDuplicateSubjectNumbers
                && newSTNumber != null
                && newSTNumber.equals(result.getSTNumber())) {
              if (existingResult == null
                  || (existingResult != null
                      && !existingResult.getSTNumber().equals(newSTNumber))) {
                errors.reject("error.duplicate.STNumber", null, null);
              }
            }
            if (disallowDuplicateSubjectNumbers
                && newSubjectNumber != null
