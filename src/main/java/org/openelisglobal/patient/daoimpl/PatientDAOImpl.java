...
public class PatientDAOImpl extends BaseDAOImpl<Patient, String> implements PatientDAO {
  ...
  protected Patient getPatientByStringProperty(String propertyName, String propertyValue) {
    ...
    try {
      ...
      throw new LIMSRuntimeException(
          "Error in Patient getPatientByStringProperty(" + propertyName + "\", ) ", e);
    }
  ...
  public List<String> getPatientIdentityBySampleStatusIdAndProject(
      List<Integer> inclusiveStatusIdList, String project) throws LIMSRuntimeException {

    boolean useIdList = inclusiveStatusIdList != null && inclusiveStatusIdList.size() > 0;

    StringBuilder sqlBuilder = new StringBuilder();
    sqlBuilder.append(
        "select COALESCE(pat.national_id, pat.external_id) as subjectNo from clinlims.sample s ");
    ...
    try {
      NativeQuery query =
          entityManager.unwrap(Session.class).createNativeQuery(sqlBuilder.toString());
      if (useIdList) {
