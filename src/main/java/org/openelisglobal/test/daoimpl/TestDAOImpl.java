...
public class TestDAOImpl extends BaseDAOImpl<Test, String> implements TestDAO {
  ...
  public List<Test> getAllActiveOrderableTests() throws LIMSRuntimeException {
    try {
      String sql =
          "from Test t WHERE t.isActive = 'Y'  and t.orderable = true Order by t.description";
      Query<Test> query = entityManager.unwrap(Session.class).createQuery(sql, Test.class);
  ...
  public List<Test> getPageOfSearchedTests(int startingRecNo, String searchString)
    ...
    try {
      ...
      if (wCdPosition == -1) // no wild card looking for exact match
      {
        ...
        sql =
            "from Test t  where trim(lower (t.description)) = :param  order by"
                + " t.testSection.testSectionName";
      } else {
        newSearchStr = searchString.replace(wildCard, "%").toLowerCase().trim();
        sql =
            "from Test t where trim(lower (t.description)) like :param  order by"
                + " t.testSection.testSectionName";
      }
      Query<Test> query = entityManager.unwrap(Session.class).createQuery(sql, Test.class);
  ...
  public List<Test> getTests(String filter, boolean onlyTestsFullySetup)
    ...
    try {
      String sql =
          "from Test t where (upper(t.localizedTestName.english) like upper(:param) or"
  ...
  public List<Test> getTestsByName(String testName) throws LIMSRuntimeException {
    String sql =
        "from Test t where (t.localizedTestName.english = :testName or t.localizedTestName.french ="
            + " :testName)";
    try {
      Query<Test> query = entityManager.unwrap(Session.class).createQuery(sql, Test.class);
  ...
  public List<Test> getActiveTestsByPanelName(String panelName) throws LIMSRuntimeException {
    String sql =
        "SELECT t.* from test t JOIN panel_item pi ON t.id = pi.test_id JOIN panel p ON p.id ="
  ...
  public boolean duplicateTestExists(Test test) throws LIMSRuntimeException {
    try {
      ...
      if (test.getIsActive().equalsIgnoreCase("Y")) {
        ...
        // duplicates
        String sql =
            "from Test t where (trim(lower(t.description)) = :description and t.isActive='Y' and"
                + " t.id != :testId)";
        Query<Test> query = entityManager.unwrap(Session.class).createQuery(sql, Test.class);
  ...
  public Integer getNextAvailableSortOrderByTestSection(Test test) throws LIMSRuntimeException {
    ...
    try {
      ...
      String sql =
          "from Test t where t.testSection = :param and t.sortOrder is not null order by"
              + " t.sortOrder desc";
      Query<Test> query = entityManager.unwrap(Session.class).createQuery(sql, Test.class);
  ...
  /**
   * @see org.openelisglobal.test.dao.TestDAO#getAllOrderBy(java.lang.String) Read all entities from
   *     the database sorted by an appropriate property
   */
  @Override
  @Transactional(readOnly = true)
    ...
    try {
      if (!StringUtil.isJavaIdentifier(columnName)) {
        throw new IllegalArgumentException(
            "\"" + columnName + "\" is not valid syntax for a column name");
      }
  ...
  public List<Test> getAllTestsByDictionaryResult() {
    String sql =
        "From Test t where t.id in (select tr.test from TestResult tr where tr.testResultType in"
  ...
  public List<Test> getTbTestByMethod(String method) throws LIMSRuntimeException {
    List<Integer> methodIds =
        Arrays.asList(method.split(",")).stream()
            .map(e -> Integer.parseInt(e))
            .collect(Collectors.toList());
    String sql =
        "SELECT t.* From test t JOIN tb_method_test tm ON t.id = tm.test_id where tm.method_id in"
  ...
  public List<Test> getTbTest() throws LIMSRuntimeException {
    String sql =
        "SELECT t.* From test t JOIN test_section ts ON t.test_section_id = ts.id where"
  ...
  public List<Panel> getTbPanelsByMethod(String method) throws LIMSRuntimeException {
    ...
    String sql =
        "SELECT p.* From panel p JOIN tb_method_panel tm ON p.id = tm.panel_id where tm.method_id"
