...
public class TestSectionServiceImpl extends AuditableBaseObjectServiceImpl<TestSection, String>
  ...
  @Autowired private TestSectionDAO baseObjectDAO;
  @Autowired private SystemUserSectionService systemUserSectionService;
  ...
  private synchronized void createTestIdToNameMap() {
    ...
    for (TestSection testSection : testSections) {
      testUnitIdToNameMap.put(
          testSection.getId(), buildTestSectionName(testSection).replace("\n", " "));
    }
  }
  ...
  public List<TestSection> getTestSectionsBySysUserId(String filter, int sysUserId) {
    ...
    List<SystemUserSection> userTestSectionList =
        systemUserSectionService.getAllSystemUserSectionsBySystemUserId(sysUserId);
