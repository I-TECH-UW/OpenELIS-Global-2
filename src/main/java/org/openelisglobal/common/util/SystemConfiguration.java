...
public class SystemConfiguration {
  ...
  public String getDefaultSampleLabel(String accessionNumber) {
    ...
    String prependHumanReadable =
        properties.getProperty("print.label.sample.prepend.humanreadable");
    ...
    if (prependBarcode != null && prependHumanReadable != null && postpend != null) {
      int sampleLabelLength =
          prependBarcode.length()
              + accessionNumber.length()
              + prependHumanReadable.length()
              + accessionNumber.length()
              + postpend.length();
      StringBuffer sb = new StringBuffer(sampleLabelLength);
      sb.append(prependBarcode)
          .append(accessionNumber)
          .append(prependHumanReadable)
          .append(accessionNumber)
          .append(postpend);
      return sb.toString();
    }
