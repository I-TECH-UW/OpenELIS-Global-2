...
public abstract class BaseController implements IActionConstants {
  ...
  @Autowired protected UserModuleService userModuleService;
  @Autowired protected PageBuilderService pageBuilderService;

  protected abstract String findLocalForward(String forward);

  /**
   * Must be implemented by subclasses to set the title for the requested page. The value returned
   * should be a key String from the ApplicationResources.properties file.
   *
  ...
  /**
   * Must be implemented by subclasses to set the subtitle for the requested page. The value
   * returned should be a key String from the ApplicationResources.properties file.
   *
  ...
  /**
   * This getPageTitleKey method accepts a request and form parameter so that a subclass can
   * override the method and conditionally return different titles.
   *
  ...
  /**
   * This getSubtitleKey method accepts a request and form parameter so that a subclass can override
   * the method and conditionally return different subtitles.
   *
   * @param request the request
   * @param form the form associated with this request.
   * @return the subtitle key this page.
  ...
  /**
   * Template method to allow subclasses to handle special cases. The default is to return the
   * message
   *
  ...
  protected String getMessageForKey(String messageKey) {
    ...
    return MessageUtil.messageNotFound(message, messageKey)
        ? getActualMessage(messageKey)
        : message;
  }
  ...
  protected boolean userHasPermissionForModule(HttpServletRequest request, String module) {
    if (!userModuleService.isUserAdmin(request)
      ...
      HashSet accessMap =
          (HashSet) request.getSession().getAttribute(IActionConstants.PERMITTED_ACTIONS_MAP);
  ...
  protected ModelAndView findForward(
      String forward, Map<String, Object> requestObjects, BaseForm form) {
  ...
  protected void saveErrors(Errors errors) {
    ...
    } else if (previousErrors.hasErrors()
        && previousErrors.getObjectName().equals(errors.getObjectName())) {
    ...
    try {
      ...
      for (ObjectError error : result.getGlobalErrors()) {
        errorMsgBuilder.append(
            MessageUtil.getMessageOrDefault(
