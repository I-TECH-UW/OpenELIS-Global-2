...
/**
 * Class which allows users to create trees on the client browser, hiding all
 * the javascript implementation details while presenting the developer with an
 * easy to use javax.swing.Jtree like interface to work with.
 *
 * @author jalpesh
 *
 */
public class Tree {
	...
	/**
	 * Create a tree with a given name. Please note that the name of the tree should
	 * be unique in a given user session. So for example if your site has five trees
	 * in different pages, all the trees should have a unique name
	 *
	 * @param name The name of the tree. Please note that the name of the tree
	 *             should not contain any of the following characters -
	 *             single/doublequotes (',"), forward/backward slash (/,\) or the
	 *             period (.)
	 */
	 ...
	 * @param isLocal  Whether or not the tree is in local mode or a client/server
	 *                 mode.
	 */
	public Tree(String name, TreeNode rootNode, boolean isLocal) {
		this.name = name;
		baseDirectory = "static/images/tree_images/";
		useCookies = true;
	...
	/**
	 * Each tree can optionally have one node which is highlighted. This method
	 * returns the highlighted node path for that tree. If this parameter is not
	 * set, upon rendering the tree will try to find a highlighted node in the
	 * TreeStateManager if it exists, and then use it instead.
	 *
	...
	/**
	 * Each tree can optionally have one node which is highlighted. This method sets
	 * the highlighted node path for that tree. If this parameter is not set, upon
	 * rendering the tree will try to find a highlighted node in the
	 * TreeStateManager if it exists, and then use it instead.
	 *
	 ...
	 *         This defaults to "images/tree_images/" which will look for the
	 *         images/tree_images directory inside your current directory.
	 */
	...
	/**
	 * @param baseDirectory Set the base directory where the tree generator will
	 *                      look for images of the tree. Defaults to
	 *                      "images/tree_images/" if not set
	 */
	...
	/**
	 * @return Returns whether or not the tree is in a local mode or not. A tree in
	 *         a local mode will not hit the server in case of an expand or a
	 *         collapse action.
	 */
	...
	/**
	 * @param isLocal Sets the local mode of the tree. By default it is false. Local
	 *                mode determines whether the entire tree is generated on the
	 *                client or, the tree is generated on a need basis only.
	 */
	...
	/**
	 * @return Returns the name of the tree. Each tree should have a unique name to
	 *         avoid conflicts.
	 */
	...
	/**
	 * @param rootNode Each tree should at least have one root node. This is set
	 *                 using this method.
	 */
	...
	/**
	 * @return Returns true or false indicating whether or not the client is to use
	 *         cookies if the tree is in a local mode (ie setLocal is true)
	 */
	...
	/**
	 * @param useCookies Indicate the tree whether or not to use cookies in case of
	 *                   a local mode.
	 */
	...
	/**
	 * Set the values of the displayed text messages for this tree. This is useful
	 * for localization issues. If you tree is only going to be used in US English
	 * local you can use the default values. However it is advisable to set these
	 * values if you want the tree to be fully localized.
	 *
	 ...
	 */
	public void setI18nText(String textNode, String textCollapse, String textCollapsed, String textExpand,
			String textExpanded, String textLevel, String textOf, String textSelected, String textNext,
			String textPrev) {
	...
	protected String getJavascriptDefinition(HttpServletRequest request) {
		...
		// we need name, root node, base directory, isLocal, and optionally the
		// highlight node
		// root node will always be defined as _a
		String script = "   tree = new Tree('" + TreeNode.escapeQuotes(getName()) + "',_a,'" + getBaseDirectory() + "',"
				+ isLocal()
				+ (highlightNodePath != null ? ",'" + TreeNode.escapeQuotes(highlightNodePath) + "'" : ",null")
				+ ");\n";
	...
	public String renderTree(HttpServletRequest request) {
		...
		// mozilla does not work very well with overflow: style, especially if there is
		// a vertical scrollbar in the page. Whenever you click
		// on a link, it will jump to the middle of the div. So we are eliminating the
		// style for every browser other than IE -->
		...
		treeScript += "pathDelimeter = '"
				+ TreeNode.escapeQuotes(new String(new char[] { TreeNode.getPathDelimeter() })) + "';\n";
		...
		// finally generate the popups. Order of generation is important - first
		// generate the nodes, then the tree and finally
		// the popups.
	...
	private String generateNodeScript(TreeNode node, String parentVarName) {
		// each node should know about the tree it belongs to. Call this before
		// generating the script definition
		node.setTree(this);

		String script = node.getJavascriptDefinition(parentVarName);
		totalNodes++; // keep track of nodes in the system.
	...
	/**
	 * Sets the instrumentation for the tree to on or off. When instrumentation is
	 * turned on, the tree displays time taken to render the tree to the user.
	 *
	...
	public String stopTimer() {
		...
		s += "\tvar s3 = 'Client/Server ratio : ' + Math.round(100*__tt/__total) + '% :: ' + "
				+ " Math.round(100*__stt/__total) + '%'; \n";
