...
/**
 * This class represents a PopupMenu which is displayed when a user right clicks
 * on a node. Tree popup are unique to each tree and each node can have one
 * TreePopup associated with it. When creating a TreePopup object, care should
 * be taken that the name of the TreePopup is unique within that tree.
 * 
 * @author jalpesh
 */
public class TreePopup implements Serializable {
    ...
    /**
     * Create a popup with the specified name. The tree popup object should be
    ...
    protected String getJavascriptDefinition() {
        ...
        for (int i = 0; i < popupItems.size(); i++) {
            ...
            script += "   " + name + ".push(new JSTreePopup('" + text + "','" + item.action + "',"
                    + (item.imageSrc == null ? "null" : "'" + item.imageSrc + "'") + "));\n";
        }
