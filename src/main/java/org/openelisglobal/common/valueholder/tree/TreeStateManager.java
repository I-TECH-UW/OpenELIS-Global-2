 ...
 *
 */
public class TreeStateManager implements java.io.Serializable {
    ...
    public void processState(HttpServletRequest request) {
        // check to ensure that expand all nodes (below a nodePath) is not called. If it
        // is not invoked,
        ...
        // if a node was selected, mark it has highlighted, and also store the data
        // field
        // we only do this if there is a request to highlight a certain node. So the net
        // effect
        // is that a highlighted node is always sticky, it remains highlighted until
