...
public abstract class BaseObject<PK extends Serializable> implements Serializable, Cloneable {
  ...
  @Transient private Timestamp originalLastupdated;
  @Transient private String sysUserId;
  @Transient private String nameKey;
  ...
  /**
   * A useful routine for sorting any (DB) entity object in Global OpenELIS by it's localized name.
   * If it starts with a number, sort by that ("2" comes after "1", not after "10". Try not to do
   * this too often, since your are looking at strings from the resource file, but if you need a
   * list for a UI or a report in local Alphabetical order, this is just the thing to do.
   *
   * @param list
   */
