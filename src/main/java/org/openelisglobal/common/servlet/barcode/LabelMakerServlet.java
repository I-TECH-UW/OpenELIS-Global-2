...
/**
 * Servlet for pages to send bar code label requests. Returns as pdf unless... User unauthenticated
 * - gives error response Invalid request parameters - gives error response Maximum printing has
 * been reached (pdf length 0) - returns override page
 *
 * @author Caleb
 */
public class LabelMakerServlet extends HttpServlet implements IActionConstants {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if ("block".equals(request.getParameter("labelType"))
        || "slide".equals(request.getParameter("labelType"))) {
  ...
  private void printPathologyBarcodeLabel(HttpServletRequest request, HttpServletResponse response)
    ...
    labelMaker.generateGenericBarcodeLabel(
        request.getParameter("code"), request.getParameter("labelType"));
  ...
  private void prePrintLabels(HttpServletRequest request, HttpServletResponse response)
    ...
    for (String testId : testIdsArray) {
      tests.add(
          SpringContext.getBean(TestService.class).getActiveTestById(Integer.parseInt(testId)));
    }
    ...
    if (GenericValidator.isBlankOrNull(startingAt)
        || startingAt.trim().equals("null")
        || startingAt.trim().equals("undefined")) {
      startingAt = "";
    }
    labelMaker.generatePrePrintLabels(
        Integer.parseInt(request.getParameter("numSetsOfLabels")),
  ...
  private void printExistingOrder(HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    // get parameters
  ...
  private Errors validate(
      String labNo, String programCode, String type, String quantity, String override) {
    ...
    if (validateAccessionNumber) {
      ...
      if (!(IAccessionNumberValidator.ValidationResults.SUCCESS
          == accessionNumberValidator.validFormat(accessionNumber, false))) {
        errors.reject(
            "barcode.label.error.accession.invalid", "barcode.label.error.accession.invalid");
      }
    ...
    if (sampleService.getSampleByAccessionNumber(labNo) == null) {
      errors.reject(
          "barcode.label.error.accession.nosample", "barcode.label.error.accession.nosample");
    }
    // validate override
