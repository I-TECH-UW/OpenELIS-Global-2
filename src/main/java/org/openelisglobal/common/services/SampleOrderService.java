...
public class SampleOrderService {
  ...
  private ProgramSampleService programSampleService =
      SpringContext.getBean(ProgramSampleService.class);
  ...
  private boolean needPaymentOptions =
      ConfigurationProperties.getInstance()
          .isPropertyValueEqual(ConfigurationProperties.Property.TRACK_PATIENT_PAYMENT, "true");
  private boolean needTestLocationCode =
      FormFields.getInstance().useField(FormFields.Field.TEST_LOCATION_CODE);
  ...
  private SampleOrderItem getBaseSampleOrderItem() {
    ...
    orderItems.setReceivedTime(
        DateUtil.convertTimestampToStringHourTime(DateUtil.getNowAsTimestamp()));

    orderItems.setProvidersList(
        DisplayListService.getInstance()
            .getFreshList(DisplayListService.ListType.PRACTITIONER_PERSONS));
    ...
    if (ConfigurationProperties.getInstance()
        .isPropertyValueEqual(ConfigurationProperties.Property.ORDER_PROGRAM, "true")) {
  ...
  /**
   * If this service is created with the default no parameter constructor then this will generate a
   * SampleOrderItem populated with the appropriate lists but no other values.
   *
    ...
    if (sample != null) {
      ...
      sampleOrder.setRequestDate(
          observationHistoryService.getValueForSample(
              ObservationType.REQUEST_DATE, sample.getId()));
      sampleOrder.setReferringPatientNumber(
          observationHistoryService.getValueForSample(
              ObservationType.REFERRERS_PATIENT_ID, sample.getId()));
      sampleOrder.setNextVisitDate(
          observationHistoryService.getValueForSample(
              ObservationType.NEXT_VISIT_DATE, sample.getId()));
      sampleOrder.setPaymentOptionSelection(
          observationHistoryService.getRawValueForSample(
              ObservationType.PAYMENT_STATUS, sample.getId()));
      sampleOrder.setTestLocationCode(
          observationHistoryService.getRawValueForSample(
              ObservationType.TEST_LOCATION_CODE, sample.getId()));
      sampleOrder.setOtherLocationCode(
          observationHistoryService.getValueForSample(
              ObservationType.TEST_LOCATION_CODE_OTHER, sample.getId()));
      sampleOrder.setBillingReferenceNumber(
          observationHistoryService.getValueForSample(
              ObservationType.BILLING_REFERENCE_NUMBER, sample.getId()));
      ...
      ProgramSample programSample =
          programSampleService.getProgrammeSampleBySample(
              Integer.valueOf(sample.getId()), programName);
        ...
        if (programSample.getQuestionnaireResponseUuid() != null) {
          sampleOrder.setAdditionalQuestions(
              fhirUtil
                  ...
                  .withId(programSample.getQuestionnaireResponseUuid().toString())
                  .execute());
        }
      }
      ...
      if (requesterService.getPerson() != null) {
        Provider provider =
            SpringContext.getBean(ProviderService.class)
  ...
  public SampleOrderPersistenceArtifacts getPersistenceArtifacts(
      Sample sample, String currentUserId) {
    if (sampleOrder == null) {
  ...
  private void createSampleArtifacts(
      Sample sample, String currentUserId, SampleOrderPersistenceArtifacts artifacts) {
    if (sample == null) {
      sample = new Sample();
  ...
  private void createProviderArtifacts(
    ...
    createOrganizationDepartProviderArtifacts(
        sampleOrder, currentUserId, artifacts, requesterService);
  }

  private void createPersonProviderArtifacts(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts,
            ...
            SpringContext.getBean(ProviderService.class)
                .getProviderByPerson(
                    SpringContext.getBean(PersonService.class)
                        .get(sampleOrder.getProviderPersonId()));
    ...
    List<SampleRequester> personSampleRequesters =
        requesterService.getSampleRequestersByType(RequesterService.Requester.PERSON, true);
    SampleRequester samplePersonRequester =
        personSampleRequesters.size() > 0 ? personSampleRequesters.get(0) : null;
  ...
  private boolean namesDiffer(Person providerPerson, SampleOrderItem sampleOrder) {
    ...
    if (providerPerson.getId().trim().equals(sampleOrder.getProviderPersonId().trim())) {
      return StringUtil.compareWithNulls(
                  providerPerson.getFirstName(), sampleOrder.getProviderFirstName())
              != 0
          || StringUtil.compareWithNulls(
                  providerPerson.getLastName(), sampleOrder.getProviderLastName())
              != 0;
    } else {
      return false;
    }
  ...
  private void createObservationHistoryArtifacts(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts) {
  ...
  private void createOrUpdateObservation(
      String currentUserId,
      List<ObservationHistory> observations,
      String patientId,
      ObservationType observationType,
      String value,
      ValueType valueType) {
    ObservationHistory observation =
        observationHistoryService.getObservationForSample(
            observationType, sampleOrder.getSampleId());
  ...
  private void createOrganizationProviderArtifacts(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts,
      RequesterService requesterService) {
    ...
    SampleRequester orgRequester =
        sampleService.getOrganizationSampleRequester(
  ...
  private void createOrganizationDepartProviderArtifacts(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts,
      RequesterService requesterService) {

    SampleRequester orgDepartRequester =
        sampleService.getOrganizationSampleRequester(
    ...
    if (orgDepartRequester == null) {
      ...
      handleExistingOrganizationDepartRequester(
          sampleOrder, currentUserId, artifacts, orgDepartRequester);
    }
  }

  private void handleNoExistingOrganizationRequester(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts) {
    SampleRequester orgRequester;
    ...
    orgRequester.setRequesterId(
        Long.parseLong(sampleOrder.getReferringSiteId())); // may be overridden latter
  ...
  private void handleNoExistingOrganizationDepartRequester(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts) {
  ...
  private void handleExistingOrganizationRequester(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts,
      SampleRequester orgRequester) {
  ...
  private void handleExistingOrganizationDepartRequester(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts,
      SampleRequester orgRequester) {
    ...
    if (String.valueOf(orgRequester.getRequesterId())
        .equals(sampleOrder.getReferringSiteDepartmentId())) {
      org = orgService.getOrganizationById(String.valueOf(orgRequester.getRequesterId()));

    } else {
      org =
          orgService.getOrganizationById(
              String.valueOf(sampleOrder.getReferringSiteDepartmentId()));
    }
    if (org != null) {
      org.setSysUserId(currentUserId);
      artifacts.setProviderDepartmentOrganization(org);
  ...
  private void updateExistingOrganizationCode(
      SampleOrderItem sampleOrder,
      String currentUserId,
      SampleOrderPersistenceArtifacts artifacts,
      Organization org) {
    if (sampleOrder.getReferringSiteCode() != null
        && !sampleOrder.getReferringSiteCode().equals(org.getCode())) {
      org.setCode(sampleOrder.getReferringSiteCode());
      org.setSysUserId(currentUserId);
    }
  ...
  public class SampleOrderPersistenceArtifacts {
    ...
    public void setDeletableSampleOrganizationRequester(
        SampleRequester deletableSampleOrganizationRequester) {
    ...
    public void setSampleOrganizationDepartRequester(
        SampleRequester sampleOrganizationDepartRequester) {
