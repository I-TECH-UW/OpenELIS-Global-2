...
public class ReportTrackingService implements IReportTrackingService {
  ...
  private void initialize() {
    ...
    NON_CONFORMITY_DOCUMENT_TYPE_ID =
        documentTypeService.getDocumentTypeByName("nonConformityNotification").getId();
    RESULT_EXPORT_DOCUMENT_TYPE_ID =
        documentTypeService.getDocumentTypeByName("resultExport").getId();
    MALARIA_CASE_DOCUMENT_TYPE_ID =
        documentTypeService.getDocumentTypeByName("malariaCase").getId();
  }

  @Override
  @Transactional
  public void addReports(
      List<String> refIds, ReportType type, String name, String currentSystemUserId) {
  ...
  private DocumentTrack getParent(String id, String refTableId, String documentTypeId) {
    List<DocumentTrack> docs =
        documentTrackService.getByTypeRecordAndTable(documentTypeId, refTableId, id);
  ...
  public List<DocumentTrack> getReportsForSample(Sample sample, ReportType type) {
    return documentTrackService.getByTypeRecordAndTable(
        getReportTypeId(type), getReferenceTable(type), sample.getId());
  }

  @Override
  public List<DocumentTrack> getReportsForSampleAndReportName(
      Sample sample, ReportType type, String name) {
    return documentTrackService.getByTypeRecordAndTableAndName(
        getReportTypeId(type), getReferenceTable(type), sample.getId(), name);
  }
