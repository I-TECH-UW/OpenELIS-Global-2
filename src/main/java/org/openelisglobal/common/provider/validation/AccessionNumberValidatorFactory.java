...
public class AccessionNumberValidatorFactory implements ConfigurationListener {
  ...
  private IAccessionNumberGenerator getConfiguredMainGenerator()
      throws LIMSInvalidConfigurationException {

    String accessionFormat =
        ConfigurationProperties.getInstance().getPropertyValueUpperCase(Property.AccessionFormat);
    ...
    synchronized (this) {
      ...
      if (mainGenerator == null) {
        throw new LIMSInvalidConfigurationException(
            "AccessionNumberValidatorFactory: Unable to find generator/validator for "
                + accessionFormat);
      } else {
        return mainGenerator;
      }
  ...
  private IAccessionNumberValidator getAllActiveValidator()
      throws LIMSInvalidConfigurationException {
