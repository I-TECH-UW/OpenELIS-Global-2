...
public class SampleItemTestProvider extends BaseQueryProvider {
  ...
  public void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    /**
     * the name (something derived from html form id) of the check box for the relevant sample item
     * type
     */
  ...
  /**
   * Figure out if the given "test" (actually something of a test panel) Ask the project form mapper
   */
  private boolean wasTestSelected(
      String sampleKey, String projectFormName, String sampleItemType, String testTag)
  ...
  // class.
  public static List<Analysis> findAnalysis(
      String sampleKey, String projectFormName, ProjectData projectData) {
    IProjectFormMapper projectFormMapper =
        new ProjectFormMapperFactory().getProjectInitializer(projectFormName, null);
    ...
    if (typeOfSampleTestsList.size() == 0) {
      throw new IllegalArgumentException(
          "The combination of sampleItem type and test (panel) is not valid.");
    }
  ...
  private boolean wasSampleTypeSelected(
      String sampleId, String projectFormName, String sampleItemType) {
    try {
