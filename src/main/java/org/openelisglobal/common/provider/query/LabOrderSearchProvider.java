...
public class LabOrderSearchProvider extends BaseQueryProvider {
  ...
  private FhirPersistanceService fhirPersistanceService =
      SpringContext.getBean(FhirPersistanceService.class);
  ...
  public void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    if (!eOrders.isEmpty()) {
                ...
                localFhirClient
                    ...
                    .where(
                        ServiceRequest.IDENTIFIER
                            .exactly()
                            .systemAndIdentifier(remotePath, orderNumber))
                    .include(ServiceRequest.INCLUDE_SPECIMEN)
                    .execute();
        for (BundleEntryComponent bundleComponent : srBundle.getEntry()) {
          if (bundleComponent.hasResource()
      ...
      if (serviceRequest != null) {
        ...
        LogEvent.logDebug(
            this.getClass().getSimpleName(), "processRequest", "no matching serviceRequest");
      }
      ...
      if (!GenericValidator.isBlankOrNull(
        ...
        try {
          ...
          LogEvent.logWarn(
              this.getClass().getSimpleName(), "processRequest", "no matching organization");
        }
      }
      ...
      if (!GenericValidator.isBlankOrNull(
        ...
        try {
          ...
          LogEvent.logWarn(
              this.getClass().getSimpleName(), "processRequest", "no matching location");
        }
      }
      ...
      if (!GenericValidator.isBlankOrNull(
        ...
        try {
          ...
          LogEvent.logWarn(
              this.getClass().getSimpleName(), "processRequest", "no matching requester");
        }
      }
      ...
      if (specimen != null
        ...
        try {
          ...
          LogEvent.logWarn(
              this.getClass().getSimpleName(), "processRequest", "no matching collector");
        }
      }
    }
      ...
      if (identifier.hasSystem()) {
        ...
        if (identifier.getSystem().equalsIgnoreCase("iSantePlus ID")
          ...
          patientGuid = identifier.getId();
            ...
            .getSystem()
            .equalsIgnoreCase(fhirConfig.getOeFhirSystem() + "/pat_guid")) {
  ...
  private void addLocation(StringBuilder xml) {
    ...
    if (location != null) {
      org.openelisglobal.organization.valueholder.Organization organization =
          organizationService.getOrganizationByFhirId(location.getIdElement().getIdPart());
    ...
    if (requesterPerson != null) {
      ...
      Provider provider =
          providerService.getProviderByFhirId(
              UUID.fromString(requesterPerson.getIdElement().getIdPart()));
      if (provider != null) {
      ...
      requesterValuesMap.put(
          PROVIDER_FIRST_NAME, requesterPerson.getNameFirstRep().getGivenAsSingleString());
    }
    ...
    if (!GenericValidator.isBlankOrNull(sampleTypeAbbreviation)) {
      ...
      if (!GenericValidator.isBlankOrNull(typeOfSampleId)) {
        ...
        if (typeOfSample != null) {
          test =
              testService
                  .getActiveTestByLoincCodeAndSampleType(loinc, typeOfSample.getId())
                  .orElse(null);
        }
      }
    }
  ...
  private void createMapsForTests(List<Request> testRequests) {
    for (Request testRequest : testRequests) {
      ...
      List<TypeOfSample> sampleTypes =
          typeOfSampleService.getTypeOfSampleForTest(singleTest.getId());
      ...
      if (tests.size() > 1) {
        if (!GenericValidator.isBlankOrNull(testRequest.getSampleType())) {
          for (Test test : tests) {
            List<TypeOfSample> typeOfSamples =
                typeOfSampleService.getTypeOfSampleForTest(test.getId());
            Optional<TypeOfSample> matchingSampleType =
                typeOfSamples.stream()
        ...
        if (!hasSingleSampleType) {
          List<TestSampleType> testSampleTypeList =
              testNameTestSampleTypeMap.get(testRequest.getName());
  ...
  private void createMapsForPanels(List<Request> panelRequests) {
    for (Request panelRequest : panelRequests) {
      ...
      if (panel != null) {
        List<TypeOfSample> typeOfSamples =
            typeOfSampleService.getTypeOfSampleForPanelId(panel.getId());
  ...
  private void addSampleType(
      StringBuilder xml, TypeOfSample typeOfSample, PanelTestLists panelTestLists) {
  ...
  private void addCollection(
      StringBuilder xml, String parent, Specimen specimen, Practitioner collector) {
    ...
    if (specimen != null
      ...
      XMLUtil.appendKeyValue(
          "date",
          DateUtil.formatDateAsText(specimen.getCollection().getCollectedDateTimeType().getValue()),
          xml);
      XMLUtil.appendKeyValue(
          "time",
          DateUtil.formatTimeAsText(specimen.getCollection().getCollectedDateTimeType().getValue()),
          xml);
      XMLUtil.appendKeyValue(
          "dateTime", specimen.getCollection().getCollectedDateTimeType().getValueAsString(), xml);
    }
    if (collector != null) {
      XMLUtil.appendKeyValue(
          "collector",
          collector.getNameFirstRep().getGivenAsSingleString()
              + collector.getNameFirstRep().getFamily(),
          xml);
    }
  ...
  private List<Test> getTestsForPanelAndType(String panelId, String sampleTypeId) {
    List<TypeOfSampleTest> sampleTestList =
    ...
    List<PanelItem> panelList =
        panelItemService.getPanelItemsForPanelAndItemList(panelId, testList);
  ...
  private void addCrosstestForTestName(
      StringBuilder xml, String testName, List<TestSampleType> list) {
  ...
  private void addAlerts(StringBuilder xml, String patientGuid) {
    ...
    if (patient == null) {
      XMLUtil.appendKeyValue(
          "user_alert", MessageUtil.getMessage("electronic.order.warning.missingPatient"), xml);
      ...
      XMLUtil.appendKeyValue(
          "user_alert",
          MessageUtil.getMessage("electroinic.order.warning.missingPatientInfo"),
          xml);
    }
  }
