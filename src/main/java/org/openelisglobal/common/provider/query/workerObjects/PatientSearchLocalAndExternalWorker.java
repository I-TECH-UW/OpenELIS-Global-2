...
public class PatientSearchLocalAndExternalWorker extends PatientSearchWorker {
  ...
  /**
   * @see
   *     org.openelisglobal.common.provider.query.workerObjects.PatientSearchWorker#createSearchResultXML(java.lang.String,
   *     java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String,
   *     java.lang.StringBuilder)
   */
  @Override
  public String createSearchResultXML(
      String lastName,
      String firstName,
      String STNumber,
    ...
    if (config.getPropertyValue(Property.INFO_HIGHWAY_ENABLED).equals("true")) {
      IExternalPatientSearch externalSearch =
          (IExternalPatientSearch) SpringContext.getBean("InfoHighwaySearch");
      externalSearch.setSearchCriteria(
          lastName, firstName, STNumber, subjectNumber, nationalID, guid);
      externalSearch.setConnectionCredentials(
          config.getPropertyValue(Property.INFO_HIGHWAY_ADDRESS),
    ...
    if (config.getPropertyValue(Property.PatientSearchEnabled).equals("true")) {
      ...
      externalSearch.setSearchCriteria(
          lastName, firstName, STNumber, subjectNumber, nationalID, guid);
      externalSearch.setConnectionCredentials(
          config.getPropertyValue(Property.PatientSearchURL),
    ...
    for (IExternalPatientSearch externalSearch : externalSearches) {
      try {
        ...
        Integer externalSearchResult =
            futureExternalSearchResult.get(externalSearch.getTimeout(), TimeUnit.MILLISECONDS);
        ...
        if (externalSearchResult == 200) {
          ...
          LogEvent.logError(
              ...
              "could not get external search results from "
                  + externalSearch.getConnectionString()
                  + " - failed response");
        }
      ...
      newPatientsFromExternalSearch.forEach(
          e -> e.setDataSourceName(MessageUtil.getMessage("patient.imported.source")));
  ...
  private void insertNewPatients(ExtendedPatientSearchResults results) {
    ...
    persistIdentityType(
        patientIdentityService, results.getSubjectNumber(), "SUBJECT", patient.getId());
    persistIdentityType(
        patientIdentityService, results.getMothersName(), "MOTHER", patient.getId());
    persistIdentityType(patientIdentityService, results.getGUID(), "GUID", patient.getId());
    persistIdentityType(
        patientIdentityService, results.getDataSourceId(), "ORG_SITE", patient.getId());
    ...
    patientService.insertNewPatientAddressInfo(
        ADDRESS_PART_COMMUNE_ID, results.getCampCommune(), "T", patient, sysUserId);
    patientService.insertNewPatientAddressInfo(
        ADDRESS_PART_VILLAGE_ID, results.getTown(), "T", patient, sysUserId);
   ...
   * then they will be
   */
  private void findNewPatients(
      List<PatientSearchResults> results,
      List<ExtendedPatientSearchResults> externalResults,
