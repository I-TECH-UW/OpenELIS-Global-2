...
public class TestReflexUserChoiceProvider extends BaseQueryProvider {
  ...
  public void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    if (GenericValidator.isBlankOrNull(resultIds)
    ...
    } else if (resultIds.length() > MAX_FIELD_SIZE
        || testIds.length() > MAX_FIELD_SIZE
        || rowIndex.length() > MAX_FIELD_SIZE
        || analysisIds.length() > MAX_FIELD_SIZE
      ...
      jResult =
          createJsonTestReflex(
              resultIds, analysisIds, testIds, accessionNumber, rowIndex, jsonResult);
  ...
  private String createJsonTestReflex(
      String resultIds,
      String analysisIds,
      String testIds,
    ...
    if (resultIdSeries.length > 1) {
      ...
      List<TestReflex> candidateReflexList =
          reflexUtil.getTestReflexsForDictioanryResultTestId(resultIds, testIds, true);
      ...
      for (TestReflex candidateReflex : candidateReflexList) {
        if (TestReflexUtil.isUserChoiceReflex(candidateReflex)) {
          if (GenericValidator.isBlankOrNull(candidateReflex.getSiblingReflexId())) {
            ...
            reflexTriggers.add(
                TestServiceImpl.getUserLocalizedTestName(candidateReflex.getTest())
                    + ":"
  ...
  private void createTriggerList(
      HashSet<String> reflexTriggers, String reflexTriggerIds, JSONObject jsonResult) {
  ...
  private Sample getSampleForKnownTest(
      String analysisIds, String accessionNumber, AnalysisService analysisService) {
