...
public class SampleEntryTestsForTypeProvider extends BaseQueryProvider {
  ...
  private TestDictionaryService testDictionaryService =
      SpringContext.getBean(TestDictionaryService.class);
  private TypeOfSamplePanelService samplePanelService =
      SpringContext.getBean(TypeOfSamplePanelService.class);
  ...
  public void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    List<IdValuePair> testSections =
        userService.getUserTestSections(String.valueOf(usd.getSystemUserId()), receptionRoleId);
  ...
  private String createSearchResultXML(
      String sampleType, StringBuilder xml, List<String> testUnitIds) {
  ...
  private void addTest(Test test, StringBuilder xml) {
    ...
    XMLUtil.appendKeyValue(
        "userBenchChoice",
        String.valueOf(USER_TEST_SECTION_ID.equals(test.getTestSection().getId())),
        xml);
    if (isVariableTypeOfSample) {
  ...
  private void addVariableSampleTypes(Test test, StringBuilder xml) {
    ...
    if (!GenericValidator.isBlankOrNull(testDictionary.getQualifiableDictionaryId())) {
      XMLUtil.appendAttributeKeyValue(
          "qualifiableId", testDictionary.getQualifiableDictionaryId(), xml);
    }
    xml.append(" >");
  ...
  private List<PanelTestMap> linkTestsToPanels(
      List<TypeOfSamplePanel> panelList, List<Test> tests) {
    ...
    for (TypeOfSamplePanel samplePanel : panelList) {
      ...
      if ("Y".equals(panel.getIsActive())) {
        String matchTests =
            getTestIndexesForPanels(samplePanel.getPanelId(), testNameOrderMap, panelItemService);
        if (!GenericValidator.isBlankOrNull(matchTests)) {
          ...
          selected.add(
              new PanelTestMap(
                  samplePanel.getPanelId(), panelOrder, panel.getLocalizedName(), matchTests));
        }
      }
    }
  ...
  private String getTestIndexesForPanels(
      String panelId, Map<String, Integer> testIdOrderMap, PanelItemService panelItemService) {
    ...
    return withExtraComma.length() > 0
        ? withExtraComma.substring(0, withExtraComma.length() - 1)
        : "";
  }
