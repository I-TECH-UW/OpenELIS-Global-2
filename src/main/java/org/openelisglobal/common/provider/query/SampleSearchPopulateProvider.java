...
public class SampleSearchPopulateProvider extends BaseQueryProvider {
  ...
  public void processRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    boolean unvalidatedTestOnly =
        GenericValidator.isBlankOrNull(request.getParameter("unvalidatedTestOnly"))
            ? false
    ...
    if (!GenericValidator.isBlankOrNull(loinc) && !GenericValidator.isBlankOrNull(patientId)) {
      sample = getSampleForPatientIdAndLoinc(patientId, loinc, unvalidatedTestOnly);
        ...
        && !GenericValidator.isBlankOrNull(loinc)) {
      sample = getSampleByAccessionNumberAndTestId(accessionNo, testId, unvalidatedTestOnly);

      StatusSet statusSet =
          SpringContext.getBean(IStatusService.class).getStatusSetForAccessionNumber(accessionNo);
      ...
      StatusSet statusSet =
          SpringContext.getBean(IStatusService.class).getStatusSetForAccessionNumber(accessionNo);
      patientId = statusSet.getPatientId();
    } else if (!GenericValidator.isBlankOrNull(testId)
  ...
  private Sample getSampleByAccessionNumberAndTestId(
      String accessionNo, String testId, boolean unvalidatedTestOnly) {
    Sample sample = sampleService.getSampleByAccessionNumber(accessionNo);
    if (GenericValidator.isBlankOrNull(testId)
        || (sampleService.sampleContainsTest(sample.getId(), testId)
  ...
  private Sample getSampleByAccessionNumberAndTestLoinc(
      String accessionNo, String loinc, boolean unvalidatedTestOnly) {
    Sample sample = sampleService.getSampleByAccessionNumber(accessionNo);
  ...
  private boolean testNotFinialized(String testId, String sampleId) {
    ...
    List<Analysis> analysises =
        analysisService.getAnalysesBySampleIdTestIdAndStatusId(
            Arrays.asList(Integer.parseInt(sampleId)),
            Arrays.asList(Integer.parseInt(testId)),
            statusList);
  ...
  private boolean testWithLoincNotFinialized(String loinc, String sampleId) {
    ...
    List<Analysis> analysises =
        analysisService.getAnalysesBySampleIdAndStatusId(sampleId, statusList);
    for (Analysis analysis : analysises) {
  ...
  private Sample getSampleForPatientIdAndLoinc(
      String patientId, String loinc, boolean unvalidatedTestOnly) {
  ...
  private Sample getSampleForPatientIdAndTestId(
      String patientId, String testId, boolean unvalidatedTestOnly) {
    List<Sample> samples = sampleHumanService.getSamplesForPatient(patientId);
    if (samples == null || samples.size() == 0) {
      return null;
    }
    ...
    for (Sample sample : samples) {
      if (GenericValidator.isBlankOrNull(testId)
          || (sampleService.sampleContainsTest(sample.getId(), testId)
              && (!unvalidatedTestOnly || testNotFinialized(testId, sample.getId())))) {
        if ((best == null
            || best.getEnteredDate().getTime() <= sample.getEnteredDate().getTime())) {
  ...
  private Sample findBestLoincMatch(
      List<Sample> samples, String loinc, boolean unvalidatedTestOnly) {
    Sample best = null;
    for (Sample sample : samples) {
      if (GenericValidator.isBlankOrNull(loinc)
          || (sampleService.sampleContainsTestWithLoinc(sample.getId(), loinc)
