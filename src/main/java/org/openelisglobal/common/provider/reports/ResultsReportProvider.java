...
/**
 * @author benzd1 bugzilla 2264 bugzilla 1856 move pending tests to top section sort tests
 *     (parent/child recursive by sort order of test) add section previously reported tests
 */
...
public class ResultsReportProvider extends BaseReportsProvider {
  ...
  public ResultsReportProvider() {
    PlatformTransactionManager transactionManager =
        SpringContext.getBean(PlatformTransactionManager.class);
  ...
  public boolean processRequest(
    ...
    File mainReportFile =
        new File(classLoader.getResource("/reports/rslts_main_report.jasper").getFile());
    ...
    // mod)
    File providerDetailsReportFile =
        new File(classLoader.getResource("/reports/rslts_provider_details.jasper").getFile());
    File projectDetailsReportFile =
        new File(classLoader.getResource("/reports/rslts_project_details.jasper").getFile());
    File sourceTypeDetailsReportFile =
        new File(classLoader.getResource("/reports/rslts_sourcetype_details.jasper").getFile());
    File patientDetailsReportFile =
        new File(classLoader.getResource("/reports/rslts_patient_details.jasper").getFile());
    File testResultsReportFile =
        new File(classLoader.getResource("/reports/rslts_test_results.jasper").getFile());
    File resultValueReportFile =
        new File(classLoader.getResource("/reports/rslts_result_value.jasper").getFile());
    File logoGifFile =
        new File(classLoader.getResource("/reports/images/rslts_logo.gif").getFile());
    // bugzilla 1900
    File previewWaterMark =
        new File(classLoader.getResource("/reports/images/rslts_previewwatermark.gif").getFile());
        ...
        new TransactionCallback<Boolean>() {
          ...
          public Boolean doInTransaction(TransactionStatus status) {
            ...
            try {
              ...
              // parameters
              JasperReport providerDetailsReport =
                  (JasperReport) JRLoader.loadObjectFromFile(providerDetailsReportFile.getPath());
              JasperReport projectDetailsReport =
                  (JasperReport) JRLoader.loadObjectFromFile(projectDetailsReportFile.getPath());
              JasperReport sourceTypeDetailsReport =
                  (JasperReport) JRLoader.loadObjectFromFile(sourceTypeDetailsReportFile.getPath());
              JasperReport patientDetailsReport =
                  (JasperReport) JRLoader.loadObjectFromFile(patientDetailsReportFile.getPath());
              JasperReport testResultsReport =
                  (JasperReport) JRLoader.loadObjectFromFile(testResultsReportFile.getPath());
              JasperReport resultValueReport =
                  (JasperReport) JRLoader.loadObjectFromFile(resultValueReportFile.getPath());
              ...
              // bugzilla 1900
              if (resultsReportType.equals(RESULTS_REPORT_TYPE_PREVIEW)) {
                unfilteredListOfCurrentAnalyses =
                    analysisService.getMaxRevisionAnalysesReadyForReportPreviewBySample(
                        accessionNumbers);
              } else {
                unfilteredListOfCurrentAnalyses =
              ...
              sourceMessage =
                  getMessageForKey(request, "label.jasper.results.report.sourceofsample");
              typeMessage = getMessageForKey(request, "label.jasper.results.report.typeofsample");
              sourceOtherMessage =
                  getMessageForKey(request, "label.jasper.results.report.sourceother");
              notApplicableMessage =
                  getMessageForKey(request, "label.jasper.results.report.notapplicable");
              testingPendingMessage =
                  getMessageForKey(request, "label.jasper.results.report.testing.pending");

              currentTestsToReport =
                  populateTests(unfilteredListOfCurrentAnalyses, CURRENT_SECTION);
              ...
              for (int i = 0; i < currentTestsToReport.size(); i++) {
                ...
                if (samples.containsKey(accessionNumber)) {
                  ...
                  if (!StringUtil.isNullorNill(sample.getId())) {
                    ...
                    String patientName =
                        " "
                            + StringUtil.trim(person.getLastName())
                            + ", "
                    ...
                    JRHibernateDataSource resultsReport_Projects =
                        new JRHibernateDataSource(labProjects);
                    ...
                    if (sampleItem.getSourceOfSample() != null
                      ...
                      reportSample.setSourceOfSample(
                          sourceMessage
                              + ":"
              ...
              for (Iterator entryIter = samples.entrySet().iterator(); entryIter.hasNext(); ) {
                ...
                if (resultsReportType.equals(RESULTS_REPORT_TYPE_PREVIEW)) {
                  pendingAnalyses =
                      analysisService.getMaxRevisionPendingAnalysesReadyForReportPreviewBySample(
                          samp.getSample());
                } else {
                  pendingAnalyses =
                      analysisService.getMaxRevisionPendingAnalysesReadyToBeReportedBySample(
                          samp.getSample());
                }
                ...
                List previouslyReportedTests =
                    analysisService.getAnalysesAlreadyReportedBySample(samp.getSample());
                ...
                moveParentTestsOfCurrentTestsFromPreviouslyReportedTests(
                    currentTests, previouslyReportedTests);
                ...
                previouslyReportedTests =
                    completeHierarchyOfTestsForSorting(previouslyReportedTests);
              ...
              UserSessionData usd =
                  (UserSessionData)
                      request.getSession().getAttribute(IActionConstants.USER_SESSION_DATA);
  ...
  private List populatePendingTests(List pendingAnalyses) {
    ...
    for (int i = 0; i < pendingAnalyses.size(); i++) {
      ...
      reportTest.setTestDescription(
          TestServiceImpl.getLocalizedTestNameWithType(pendingAnalysis.getTest()));
      ...
      while (currentIt.hasNext()) {
        ...
        if (!currentTest.getAnalysis().getRevision().equals("0")
          ...
          Analysis previousAnalysis =
              analysisService.getPreviousAnalysisForAmendedAnalysis(currentTest.getAnalysis());
          ResultsReportTest reportTest = new ResultsReportTest();
          ...
          reportTest.setTestMessage(testMessage);
          reportTest.setTestDescription(
              TestServiceImpl.getUserLocalizedTestName(previousAnalysis.getTest()));
