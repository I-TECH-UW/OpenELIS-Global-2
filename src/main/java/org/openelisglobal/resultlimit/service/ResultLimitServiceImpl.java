...
public class ResultLimitServiceImpl extends AuditableBaseObjectServiceImpl<ResultLimit, String>
  ...
  @Autowired protected ResultLimitDAO baseObjectDAO;
  ...
  public void initializeGlobalVariables() {
    ...
    SELECT_LIST_RESULT_TYPE_IDS =
        typeOfTestResultService.getTypeOfTestResultByType("D").getId()
  ...
  @Override
  @Transactional(readOnly = true)
  public String getDisplayNormalRange(
  ...
  @Override
  @Transactional(readOnly = true)
  public String getDisplayReferenceRange(
  ...
  /**
   * Get the valid range for numeric result limits. For other result types an empty string will be
   * returned
   *
  ...
  @Override
  @Transactional(readOnly = true)
  public String getDisplayValidRange(
      ResultLimit resultLimit, String significantDigits, String separator) {
    String range = "";
    if (resultLimit != null && !GenericValidator.isBlankOrNull(resultLimit.getResultTypeId())) {
      if (NUMERIC_RESULT_TYPE_ID.equals(resultLimit.getResultTypeId())) {
        range =
            getDisplayNormalRange(
                resultLimit.getLowValid(),
                resultLimit.getHighValid(),
                significantDigits,
                separator);
      }
    }
  ...
  @Override
  @Transactional(readOnly = true)
  public String getDisplayReportingRange(
      ResultLimit resultLimit, String significantDigits, String separator) {
    String range = "";
    if (resultLimit != null && !GenericValidator.isBlankOrNull(resultLimit.getResultTypeId())) {
      if (NUMERIC_RESULT_TYPE_ID.equals(resultLimit.getResultTypeId())) {
        range =
            getDisplayNormalRange(
                resultLimit.getLowReportingRange(),
                resultLimit.getHighReportingRange(),
                significantDigits,
                separator);
      }
    }
  ...
  /**
   * Get the valid low critical range for numeric result limits. For other result types an empty
  ...
  @Override
  @Transactional(readOnly = true)
  public String getDisplayCriticalRange(
      ResultLimit resultLimit, String significantDigits, String separator) {
    String range = "";
    if (resultLimit != null && !GenericValidator.isBlankOrNull(resultLimit.getResultTypeId())) {
      if (NUMERIC_RESULT_TYPE_ID.equals(resultLimit.getResultTypeId())) {
  ...
  public String getDisplayAgeRange(ResultLimit resultLimit, String separator) {
    ...
    if (resultLimit.getMaxAge() == Float.POSITIVE_INFINITY) {
      return ">"
          + nf.format(minYears)
          + MessageUtil.getContextualMessage("abbreviation.year.single")
          + nf.format(minMonths)
          + MessageUtil.getContextualMessage("abbreviation.month.single")
          + nf.format(minDays)
          + MessageUtil.getContextualMessage("abbreviation.day.single");
    }
  ...
  /**
   * The id in the returned set of IdValuePair refers to the upper end of the age range in months It
   * will be either a number or "Infinity"
   *
  ...
  public List<IdValuePair> getPredefinedAgeRanges() {
    ...
    List<SiteInformation> siteInformationList =
        siteInformationService.getSiteInformationByDomainName("resultAgeRange");
  ...
  public ResultLimit getResultLimitForAnalysis(Analysis analysis) {
    return getResultLimitForTestAndPatient(
        analysis.getTest(),
