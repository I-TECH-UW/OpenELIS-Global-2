...
public class PersonServiceImpl extends AuditableBaseObjectServiceImpl<Person, String>
    implements PersonService {

  private Map<String, String> addressPartIdToNameMap;

  @Autowired protected PersonDAO baseObjectDAO;
  ...
  public Map<String, String> getAddressComponents(Person person) {
    ...
    List<PersonAddress> addressParts =
    ...
    for (PersonAddress parts : addressParts) {
      if ("D".equals(parts.getType()) && !GenericValidator.isBlankOrNull(parts.getValue())) {
        addressMap.put(
            addressPartIdToNameMap.get(parts.getAddressPartId()),
