...
public class NonConformityController extends BaseController {
  ...
  @Autowired private NonConformityFormValidator formValidator;
  ...
  public ModelAndView showNonConformity(
      HttpServletRequest request,
      @Validated(NonConformitySearch.class) NonConformityForm form,
  ...
  private String getSampleRequesterOrganizationName(Sample sample) {
    List<SampleRequester> sampleRequestors =
    ...
    for (SampleRequester sampleRequester : sampleRequestors) {
      if (sampleRequester.getRequesterTypeId() == typeID) {
        ...
        if (org != null
          ...
          orgName +=
              GenericValidator.isBlankOrNull(org.getShortName()) ? "" : ("-" + org.getShortName());
          return orgName;
        }
      }
    }
  ...
  private List<QaEventItem> getSampleQaEventItems(Sample sample) {
    ...
    if (sample != null) {
      ...
      for (SampleQaEvent event : sampleQAEventList) {
        ...
        item.setSampleType(
            (sampleItem == null) ? "-1" : sampleItemService.getTypeOfSampleId(sampleItem));
  ...
  private ObservationHistory getServiceObservation(
      List<ObservationHistory> observationHistoryList) {
    for (ObservationHistory observation : observationHistoryList) {
      if (observation
          .getObservationHistoryTypeId()
  ...
  private List<TestSection> createSectionList() {
    ...
    if (ConfigurationProperties.getInstance()
        .isPropertyValueEqual(Property.NONCONFORMITY_RECEPTION_AS_UNIT, "true")) {
    ...
    if (ConfigurationProperties.getInstance()
        .isPropertyValueEqual(Property.NONCONFORMITY_SAMPLE_COLLECTION_AS_UNIT, "true")) {
      TestSection extra = new TestSection();
  ...
  public ModelAndView showNonConformityUpdate(
      HttpServletRequest request,
      @ModelAttribute("form") @Validated(NonConformityForm.NonConformity.class)
          NonConformityForm form,
      BindingResult result,
