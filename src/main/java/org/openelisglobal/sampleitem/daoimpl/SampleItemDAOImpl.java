...
public class SampleItemDAOImpl extends BaseDAOImpl<SampleItem, String> implements SampleItemDAO {
  ...
  public void getData(SampleItem sampleItem) throws LIMSRuntimeException {
    try {
      SampleItem sampleIt =
          entityManager.unwrap(Session.class).get(SampleItem.class, sampleItem.getId());
      if (sampleIt != null) {
  ...
  public SampleItem getData(String sampleItemId) throws LIMSRuntimeException {
    try {
      SampleItem sampleItem =
          entityManager.unwrap(Session.class).get(SampleItem.class, sampleItemId);
      return sampleItem;
  ...
  public List<SampleItem> getAllSampleItems() throws LIMSRuntimeException {
    ...
    try {
      ...
      Query<SampleItem> query =
          entityManager.unwrap(Session.class).createQuery(sql, SampleItem.class);
      list = query.list();
    } catch (RuntimeException e) {
      LogEvent.logError(e);
        ...
        if (si.getSourceOfSampleId() != null) {
          sos =
              entityManager
                  ...
                  .get(SourceOfSample.class, si.getSourceOfSampleId());
  ...
  public List<SampleItem> getSampleItemsBySampleId(String id) throws LIMSRuntimeException {
    ...
    try {
      String sql =
          "from SampleItem sampleItem where sampleItem.sample.id = :sampleId order by"
  ...
  /**
   * @see
   *     org.openelisglobal.sampleitem.dao.SampleItemDAO#getSampleItemsBySampleIdAndType(java.lang.String,
   *     org.openelisglobal.typeofsample.valueholder.TypeOfSample)
   */
  @Override
  @Transactional(readOnly = true)
  public List<SampleItem> getSampleItemsBySampleIdAndType(
      String sampleId, TypeOfSample typeOfSample) {
    try {
      String sql =
          "from SampleItem si where si.sample.id = :sampleId and si.typeOfSample.id ="
              + " :typeOfSampleId";
      Query<SampleItem> query =
          entityManager.unwrap(Session.class).createQuery(sql, SampleItem.class);
  ...
  public List<SampleItem> getSampleItemsBySampleIdAndStatus(
    ...
    try {
      String sql =
          "from SampleItem sampleItem where sampleItem.sample.id = :sampleId and"
