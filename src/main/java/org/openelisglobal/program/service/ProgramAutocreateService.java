    ...
    if (autocreateOn) {
      for (Resource programResource : programResources) {
        try (BufferedReader reader =
            new BufferedReader(new InputStreamReader(programResource.getInputStream()))) {
          reader.mark(100);
            ...
            while ((linee = reader.readLine()) != null) {
              ...
              System.out.println(
                  "\t\t<insert schemaName=\"clinlims\" tableName=\"localization\">\n"
                      ...
                      + //
                      "\t\t\t<column name=\"english\" value=\""
                      + lines[1].trim()
                      + "\" />\n"
                      + //
                      "\t\t\t<column name=\"french\" value=\""
                      + lines[1].trim()
                      + "\"/>\n"
                      + //
                      "\t\t</insert>\n"
                      ...
                      + lines[2].trim()
                      + "' and description = 'test reporting name' limit 1)\" />\n"
                      + //
                      ...
                      + " clinlims.test where description = '"
                      + lines[1].trim()
                      + "' ) \" />\n"
                      + //
          ...
          if (!GenericValidator.isBlankOrNull(program.getCode())) {
            Optional<Program> dbProgram =
                programService.getMatch("code", form.getProgram().getCode());
          ...
          if (StringUtils.isNotBlank(form.getTestSectionName())) {
            Optional<TestSection> testSection =
                testSectionService.getMatch("testSectionName", form.getTestSectionName());
