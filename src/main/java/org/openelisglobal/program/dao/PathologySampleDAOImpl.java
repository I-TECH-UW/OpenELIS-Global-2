...
@Component
@Transactional
public class PathologySampleDAOImpl extends BaseDAOImpl<PathologySample, Integer>
    implements PathologySampleDAO {
  ...
  public List<PathologySample> getWithStatus(List<PathologyStatus> statuses) {
    ...
    Query<PathologySample> query =
        entityManager.unwrap(Session.class).createQuery(sql, PathologySample.class);
  ...
  @Override
  public List<PathologySample> searchWithStatusAndAccesionNumber(
      List<PathologyStatus> statuses, String labNumber) {
    String sql =
        "from PathologySample ps where ps.status in (:statuses) and ps.sample.accessionNumber ="
            + " :labNumber";
    Query<PathologySample> query =
        entityManager.unwrap(Session.class).createQuery(sql, PathologySample.class);
    query.setParameterList(
  ...
  public Long getCountWithStatusBetweenDates(
    ...
    String sql =
        "select count(*) from PathologySample ps where ps.status in (:statuses) and ps.lastupdated"
