...
public class TestCalculatedUtil {
  ...
  private ResultCalculationService resultcalculationService =
      SpringContext.getBean(ResultCalculationService.class);

  private TestCalculationService calculationService =
      SpringContext.getBean(TestCalculationService.class);
  ...
  public List<Analysis> addNewTestsToDBForCalculatedTests(
      List<ResultSet> resultSetList, String sysUserId) throws IllegalStateException {
    ...
    for (ResultSet resultSet : resultSetList) {
      ...
      for (Calculation calculation : calculations) {
        ...
        List<ResultCalculation> resultCalculations =
            resultcalculationService.getResultCalculationByPatientAndCalculation(
                resultSet.patient, calculation);
    ...
    for (ResultSet resultSet : resultSetList) {
      ...
      if (resultSet.result.getTestResult() == null) {
        ...
        resultCalculations =
            resultcalculationService.getResultCalculationByPatientAndTest(
      ...
      if (!resultCalculations.isEmpty()) {
        for (ResultCalculation resultCalculation : resultCalculations) {
          ...
          for (Map.Entry<Integer, Integer> entry :
              resultCalculation.getTestResultMap().entrySet()) {
                    ...
                    operation -> {
                      switch (operation.getType()) {
                        ...
                        case PATIENT_ATTRIBUTE:
                          if (operation
                              .getValue()
                              .equals(Operation.PatientAttribute.AGE.toString())) {
                            int age =
                                DateUtil.getAgeInYears(
                                    new Date(resultSet.patient.getBirthDate().getTime()),
                                    new Date());
                            function.append(age);
                          }
                          break;
  ...
  private Analysis createCalculatedResult(
    ...
    if (test != null) {
      ...
      ResultLimit resultLimit =
          resultLimitService.getResultLimitForTestAndPatient(
              test.getId(), resultCalculation.getPatient());
      if (resultLimit != null) {
      ...
      if (resultCalculation.getResult() != null) {
        analysis =
            createCalculatedAnalysis(
                resultCalculation.getResult().getAnalysis(),
                test,
                resultSet.result,
                value,
                calculation.getName(),
  ...
  private void createInternalNote(
    ...
    Note note =
        noteService.createSavableNote(
            newAnalysis,
            NoteType.INTERNAL,
            "Result Succesfully Calculated From Calculation Rule :" + calculatioName,
            CALCULATION_SUBJECT,
            systemUserId);
    if (!noteService.duplicateNoteExists(note)) {
      notes.add(note);
    ...
    if (StringUtils.isNotBlank(externalNote)) {
      Note note3 =
          noteService.createSavableNote(
              newAnalysis, NoteType.EXTERNAL, externalNote, CALCULATION_SUBJECT, systemUserId);
  ...
  private void createMissingValueInternalNote(
      Analysis newAnalysis, Analysis currentAnalysis, String calculatioName, String systemUserId) {
    List<Note> notes = new ArrayList<>();
    Note note =
        noteService.createSavableNote(
            newAnalysis,
            NoteType.INTERNAL,
            "Result Missing Calculation Parameters From Calculation Rule : " + calculatioName,
            CALCULATION_SUBJECT,
            systemUserId);
    if (!noteService.duplicateNoteExists(note)) {
      notes.add(note);
    ...
    if ("D".equals(resultType)) {
      ...
      testResult =
          testResultService.getTestResultsByTestAndDictonaryResult(
              test.getId(), calculation.getResult());
      return testResult;
    } else {
  ...
  private void addNumericOperation(
      Operation operation,
      ResultCalculation resultCalculation,
  ...
  private Analysis createCalculatedAnalysis(
    ...
    if (resultCalculated) {
      createInternalNote(
          generatedAnalysis, currentAnalysis, calculationName, systemUserId, externalNote);
    } else {
      createMissingValueInternalNote(
          generatedAnalysis, currentAnalysis, calculationName, systemUserId);
    }
