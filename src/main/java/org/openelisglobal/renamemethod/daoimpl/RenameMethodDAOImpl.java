...
@Component
@Transactional
public class RenameMethodDAOImpl extends BaseDAOImpl<RenameMethod, String>
    implements RenameMethodDAO {
    ...
    try {
      String sql =
          "from Method m where upper(m.methodName) like upper(:param) and m.isActive='Y' order by"
              + " upper(m.methodName)";
      Query<RenameMethod> query =
          entityManager.unwrap(Session.class).createQuery(sql, RenameMethod.class);
