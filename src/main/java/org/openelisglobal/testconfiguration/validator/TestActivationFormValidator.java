      ...
      if (!JSONUtils.isEmpty(changeList)) {
        ...
        for (int i = 0; i < activateTests.size(); ++i) {
          ...
          ValidationHelper.validateIdField(
              StringUtil.nullSafeToString(activateTest.get("id")),
              "JsonChangeList",
              "id",
              errors,
              true);
          ...
          ValidationHelper.validateFieldAndCharset(
              StringUtil.nullSafeToString(activateTest.get("sortOrder")),
              "JsonChangeList",
              "sort order[" + i + "]",
              errors,
              true,
          ...
          ValidationHelper.validateField(
              StringUtil.nullSafeToString(activateTest.get("activated")),
              "JsonChangeList",
              "activated[" + i + "]",
              errors,
              true,
        ...
        for (int i = 0; i < deactivateTests.size(); ++i) {
          ...
          ValidationHelper.validateIdField(
              StringUtil.nullSafeToString(deactivateTest.get("id")),
              "JsonChangeList",
              "id[" + i + "]",
              errors,
              true);
