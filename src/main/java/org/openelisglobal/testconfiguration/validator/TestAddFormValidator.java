    ...
    try {
      ...
      if (!JSONUtils.isEmpty(newTest)) {
        ...
        ValidationHelper.validateYNField(
            StringUtil.nullSafeToString(newTest.get("antimicrobialResistance")),
            "JsonWad",
        ...
        ValidationHelper.validateYNField(
            StringUtil.nullSafeToString(newTest.get("inLabOnly")), "JsonWad", "inLabOnly", errors);
        ...
        for (int i = 0; i < sampleTypes.size(); ++i) {
          ...
          ValidationHelper.validateIdField(
              StringUtil.nullSafeToString(sampleType.get("typeId")), "JsonWad", errors, false);

          JSONArray tests = JSONUtils.getAsArray(sampleType.get("tests"));
          for (int j = 0; j < tests.size(); ++j) {
            JSONObject test = JSONUtils.getAsObject(tests.get(j));
            ValidationHelper.validateIdField(
                StringUtil.nullSafeToString(test.get("id")), "JsonWad", errors, false);
          }
        }
        ...
        if (newTest.containsKey("lowValid")) {
          ...
          for (int i = 0; i < resultLimits.size(); ++i) {
            ...
            ValidationHelper.validateField(
                StringUtil.nullSafeToString(resultLimit.get("lowReportingRange")),
                "JsonWad",
                "result limit [" + i + "] lowReportingRange",
                errors,
                false,
                255,
                ValidationHelper.FLOAT_REGEX);
            ...
            ValidationHelper.validateField(
                StringUtil.nullSafeToString(resultLimit.get("highReportingRange")),
                "JsonWad",
                "result limit [" + i + "] highReportingRange",
                errors,
                false,
                255,
                ValidationHelper.FLOAT_REGEX);
