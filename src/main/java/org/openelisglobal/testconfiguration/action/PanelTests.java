...
public class PanelTests {
  ...
  public PanelTests(IdValuePair panelPair) {
    ...
    List<TypeOfSamplePanel> typeOfSamplePanels =
        typeOfSamplePanelService.getTypeOfSamplePanelsForPanel(panelPair.getId());
    ...
    if (typeOfSamples != null && typeOfSamples.size() > 0) {
      ...
      sampleTypeIdValuePair =
          new IdValuePair(typeOfSample.getId(), typeOfSample.getLocalizedName());
    }
  }
  ...
  public void setTests(List<IdValuePair> tests, Set<String> testIdSet) {
    this.tests = tests;
    List<Test> allTests =
        typeOfSampleService.getAllTestsBySampleTypeId(sampleTypeIdValuePair.getId());
