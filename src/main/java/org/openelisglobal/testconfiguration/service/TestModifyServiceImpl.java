...
public class TestModifyServiceImpl implements TestModifyService {
  ...
  public void updateTestSets(
    ...
    List<TypeOfSampleTest> typeOfSampleTest =
        typeOfSampleTestService.getTypeOfSampleTestsForTest(testAddParams.testId);
    ...
    List<ResultLimit> resultLimitItems =
        resultLimitService.getAllResultLimitsForTest(testAddParams.testId);
    ...
    for (TestSet set : testSets) {
      ...
      updateTestNames(
          testAddParams.testId, nameLocalization, reportingNameLocalization, currentUserId);
      updateTestEntities(
          testAddParams.testId,
          testAddParams.loinc,
          currentUserId,
          testAddParams.uomId,
          testAddParams.testSectionId,
          set.test.isNotifyResults(),
          set.test.isInLabOnly(),
      ...
      for (PanelItem item : set.panelItems) {
        ...
        if (item.getPanel() != null) {
          ...
          if (typeOfSamplePanelService
              .getTypeOfSamplePanelsForSampleType(sampleType.getId())
              .isEmpty()) {
  ...
  private void updateTestEntities(
      String testId,
      String loinc,
      String userId,
      String uomId,
      String testSectionId,
      boolean notifyResults,
      boolean inLabOnly,
      boolean antimicrobialResistance) {
    Test test = testService.get(testId);
  ...
  private void updateTestNames(
    ...
    if (test != null) {
      ...
      for (Locale locale : localizationService.getAllActiveLocales()) {
        ...
        reportingName.setLocalizedValue(
            locale, reportingNameLocalization.getLocalizedValue(locale).trim());
      }
