...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:java="http://www.oracle.com/XSL/Transform/java">
  ...
  <xsl:template match="sampleItem">
     <xsl:if test="count(sortOrder) != 0"> 
       <tr>
      <td width="33%">
       <xsl:value-of select="java:getString($resources,'sampleitem.sortOrder')"/>
      </td>
      <td width="33%">      
      <i>
      ...
      <tr>
      <td width="33%">
       <xsl:value-of select="java:getString($resources,'resultsentry.sampleSource')"/>
      </td>
      <td width="33%">
       <i>
      ...
      <tr>
      <td width="33%">
       <xsl:value-of select="java:getString($resources,'resultsentry.sampleType')"/>
      </td>
      <td width="33%">
      <i>
     ...
     <xsl:if test="count(sourceOther) != 0"> 
      <tr>
      <td width="33%">
       <xsl:value-of select="java:getString($resources,'resultsentry.sourceOther')"/>
      </td>
      <td>
      <i>
      ...
      <tr>
      <td width="33%">
        <xsl:value-of select="java:getString($resources,'sampleitem.quantity')"/>
      </td>
      <td width="33%">
      <i>
