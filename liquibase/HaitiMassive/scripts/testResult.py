...
results = open("MassiveTestResults.sql", 'w')
...
for row in range(0, len(test_names)):
    if len(test_names[row]) > 1 and len(current[row]) < 2: #it's a new entry
        ...
        if description not in descriptions:
            ...
            if result_type == 'D' or result_type == 'M':
                ...
                for j in range(0, len(split_selections)):
                    dictionary_select = " ( select max(id) from clinlims.dictionary where dict_entry ='" + split_selections[j].strip() + "' ) "
