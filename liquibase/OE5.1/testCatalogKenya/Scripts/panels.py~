...
for row in range(0, len(panels)):
        ...
        if panel_name not in used and 'n/a' not in panel_name:
            ...
            panel_results.write(" '" + panel_name.strip() + "' , " + esc_name(panel_name) + " , now() ,'panel.name." + panel_name.split()[0] + "' ," + str(count) + ");\n")
            count += 10
