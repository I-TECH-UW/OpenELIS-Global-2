...
insert_validation_head = "INSERT INTO result_limits(  id, test_id, test_result_type_id, lastupdated, always_validate) \n\t"
...
select_freeText_type = " (select id from clinlims.type_of_test_result where test_result_type = 'R' ) "
...
for row in range(0, len(test_names)):
    ...
    elif  'Validation' in result_type[row]:
           description = esc_char(test_names[row] + "(" + sample_types[row] + ")")
           if description not in descriptions:
               descriptions.append(description)
               dict_normal_results.append( insert_validation_head )
               dict_normal_results.append( next_val + ", ( select id from clinlims.test where description = " + description + " ) ," + select_freeText_type + ", " )
               dict_normal_results.append( " now() , true);\n")

for line in dict_normal_results:
    result.write(line)
    
result.write("MOVE TO TOP OF FILE and clean end\n")
result.write("DELETE from result_limits where test_id in ( select id from test where description in (\n")
