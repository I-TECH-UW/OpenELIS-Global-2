...
sql_head = "INSERT INTO test( id,  uom_id, description, reporting_description, is_active, is_reportable, lastupdated, test_section_id, local_abbrev, sort_order, name, loinc )\n\t"
...
for row in range(0, len(test_names)):
    if len(test_names[row]) > 1:
        ...
        if description not in descriptions:
                    ...
                    results.write( esc_char(test_names[row][:20]) + " ," + str(sort_count) + " , " + esc_char(test_names[row]) + " , " + esc_char(loinc_codes[row] ) + ");\n")
                    sort_count += 10
