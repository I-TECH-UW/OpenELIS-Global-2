import { Button, Column, Grid, Heading, Link, Section, Table, TableBody, TableCell, TableHead, TableHeader, TableRow} from "@carbon/react";
...
export default function Workplan(props) {
    ...
    const printWorkplan = () =>{
        ...
        postToOpenElisServerForPDF('/rest/printWorkplanReport',JSON.stringify(form))
    }

    const buildWorlplanTestFromTestList = (rawData) =>{
        let output = []
            const propertiesToKeep= ["accessionNumber","patientInfo","receivedDate","testName","notIncludedInWorkplan"]
    ...
    return (
        <>
            <Grid fullWidth={true}>
                <Column lg={16}>
                    <Section>
                        <Heading className="contentHeader1">
                            {title}
                        </Heading>
                    </Section>
                </Column><br /><br />
            ...
            <Grid fullWidth={true}>
                <Column lg={16}>
                    <WorkplanSearchForm type={type} createTestsList={handleTestsList} 
            ...
            <Grid fullWidth={true}>
                <Column lg={16}>
                    <Button size='md' type="button" name="print" id="print"  onClick={printWorkplan}>
            ...
            <Grid fullWidth={true}>
                <Column lg={16}>
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <img src={`images/nonconforming.gif`} alt="nonconforming"/> = <FormattedMessage id="result.nonconforming.item" />
                    <br/><br/>
                </Column>
            </Grid>
            <Grid fullWidth={true}>
                    ...
                    <>
                        <Table size={"sm"}>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableHeader>
                                        ...
                                        <FormattedMessage id="sample.entry.project.patient.and.testName" values={{br: <br/>}}/> 
                                        ): 
                            ...
                            {testsList.map((row, index) => {
                                ...
                                return (<TableRow key={index} id={'row_'+index} className={rowColorIndex%2 === 0 ?'evenRow':'oddRow'}>
                                        {!row.servingAsTestGroupIdentifier && (
                                            <TableCell>
                                                <input type="checkbox" value={row.notIncludedInWorkplan} 
                                                id={'includedCheck_'+index} className="includedCheck" 
                                                onClick={e=>disableEnableTest(e.target,index)}/>
                                            </TableCell>
                                        )}
                                        {type === 'test' &&
                                        <TableCell>
                                            {row.nonconforming && 
                                                <img src={`images/nonconforming.gif`}  alt="nonconforming"/>
                                            }
                                        </TableCell>
                                        }
                                        <TableCell>
                                            {showAccessionNumber && (
                                                <Link style={{color:"blue"}} href="#"><u>{row.accessionNumber}</u></Link>
                                                )}
                                        </TableCell>
                                        {(subjectOnWorkplan.toLowerCase() === 'true') &&(
                                            <TableCell>
            ...
            <h2><FormattedMessage id="result.noTestsFound" /></h2>
}
        </>
    );
}